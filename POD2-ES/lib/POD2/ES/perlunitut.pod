...
valor ordinal de un carácter se denomina B<código de carácter> (I<code point>).
En la práctica, la distinción entre los conceptos de código de carácter y
carácter es difusa, por lo que se suelen utilizar ambos términos
indistintamente.
...
número de bytes, en lugar del número de caracteres. Ya no se podrá saber cuál
es el número de caracteres, puesto que los caracteres solo existen en las
cadenas de texto.
