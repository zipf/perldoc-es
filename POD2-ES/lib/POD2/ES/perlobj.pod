...
como primer argumento al método. Esto significa que la llamada C<<Bicho->new()
>>, hace que el método C<new()> reciba la cadena C<"Bicho"> como primer
argumento. En la llamada C<< $alfredo->decir() >>, la variable C<$alfredo> se
pasa a C<decir()> como primer argumento.

Al igual que cualquier subrutina en Perl, todos los argumentos pasados en C<@_>
son alias del argumento original. Esto incluye al propio objeto. Si asigna
...
producirá un error; consulte la sección L</Variantes en las llamadas a los
...
De forma análoga, la función C<ref> trata de forma especial las referencias a
objetos bendecidos. Si llama a C<ref($algo)> y C<$algo> contiene una referencia
a un objeto, devolverá el nombre de la clase en la que se bendijo el objeto.
...
Le recomendamos que use este método para acceder a otra versión del paquete,
en lugar de mirar directamente en C<$Paquete::VERSION>. El paquete en que se
comprueba podría haber sobrescrito el método C<VERSION>.
...
objetos contenidos:
  ...
  sub DESTROY {
      ...
      $self->{controlador}->cerrar() if $self->{identificador};
  }
...
global, el intérprete de Perl agregará la cadena "during global destruction"
(durante la destrucción global) a la advertencia.
...
Veamos un ejemplo simple de esta técnica, basado en módulo
