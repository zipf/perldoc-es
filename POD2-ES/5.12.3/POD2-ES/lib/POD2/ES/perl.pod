...
extraer información y utilizarla para generar informes.  También es útil para
muchas tareas de administración de sistemas.  El lenguaje se diseñó para ser
práctico (fácil de usar, eficaz, completo), más que estético (reducido,
...
lenguajes aprenderán Perl fácilmente  (los historiadores de lenguajes también
destacarán algunos vestigio de B<csh>, Pascal e incluso BASIC-PLUS). La
sintaxis de expresiones es muy parecida a la de C.  A diferencia de la mayoría
de las utilidades Unix, Perl no limita de forma arbitraria el tamaño de los
...
para el procesamiento de texto, también se puede utilizar para procesar datos
binarios, y permite utilizar dbm como si fueran hashes.  Los scripts setuid de
Perl son más seguros que los programas en C, gracias a un mecanismo de
seguimiento de flujo de datos que los protege de muchas vulnerabilidades.

Si tiene que resolver un problema para el que normalmente utilizaría B<sed>,
B<awk> o B<sh>, pero necesita más capacidad o velocidad, y prefiere evitar la
complejidad de programar en C, entonces Perl es lo que busca.  También existen
traductores que convierten sus scripts de B<sed> y B<awk> en scripts de Perl.
...
Puede notificar sus informes de error a través de la dirección perlbug@perl.org
(no olvide incluir toda la información de configuración emitida por el programa
myconfig en el árbol de código fuente de perl, o por C<perl -V>).  Si ha
logrado compilar perl, puede utilizar el script B<perlbug> del subdirectorio
F<utils/> para notificar informes de error.

En realidad Perl significa Pathologically Eclectic Rubbish Lister (Presentador
de basura patológicamente ecléctico), pero nadie debe enterarse de esto :-).
