...
Además del intérprete de Perl, la distribución de Perl instala en el sistema
varias utilidades. La distribución de Perl también usa algunas utilidades
durante el proceso de instalación. En este documento se muestra la lista de
utilidades, se explica para que sirven y se proporcionan referencias a la
documentación de cada módulo, si la hay.
...
Al redactar documentación POD, puede usar la utilidad F<podchecker> para buscar
errores de marcas POD.

=item L<splain|splain>

F<splain> es una interfaz de L<perldiag>; si pega un mensaje de error, mostrará
una descripción.

=item L<roffitall|roffitall>

La utilidad C<roffitall> no se instala en el sistema, pero está dentro del
directorio F<pod/> del kit de código fuente de Perl. Esta utilidad convierte
toda la documentación de la distribución al formato F<*roff> y a partir de esto
genera un código fuente PostScript de o un archivo de texto.

=back

=head2 Conversores

Hemos incluido tres filtros de conversión que le ayudarán a convertir programas
a Perl:
...
Además de estos filtros para convertir a otros lenguajes, la utilidad
...
del código necesario para comunicar las bibliotecas C y los módulos Perl.
...
Perl incluye un perfilador, el módulo F<Devel::DProf>. La utilidad F<dprofpp>
analiza los resultados del perfilador e indica cuáles son las subrutinas que
consumen más tiempo. Consulte L<Devel::DProf> para obtener más información.
...
principalmente como una demostración de tecnología para Perl 5.8.0, pero en
...
CPAN. El código de compatibilidad con los distintos formatos específicos se
encuentra en los módulos C<CPANPLUS::Dist::*> de CPAN.
