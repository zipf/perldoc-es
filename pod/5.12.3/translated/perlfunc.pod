...
Para funciones que pueden ser usadas tanto en contexto escalar como en lista, un error no abortivo es indicado, generalmente, devolviendo el valor indefinido en el contexto escalar, y en contexto lista devolviendo la lista vacía.
...
Llama a flock(2), o una emulación de ella, sobre GESTOR_ARCHIVO.  Devuelve verdadero para éxito, falso en caso de error.  Produce un error fatal si se usa en una máquina que no implemente flock(2), bloqueo fcntl(2), o lockf(3).
...
Devuelve C<1> para el éxito; en caso de error, devuelve C<undef> si el primer argumento no es un gestor de archivo válido, o devuelve C<0> y actualiza C<$!> por cualquier otro error.
