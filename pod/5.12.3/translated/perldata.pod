...
X<__FILE__> X<__LINE__> X<__PACKAGE__> X<line> X<línea> X<file> X<archivo> X<package> X<paquete>
...
Una forma cotidiana de acceder a un array o un hash es, cada vez, por un elemento escalar.  También puede subindexar una lista para obtener un solo elemento de ella.
...
Otra forma de crear gestores de archivo anónimos es con el módulo Symbol o con el módulo IO::Handle y sus acólitos.  Estos módulos tienen la ventaja de no ocultar diferentes tipos del mismo nombre durante el local().  Ver la parte inferior de L<perlfunc/open()> para un ejemplo.
