...
Si hay múltiples hilos en ejecución, C<close> en un gestor de archivo desde un open entubado, devuelve verdadero sin esperar a que el proceso hijo termine, si el gestor de archivo sigue abierto en otro hilo.
...
Esta forma de llamada de subrutina ha sido depreciada.  SUBRUTINA puede ser una palabra simple, una variable escalar o una subrutina comenzando por C<&>.
...
excepto que es más eficiente y conciso, mantiene el nombre del archivo en los mensajes de error, busca en los directorios C<@INC> y actualiza C<%INC> si el archivo es encontrado.  Ver L<perlvar/@INC> y L<perlvar/%INC> por estas variables.  También difiere en que el código evaluado con C<do NOMBREARCHIVO> no puede ver a las variables léxicas en el ámbito en que se ejecuta; C<eval STRING> sí.  Es lo mismo, sin embargo, en que vuelve a interpretar el archivo cada vez que lo llama, así que, probablemente, no querrá hacer esto dentro de un bucle.

Si C<do> puede leer el archivo pero no puede compilarlo, devuelve undef y pone un mensaje de error en C<$@>.  Si C<do> no puede leer el archivo, devuelve undef y pone C<$!> al valor del error.  Siempre revise C<$@> en primer lugar, la compilación puede fallar de una manera que también actualice C<$!>.  Si el archivo se compila bien, C<do> devuelve el valor de la última expresión evaluada.
...
A partir de Perl 5.14, C<each> puede tomar una EXPR escalar, que debe contener la referencia a un hash o un array no bendecido.  El argumento es desreferenciado automáticamente.  Este aspecto de C<each> es considerado altamente experimental.
...
Si hay algún error de sintaxis o en tiempo de ejecución, o se ejecuta una instrucción C<die>, C<eval> devuelve un valor indefinido en contexto escalar o una lista vacía en contexto lista -o, para errores sintácticos, una lista conteniendo un único valor indefinido- y C<$@> es inicializado al mensaje de error.  La discrepancia en los valores de retorno en el contexto de lista es considerado un error por parte de algunos, y probablemente será corregido en futuras versiones.  Si no hay errores, se garantiza que C<$@> será una cadena vacía.  Tenga cuidado porque usar C<eval> ni silencia a Perl a la hora de imprimir avisos al STDERR, ni rellena el texto de los mensajes de aviso dentro de C<$@>.
...
Devuelve el descriptor de archivo para un gestor, o indefinido si el gestor no está abierto.  Si no hay un descriptor de archivo real al nivel del sistema operativo, como puede suceder con gestores de archivo relacionados con objetos de memoria a través de C<open> con una referencia para el tercer argumento, se devuelve -1.
...
Si está intentando usar esto en lugar de C<write> para capturar la salida, puede encontrar que sea más sencillo abrir un gestor de archivo en un escalar (C<< open $fh, ">", \$salida >>) y escribir en él.
...
Si el paquete actual tiene un subrutina denominada C<ToLower>, será utilizada para cambiar el tamaño de caja (ver L<perlunicode/Mapeos de caja definidos por el usuario (solo para hackers serios)>). De lo contrario se utiliza la semántica Unicode para el cambio de tamaño de caja.
...
Semántica Unicode es utilizada para el cambio de tamaño de caja.  Cualquier subrutina denominada C<ToLower> será ignorada.
...
Para obtener algo similar, pero con cadenas de fecha dependiente de la configuración región, ajuste apropiadamente sus variables de entorno a la región (por favor, véase L<perllocale>) e intente, por ejemplo:
...
Contamina la variable.  Devuelve verdadero si tiene éxito, o falso si hay un error.  Véase también la documentación de L<perlipc/"SysV IPC"> y C<IPC::SysV> y C<IPC::SysV::Msg>.

=item msgsnd ID,MSG,FLAGS
X<msgsnd>

Llama a la función msgsnd de System V IPC para enviar el mensaje MSG a la cola de mensajes ID.  MSG debe comenzar con un tipo de mensaje, contenido en un entero largo nativo, seguido por la longitud del mensaje, y finalmente el mensaje mismo.  Este tipo de empaquetado se puede lograr con C<pack("l! a*", $tipo, $mensaje)>.  Devuelve verdadero si tiene éxito, o falso si hay un error.  Véase también la documentación de C<IPC::SysV> y C<IPC::SysV::Msg>.
...
(No se le permite hacer un C<open> a un comando que entube tanto en la entrada I<como> en la salida, pero véase L<IPC::Open2>, L<IPC::Open3>, y L<perlipc/"Comunicación bidireccional con otro proceso"> para alternativas sobre esto).
...
Los dos últimos ejemplos de cada bloque muestra la tubería como "forma de lista", que todavía no está soportado en todas las plataformas.  Una buena regla general es que si la plataforma tiene un verdadero C<fork()> (en otras palabras, si su plataforma es Unix), puede utilizar la forma de lista.
...
El nombre del archivo pasado a las formas de dos argumentos (o de un argumento) de open() terminará con los espacios en blanco iniciales y finales eliminados, y activada la redirección por parte de los caracteres especiales.  Esta propiedad, conocida como "open() mágico", a menudo se puede utilizar con buenos resultados.  Un usuario podría especificar un nombre de archivo de F<"rsh cat archivo |">, o usted puede cambiar ciertos nombres de archivo, según lo que necesite:
...
Devuelve el valor numérico (en la codificación nativa de 8 bits, como ASCII o EBCDIC, o Unicode) del primer carácter de EXPR.  Si EXPR es una cadena vacía, devuelve 0.  Si se omite EXPR, se usa C<$_>.
...
De esta manera, C<our> difiere de C<use vars>, en que su ámbito es el paquete.
...
Si I<secuencia-item> se refiere a un tipo de cadena de caracteres (C<"A">, C<"a">, o C<"Z">), la I<longitud-item> es la longitud de la cadena, no el número de cadenas.  Con un contador de repeticiones explícito para el empaquetado, la cadena de empaquetado se ajusta a esa longitud.  Por ejemplo:
    ...
    unpack("W/a", "\004Gurusamy")           da ("Guru")
...
Declara el BLOQUE, o el resto de la unidad de compilación, como pertenecientes al espacio de nombres especificado.  El alcance de la declaración del paquete es, el BLOQUE indicado o, en ausencia de un BLOQUE, desde la misma declaración hasta el final del bloque que lo engloba, un archivo o eval (lo mismo que el operador C<my>).  Todos los identificadores dinámicos no cualificados en este ámbito pertenecerán al espacio de nombres indicado, salvo que sea reemplazado por otra declaración C<package>.
...
A partir de Perl 5.14, C<pop> puede tomar una EXPR escalar, que debe contener la referencia a un hash o un array no bendecido.  El argumento es desreferenciado automáticamente.  Este aspecto de C<pop> es considerado altamente experimental.
El comportamiento exacto puede cambiar en una futura versión de Perl.
...
A partir de Perl 5.14, C<push> puede tomar una EXPR escalar, que debe contener la referencia a un hash o un array no bendecido.  El argumento es desreferenciado automáticamente.  Este aspecto de C<push> es considerado altamente experimental.
El comportamiento exacto puede cambiar en una futura versión de Perl.
...
En Perl 5.14, todos los caracteres cuyo código de punto esté por encima de 127, no son escapadas como strings codificados en UTF-8, sino que todos son escapados en strings UTF-8.
Está previsto cambiar este comportamiento en 5.16, pero las reglas exactas no se han determinado todavía.
...
A partir de Perl 5.14, C<shift> puede tomar una EXPR escalar, que debe contener la referencia a un hash o un array no bendecido.  El argumento es desreferenciado automáticamente.  Este aspecto de C<shift> es considerado altamente experimental.
El comportamiento exacto puede cambiar en una futura versión de Perl.
...
=item splice ARRAY o DESPLAZAMIENTO,LONGITUD,LISTA
X<splice>
...
A partir de Perl 5.14, C<splice> puede tomar una EXPR escalar, que debe contener la referencia a un hash o un array no bendecido.  El argumento es desreferenciado automáticamente.  Este aspecto de C<splice> es considerado altamente experimental.
El comportamiento exacto puede cambiar en una futura versión de Perl.
...
Para conversiones numéricas, se puede especificar el tamaño del número a interpretar usando C<l>, C<h>, C<V>, C<q>, C<L>, o C<ll>. Para las conversiones de entero (C<d u o x X b i D U O>), se asume que los números son, por lo general, de un tamaño, por defecto, de un entero de su plataforma (normalmente 32 or 64 bits), pero esto se puede reemplazar usando en su lugar uno de los tipos estándares de C, soportados por el compilador usado para construir Perl:
   ...
   hh          interpreta un entero como un "char" o "unsigned char" del C
	       en Perl 5.14 o superior
   h           interpreta un entero como un "short" o "unsigned short" del C
   j           interpreta un entero como un "intmax_t" del C en Perl 5.14 
	       o superior, y solo con un compilador C99 (no portable)
   l           interpreta un entero como un "long" o "unsigned long" del C
   q, L, or ll interpreta un entero como un "long long", "unsigned long long",
               o "quad" del C (típicamente enteros de 64-bit)
   t           interpreta un entero como "ptrdiff_t" del C en Perl 5.14 o superior
   z           interpreta un entero como "size_t" del C en Perl 5.14 o superior
...
La misión de la función es "alimentar" a la función C<rand> para que C<rand> pueda producir una secuencia diferente cada vez que ejecute su programa.  Cuando se llama con un parámetro, C<srand> la usa para la semilla; de lo contrario elige pseudo-aleatoriamente una semilla.  En cualquier caso, a partir de Perl 5.14, devuelve la semilla.

Si C<srand()> no es llamada de forma explícita, es llamada implícitamente en el primer uso del operador C<rand>.  Sin embargo, esto no fue cierto para las versiones de Perl anteriores a la 5.004, así que si su programa se va a ejecutar en versiones anteriores de Perl, debería llamar a C<srand>; de lo contrario, la mayor parte de los programas no llamarán nunca a C<srand>.

Pero hay unas pocas situaciones en programas recientes de Perl donde es probable que desee llamar a C<srand>.  Uno de ellos es para la generación de resultados predecibles, generalmente para pruebas o depuraciones.  Allí, puede utilizar C<srand($semilla)>, con la misma C<$semilla> cada vez.  Otro caso es donde necesitará un punto de partida criptográficamente más fuerte que el valor predeterminado generalmente aceptado, que se basa en la hora del día, ID del proceso, y la asignación de memoria, o el dispositivo F</dev/urandom>, si está disponible.  Y otro caso es que puede llamar a C<srand()> después de un C<fork()> para evitar que los procesos hijos compartan el mismo valor de inicialización como el padre (y por lo tanto entre sí).
...
Un uso típico de la semilla devuelta es para un programa de prueba que tiene demasiadas combinaciones que probar, exhaustivamente, en el tiempo disponible para cada ejecución.  Se puede probar un subconjunto aleatorio cada vez, y si ocurre un fracaso, registre las semillas utilizadas para la que se ha ejecutado a fin de que más adelante se puedan utilizar para reproducir los resultados exactos.
...
La posición en el archivo de GESTOR_ARCHIVO no cambia.  Puede llamar a L<seek|/"seek GESTOR_ARCHIVO,POSICIÓN,DESDE"> antes de escribir al archivo.
...
A partir de Perl 5.14, C<unshift> puede tomar una EXPR escalar, que debe contener la referencia a un hash o un array no bendecido.  El argumento es desreferenciado automáticamente.  Este aspecto de C<unshift> es considerado altamente experimental.  El comportamiento exacto puede cambiar en una futura versión de Perl.
