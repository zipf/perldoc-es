...
=head2 General
    ...
    perl		Información general (esta sección)
    perlintro		Introducción para novatos
    perltoc		Tabla de contenidos de la documentación

=head2 Seminarios

    perlreftut		Introducción breve a las referencias
...
Perl, oficialmente, significa Lenguaje práctico para la extracción e informe (I<Practical Extraction and Report Language>),
excepto cuando no es así.

Perl es un lenguaje optimizado para el escaneo de ficheros de texto, extraer información de estos ficheros de texto, e impresión de informes basados en esa información.  It's also a good language for many
system management tasks.  El lenguaje está pensado para ser práctico (fácil de usar, eficiente, completo) en lugar de hermoso (pequeño, elegante, mínimo).
...
=head1 FICHEROS
 ...
 "@INC"			posiciones de las librerías perl
...
El lema de Perl es "Hay más que una manera de hacerlo". Adivinar cuántos más se deja como un ejercicio para el lector.

Las tres principales virtudes de un programador son pereza, impaciencia e Hibris (desmesura u orgullo exagerado en uno mismo).  Ver el libro del dromedario para ver por qué.
