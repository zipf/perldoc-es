...
=head2 La hoja
  ...
  CONTEXTO  SIGILS              ARRAYS        HASHES
  void      $scalar   completo: @array        %hash
  escalar   @array    parte:    @array[0, 2]  @hash{'a', 'b'}
  ...
  []    anón. arrayref  ${$$foo[1]}[2] igual que $foo->[1]->[2]
  {}    anón. hashref   ${$$foo[1]}[2] igual que $foo->[1][2]
  ...
  * / % x                 SINTAXIS
  + - .                   for    (LISTA) { }, for (a;b;c) { }
  << >>                   while  ( ) { }, until ( ) { }
  op. unarios             if     ( ) { } elsif ( ) { } else { }
  < > <= >= lt gt le ge   unless ( ) { } elsif ( ) { } else { }
  == != <=> eq ne cmp ~~  for es igual a foreach (SIEMPRE)
  &
  | ^              REGEX METACARATERES        REGEX MODIFICADORES
  &&               ^     comienzo cadena      /i no caja de letras
  || //            $     fin cad. (antes \n)  /m líneas ^$
  .. ...           +     uno o más            /s . incluye \n
  ?:               *     cero o más           /x ign. espacio
  ...
  open() or die $!;  `$datos`         use.perl.org
  use Modules;       /$datos/         perl.apache.org

  LISTA de RETORNO de FUNCIONES
  stat      localtime    caller         VARIABLES ESPECIALES
   0 dev    0 segundo    0 paquete      $_    variable por def.
   1 ino    1 minuto     1 nom. fichero $0    nombre programa
   ...
   8 atime  8 es_dst     8 hints        $.    número línea
...
=head1 VEA TAMBIÉN
 ...
 http://perlmonks.org/?node_id=216602      el original en PM
 http://perlmonks.org/?node_id=238031      Versión de Damian Conway para Perl 6
