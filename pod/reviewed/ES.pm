...
Por desgracia, los útiles modificadores C<-f> y C<-q> de C<perldoc> no funcionan con la documentación traducida.

Por esta razón, hemos creado una revisión de F<Pod/Perldoc.pm> 3.14 que permite utilizar la siguiente sintaxis: 

  %> perldoc -L ES <nombre_pod>
  %> perldoc -L ES -f <función>
  %> perldoc -L ES -q <expresión regular P+F>

La revisión agrega el modificador C<-L>, que permite definir el código de idioma para la traducción deseada. Si el paquete C<POD2::E<lt>idiomaE<gt>> no existe, no se aplicará el modificador.
...
La revisión se incluye con esta distribución y se encuentra en F<./patches/Perldoc.pm-3.14-patch>.
...
I<Nota: Perl 5.10 ya contiene esta funcionalidad, por lo que no es necesario aplicar la revisión.>
