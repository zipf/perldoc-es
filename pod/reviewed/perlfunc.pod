...
Para funciones que pueden ser usadas tanto en contexto escalar como en lista, un fallo no abortivo es indicado, generalmente, devolviendo el valor indefinido, y en contexto lista devolviendo la lista vacía.
...
=item Funciones para ESCALARES o cadenas de caracteres
...
=item Funciones para @ARRAY de verdad
X<array>
...
=item Funciones para %HASH reales
X<hash>
...
=item Funciones para gestores de archivos, ficheros o directorios
X<fichero> X<gestor> X<directorio> X<pipe> X<link> X<symlink>
...
(Están disponibles solo si activa la característica C<"switch">.
Ver L<feature> y L<perlsyn/"Sentencias Switch">.)

=item Palabras clave relacionadas con el ámbito

C<caller>, C<import>, C<local>, C<my>, C<our>, C<package>, C<use>

(C<state> solo está disponible si activa la característica C<"state">. Ver L<feature>.)
...
=item Palabras clave relativas a módulos Perl
X<module>

C<do>, C<import>, C<no>, C<package>, C<require>, C<use>

=item Palabras clave relativas a clases y orientados a objetos
X<object> X<class> X<package>

C<bless>, C<dbmclose>, C<dbmopen>, C<package>, C<ref>, C<tie>, C<tied>,
C<untie>, C<use>

=item Funciones de socket a bajo nivel
X<socket> X<sock>
...
=item Obteniendo información del usuario y grupo de usuarios
...
* C<sub> era una palabra clave en Perl 4, pero en Perl 5 es un operador, por lo cual puede ser usado con paréntesis.
...
=item -X GESTOR_FICHERO
X<-r>X<-w>X<-x>X<-o>X<-R>X<-W>X<-X>X<-O>X<-e>X<-z>X<-s>X<-f>X<-d>X<-l>X<-p>
X<-S>X<-b>X<-c>X<-t>X<-u>X<-g>X<-k>X<-T>X<-B>X<-M>X<-A>X<-C>

=item -X EXPR

=item -X GESTOR_DIRECTORIO

=item -X

Un test de fichero, donde X es una de las letras listadas abajo.  Este operador unario toma un argumento, bien sea un nombre de fichero o un gestor de fichero o gestor de directorio, y comprueba el fichero asociado para ver si algo sobre él es cierto.  Si el argumento se omite, se prueba C<$_>, excepto para C<-t>, que prueba STDIN.
Mientras no se diga lo contrario, se devuelve C<1> para verdadero y C<''> para falso, o el valor indefinido si el fichero no existe.  A pesar de los nombres graciosos, la precedencia es la misma que cualquier otro operador unario con nombre.  El operador puede ser cualquiera de:
    ...
    -p  Fichero es una tubería con nombre (FIFO), o el gestor de fichero es una tubería.
    ...
    -T  Fichero es un fichero de texto ASCII (por una suposición heurística).
    -B  Fichero es un archivo "binario" (opuesto de -T).

    -M  Tiempo de arranque del script menos el tiempo de modificación del fichero, en días.
...
Cuando esté en C<use filetest 'access'> las comprobaciones de fichero mencionadas arriba comprobarán cuándo los permisos pueden (o no) ser concedidos usando la familia de llamadas del sistema access(2).  Note también que C<-x> y C<-X> pueden, bajo este pragma, devolver valores verdaderos incluso si los bits de permiso de ejecución no están puestos (ni cualquier permiso ACL extra de ejecución).  Este comportamiento tan extraño es debido a las definiciones subyacentes de las llamadas del sistema. Note también que, debido a la implementación de C<use filetest 'access'>, el gestor de fichero especial C<_> no almacenará los resultados de los test de fichero mientras este pragma esté activo.  Para más información, lea la documentación del pragma C<filetest>.

Note que C<-s/a/b/> no realiza una substitución negada.  Decir C<-exp($foo)> sigue funcionando como se espera que lo haga: sólo letras sueltas, por detrás de un menos, son interpretadas como test de ficheros.

Las opciones C<-T> y C<-B> funcionan como sigue.  El primer bloque (aprox.) del fichero, es examinado para buscar caracteres extraños como códigos de control o caracteres con el bit alto puesto.  Si se encuentran demasiados caracteres extraños (>30%), es un fichero C<-B>; de lo contrario es un fichero C<-T>.  También, cualquier fichero conteniendo bytes cero en el primer bloque es considerado como un fichero binario.  Si C<-T> o C<-B> se usan con un gestor de fichero, la memoria intermedia actual de IO es examinada en vez del primer bloque.  Ambos, C<-T> y C<-B>, devuelven verdadero en un fichero vacío o en un fichero en el EOF (marca de fin de fichero) cuando se analiza un gestor de fichero.  Debido a que tiene que leer el fichero para hacer el test C<-T>, en la mayoría de las ocasiones querrá usar primero C<-f>, como en C<next unless -f $fichero && -T $fichero>.

Si a cualquiera de los test de fichero (o los operadores C<stat> o C<lstat>) se les da el gestor de fichero especial consistente en un solitario subrayado (guión bajo), entonces se usa la estructura stat del fichero del test anterior (u operador stat), ahorrando una llamada del sistema.  (Esto no funciona con  C<-t>, y necesita recordar que lstat() y C<-l> dejarán valores en la estructura stat para el enlace simbólico, no el fichero real).  (También, si la memoria intermedia de stat se rellenó con una llamada C<lstat>, C<-T> y C<-B> serán reinicializados con los resultados de C<stat _>).
Ejemplo:
...
Como en Perl 5.9.1, una dulce forma, puramente sintáctica, puede apilar operadores de test de fichero, de forma que C<-f -w -x $fichero> es equivalente a C<-x $fichero && -w _ && -f _>. (Esto solo es fascinantemente elegante: si usa el valor devuelto de C<-f $fichero> como un argumento para otro operador de test de fichero, no ocurrirá ninguna magia especial)
...
=item binmode GESTOR, CAPA
X<binmode> X<binario> X<texto> X<DOS> X<Windows>

=item binmode GESTOR

Ordena que el modo de lectura o escritura para el GESTOR sea en "binario" o "texto" en sistemas donde las bibliotecas en tiempo de ejecución distingan entre ficheros binarios y de texto.  Si GESTOR es una expresión, el valor es tomado como el nombre del gestor.  Devuelve verdadero en éxito; de lo contrario devuelve C<undef> y pone C<$!> (errno).
...
Si CAPA está presente es un único string, pero puede contener múltiples directivas. Las directivas alteran el comportamiento del gestor del fichero.
Tiene sentido que CAPA esté presente usando binmode sobre un fichero de texto.

Si se omite CAPA o se especifica como C<:raw> el gestor se prepara para pasar datos binarios. Esto incluye desactivar posibles traducciones CRLF y marcarlas como bytes (como opuesto a los caracteres Unicode).
Note que, a pesar de estar implícito en I<"Programming Perl"> (el dromedario, 3ª edición) o en otro lugar, C<:raw> I<no> es el simplemente el inverso de C<:crlf>
...
C<:bytes>, C<:crlf>, C<:utf8>, y cualquier otra directiva de la forma C<:...>, se llaman I<capas> I/O.  El pragma C<open> puede ser usado para establecer las capas I/O por defecto.  Ver L<open>.
...
Para indicar GESTOR como UTF-8, use C<:utf8> o C<:encoding(utf8)>.
C<:utf8> solo marca los datos como UTF-8 sin más comprobaciones ulteriores, mientras que C<:encoding(utf8)> comprueba que los datos sean UTF-8 válidos. Más detalles se pueden encontrar en L<PerlIO::encoding>.

En general, binmode() debe ser llamada después de open() pero antes de que cualquier I/O se realice sobre el gestor.  Llamando a binmode(), normalmente vaciará cualquier dato pendiente que esté almacenado en la salida (y quizás datos pendientes a la entrada) del gestor de fichero.  Una excepción a esto es la capa C<:encoding> que cambia la codificación por defecto del gestor; ver L<open>.
...
Devuelve el contexto de la llamada a la subrutina actual.  En contexto escalar, devuelve el nombre del paquete del llamante si aquel I<es> un llamante, es decir, si estamos en una subrutina, C<eval> o C<require>, y el valor indefinido en otro caso.  En contexto lista, devuelve

    # 0         1         2
    ($paquete, $fichero, $linea) = caller;
...
Con EXPR, devuelve alguna información extra que el depurador usa para imprimir un rastreo de la pila.  El valor de EXPR indica cuántos marcos de llamada se ha de retroceder antes del actual.
    ...
    # 0         1         2       3           4
    ($paquete, $fichero, $linea, $subrutina, $tieneargs,

    
    # 5          6          7              8       9         10
    $wantarray, $evaltext, $is_requerido, $hints, $bitmask, $hinthash)
     = caller($i);

Aquí, $subrutina puede ser C<(eval)> si el marco de llamada no es una llamada a una subrutina, sino un C<eval>.  En tal caso, se añaden los nuevos elementos $evaltext y C<$is_require>: C<$is_require> es verdadero si el marco es creado por una sentencia C<require> o C<use>, $evaltext contiene el texto de la sentencia C<eval EXPR>.  En particular, para una sentencia C<eval BLOQUE>, $subrutina es C<(eval)>, pero $evaltext es indefinido.  (Note también que cada sentencia C<use> crea un marco C<require> dentro de un marco C<eval EXPR>).  $subrutina puede ser también C<(unknown)> (desconocida) si esta subrutina en particular parece haber sido borrada de la tabla de símbolos.
...
En sistemas que soportan fchdir(2), puede pasarle un gestor de fichero o un gestor de directorio como argumento.  En sistemas que no admiten fchdir(2), pasarle un gestor lanza una excepción.
...
En sistemas que soporten fchmod(2), puede pasar gestores de archivo entre los archivos.  En sistemas que no admiten fchmod(2), pasarle un gestor de archivo lanza una excepción.  Gestores de archivo se deben pasar como comodines o referencias a comodines para que sean reconocidos; las palabras sueltas se consideran como nombres de archivo.
...
En sistemas que soporten fchown(2), puede pasar gestores de archivo entre los archivos.  En sistemas que no admiten fchown(2), pasarle un gestor de archivo lanza una excepción.  Gestores de archivo se deben pasar como comodines o referencias a comodines para que sean reconocidos; las palabras sueltas se consideran como nombres de archivo.
...
Aquí hay un ejemplo que obtiene el uid numérico a partir del fichero de contraseñas:
    ...
    @ary = glob($patron);  # expandir los nombres de los ficheros
    chown $uid, $gid, @ary;
...
Cierra el fichero o tubería asociada con el gestor, limpiando los buffers IO, y cierra el descriptor del fichero en el sistema.  Devuelve verdadero si las operaciones han tenido éxito y si ningún error fue reportado por alguna capa PerlIO.  Cierra el gestor actualmente seleccionado si se omite el argumento.

No tiene porqué cerrar el GESTOR si inmediatamente va a hacer otro C<open> sobre él mismo, porque C<open> lo cierra por usted.  (Ver C<open>). Sin embargo, un C<close> explícito en un fichero de entrada reinicia el contador de líneas (C<$.>), mientras que un C<close> implícito hecho por C<open>, no.

Si el gestor proviene de una tubería abierta, C<close> adicionalmente devolverá falso si alguna de las otras llamadas del sistema implicadas falla, o si el programa termina con un valor de estado distinto de cero.  Si el único problema fue que el programa salió distinto de cero, C<$!> se establecerá a C<0>.  El proceso de cierre de una tubería también espera por el proceso de ejecución en la tubería antes de salir -en caso de que desee ver después la salida de la tubería- e, implícitamente, pone el valor de estado de salida de ese comando en C<$?> y C<${^CHILD_ERROR_NATIVE}>.
...
Ejemplo:
    ...
    open(OUTPUT, '|sort >foo')  # entubado a sort()
        or die "No puedo iniciar sort: $!";
    #...                        # imprimir a la salida
    close OUTPUT                # esperar que sort termine
        or warn $! ? "Error cerrando la tubería de sort: $!"
                   : "Estado de salida $? desde sort";
    open(INPUT, 'foo')          # obtener los resultado de sort
        or die "No puedo abrir 'foo' para leerlo: $!";

GESTOR puede ser una expresión cuyo valor puede ser usado como un gestor indirecto, normalmente el nombre real del gestor.
...
C<last>, C<next> o C<redo> puede aparecer dentro de un bucle C<continue>; C<last> y C<redo> se comportarán como si se hubieran ejecutado dentro del bloque principal.  También C<next>, pero como ejecutará un bloque C<continue>, quizás se convierta en algo divertido.
    ...
    while (EXPR) {
        ...
        hacer_algo_sino;
        # entonces, regresa al principio para comprobar EXPR
    }
...
Si la característica <C"switch"> está habilitada, C<continue> es también una función que termina el bloque C<when> (o C<default>) y cae al siguiente caso.  Ver L<feature> y L<perlsyn/"Sentencias switch"> para más información.
...
Esto permite a su código trabajar con el estándar L<crypt|/crypt> y con implementaciones más exóticas.  En otras palabras, no asuma nada sobre la cadena devuelta, o que cuántos bytes ocupa el I<digest>.
...
Si usa crypt() en una cadena Unicode (que I<potencialmente> tiene caracteres con codificación superior al 255), Perl intenta hacer sentido a la situación intentando hacer una degradación (una copia de la cadena) a una cadena de bytes antes de llamar a crypt() (sobre esa copia).  Si esto funciona, bien.  Si no, crypt() muere con C<Wide character in crypt>.
...
Liga un fichero dbm(3), ndbm(3), sdbm(3), gdbm(3), o Berkeley DB a un hash.  HASH es el nombre del hash.  (A diferencia de un C<open> normal, el primer argumento I<no> es un gestor, aunque lo parezca).  NOMBREDB es el nombre de la base de datos (sin la extensión F<.dir> o F<.pag> si las tuviera).  Si la base de datos no existe, se crea con la protección especificada por MÁSCARA (y modificada por C<umask>).  Si su sistema sólo soporta las viejas funciones DBM, sólo puede realizar una llamada C<dbmopen> en su programa.  En las viejas versiones de Perl, si el sistema no tenía ni DBM ni ndbm, llamando a C<dbmopen> producía un error fatal; ahora lo intentaría con sdbm(3).

Si no tiene permiso de escritura al fichero DBM, sólo podrá leer las variables hash, pero no asignarlas.  Si quiere comprobar si puede escribir o no, use test de fichero o intente asignar una entrada hash ficticia dentro de un C<eval>, para atrapar el error.
...
Note que las funciones como C<keys> y C<values> pueden devolver largas listas cuando se usan con ficheros DBM grandes.  Puede preferir usar la función C<each> para iterar sobre ficheros DBM grandes.  Ejemplo:
    ...
    # imprime los desplazamientos históricos del fichero de news
...
Puede controlar qué biblioteca DBM usará cargando la biblioteca antes de llamar a dbmopen():
    ...
    dbmopen(%NS_Hist, "$ENV{HOME}/.netscape/history.db")
        or die "No puedo abrir el fichero histórico del netscape: $!";
...
Utilize en su lugar una comprobación de tamaño:
    ...
    if (@un_array) { print "el array tiene elementos\n" }
...
Nota: mucha gente tiende a sobrevalorar C<defined> y, para su sorpresa, descubren que el número C<0> y C<""> (la cadena de longitud cero) son, de hecho, valores definidos.  Por ejemplo, si dice
...
Dada una expresión que especifica un elemento o una porción de un hash, C<delete> elimina los elementos especificados de este hash de tal forma que exists() en ese elemento ya no devuelve verdadero.  Poner un elemento hash al valor indefinido no elimina su clave, pero eliminándolo sí que lo hace; véase L</exists> .

Devuelve el valor o valores borrados en contexto lista, o el último elemento en contexto escalar.  La longitud de la lista devuelta siempre coincide con el de la lista de argumentos: eliminar elementos no existentes devuelve el valor indefinido en sus posiciones correspondientes.

delete() puede usarse también en lo arrays y porciones de array, pero su comportamiento es menos claro.  Aunque exists() devolverá falso para entradas eliminadas, eliminar elementos del array nunca cambia los índices de los valores existentes; use en su lugar shift() o splice().  Sin embargo, si todos los elementos eliminados están al final del array, el tamaño del array se reduce al de la posición del elemento más alto que aún devuelve verdadero para exists(), o 0 si no hay ninguno.

B<Queda avisa> que borrar elementos de un array mediante delete() es una funcionalidad obsoleta y es probable que sea eliminada en una futura versión de Perl.

Borrando de C<%ENV> modifica el entorno.  Borrando de un hash enlazado a un fichero DBM, borra esa entrada del fichero DBM.  Borrando de un hash o array C<enlazado> no necesariamente devolverá algo; depende de la implementación del método DELETE en el paquete C<enlazado>, que puede hacer lo que quiera.
...
También puede llamar a C<die> con una referencia como argumento, y si éste queda atrapado en un C<eval>, C<$@> contiene esa referencia.  Esto permite un manejo de las excepciones más elaborado que usando objetos que mantengan un estado arbitrario sobre la naturaleza de la excepción.  Tal esquema es algunas veces preferible a emparejar ciertas cadenas de valores de C<$@> usando expresiones regulares.  Dado que C<$@> es una variable global y C<eval> puede ser usado dentro de las implementaciones de objetos, tenga el cuidado que al analizar el objeto error no sustituya la referencia en la variable global.  Es más fácil hacer una copia local de la referencia antes de cualquier manipulación.  Aquí hay un ejemplo:
    ...
    if (my $ev_err = $@) {
        if (blessed($ev_err) && $ev_err->isa("Some::Module::Exception")) {
            ...
            # manejar todas las otras posibles excepciones
        }
    }
...
excepto que es más eficiente y conciso, mantiene el nombre del fichero en los mensajes de error, busca en los directorios @INC y actualiza C<%INC> si el fichero es encontrado.  Ver L<perlvar/Nombres predefinidos> para estas variables.  También difiere en que el código evaluado con C<do NOMBREFICHERO> no puede ver a las variables léxicas en el ámbito en que se ejecuta; C<eval STRING> sí.  Es lo mismo, sin embargo, en que vuelve a interpretar el fichero cada vez que lo llama, así que, probablemente, no querrá hacer esto dentro de un bucle.
...
Puede usar C<do> para leer en un programa un fichero de configuración.  Comprobación manual de errores se puede hacer así:
    ...
    # leer ficheros de config.: primero sistema, luego usuario
    for $file ("/share/prog/defaults.rc",
               "$ENV{HOME}/.someprogrc")
    {
        unless ($return = do $file) {
            warn "no puedo interpretar $file: $@" if $@;
...
B<ATENCIÓN>: Cualquier fichero abierto en el momento del volcado I<no> será abierto mas que cuando el programa sea reencarnado, con una muy posible confusión por parte de Perl.
...
Cuando se le llama en contexto lista, devuelve una lista de dos elementos consistentes en la clave y el valor del siguiente elementos del hash, o el índice y valor del siguiente elemento del array, así que de esta manera puede iterar por el.  Cuando se le llama en contexto escalar, sólo devuelve la clave (no el valor) del hash, o el índice del array. 
...
Después de C<each> haya devuelto todas las entradas del hash o del array, la siguiente llamada a C<each> devuelve la lista vacía en contexto lista y C<undef> en contexto escalar.  La siguiente llamada reinicia la iteración.  Cada hash o array tiene su iterador interno, accedido por C<each> , C<keys>, y C<values>.  El iterador es implícitamente restablecido cuando C<each> ha llegado al final como se acaba de describir; puede ser explícitamente restablecido llamando a C<keys> o C<values> sobre el hash o array.  Si añade o borra un elemento de un hash mientras está iterando sobre él, puede encontrarse con entradas desaparecidas o duplicadas, así que no lo haga.  Excepción: siempre es seguro borrar el último ítem devuelto por C<each()>, por lo que entonces el siguiente código funcionará correctamente:
...
Devuelve 1 si la siguiente lectura sobre GESTOR devolverá un fin de fichero, o si GESTOR no está abierto.  GESTOR puede ser una expresión cuyo valor da el gestor real.  (Note que esta función realmente lee un carácter y luego hace un C<ungetc>, así que no es útil en un contexto interactivo). No lea de un fichero del que se ha llegado al final (o vuelva a llamar a C<eof(GESTOR)> sobre él) después de haber alcanzado un fin-de-fichero.  Tipos de ficheros como por ejemplo terminales pueden perder la condición de fin-de-fichero si lo hace.

Un C<eof> sin argumento usa el último fichero leído.  Usar C<eof()> con paréntesis vacíos es diferente.  Se refiere a un pseudo fichero formado por los ficheros listados en la línea de comandos y accedidos vía el operador C<< <> >>.  Como C<< <> >> no es explícitamente abierto, como lo sería un gestor de fichero, un C<eof()> antes de que un C<< <> >> sea usado, causará que C<@ARGV> sea examinado para determinar si la entrada está disponible.   De forma semejante, un C<eof()> después de que un C<< <> >> haya devuelto un fin-de-fichero asumirá que está procesando otra lista C<@ARGV>, y si no ha puesto un nuevo C<@ARGV>, leerá la entrada desde C<STDIN>; ver L<perlop/"Operadores E/S">.
...
En un bucle C<< while (<>) >>, C<eof> o C<eof(ARGV)> pueden usarse para detectar el fin de un fichero; C<eof()> sólo detectará el final del último fichero.  Ejemplos:
    ...
    # reinicia el numerado de línea en cada fichero de entrada
    while (<>) {
    ...
    # inserta guiones delante de la última línea del ultimo fichero
    while (<>) {
        if (eof()) {  # chequea por el final del último fichero
            print "--------------\n";
        }
        print;
        last if eof();          # necesario si estamos leyendo desde la terminal
    }
...
En la primera forma, el valor devuelto por EXPR es interpretado y ejecutado como si fuera un pequeño programa Perl.  El valor de la expresión (que es en sí mismo determinada en contexto escalar) es primero interpretada, y si no hay ningún error, ejecutada en el contexto léxico del actual programa Perl, por lo que no se mantienen después ningún valor de variable o definiciones de subrutinas o formatos.  Note que el valor es interpretado cada vez que se ejecuta C<eval>.
...
En la segunda forma, el código dentro del BLOQUE es interpretado sólo una vez -al mismo tiempo que el código que rodea al mismo C<eval> es interpretado- y ejecutado dentro del contexto del actual programa Perl.  Esta forma es típicamente usada para atrapar excepciones más eficientemente que la primera forma (ver arriba), mientras que también provee del beneficio de comprobar el código dentro del BLOQUE en tiempo de compilación.
...
Si hay algún error de sintaxis o en tiempo de ejecución, o se ejecuta una sentencia C<die>, C<eval> devuelve un valor indefinido en contexto escalar o una lista vacía en contexto lista, y C<$@> es inicializado al mensaje de error.  Si no hay errores, se garantiza que C<$@> será una cadena vacía.  Tenga cuidado porque usar C<eval> ni silencia a Perl a la hora de imprimir avisos al STDERR, ni rellena el texto de los mensajes de aviso dentro de C<$@>.
...
Si desea capturar los errores cuando se carga un módulo XS, algunos problemas con la interfaz binaria (como el sesgo de versión de Perl) puede ser fatal, incluso con C<eval> a menos que C<$ENV{PERL_DL_NONLAZY}> esté puesto. Ver L<perlrun>.
...
Ejemplos:
    ...
    # haz no fatal la división-por-cero
...
Esto es especialmente significativo, dado que las llamadas C<__DIE__> pueden llamar de nuevo a C<die>, que tiene el efecto de cambiar los mensajes de error:
    ...
    # __DIE__ gancho que puede modificar los mensajes de error
    {
       local $SIG{'__DIE__'} =
              sub { (my $x = $_[0]) =~ s/foo/bar/g; die $x };
       eval { die "foo vive aquí" };
       print $@ if $@;                # prints "bar vive aquí"
    }
...
La asignación a C<$@> se produce antes de la restauración de las variables localizadas, lo que significa que se requiere un espacio temporal si desea enmascarar algunos pero no todos los errores:
...
Un C<eval ''> ejecutado dentro del paquete C<DB> no ve el usual ámbito léxico circundante, sino más bien el ámbito de la primera parte del código no-DB que le llamó. No necesita preocuparse de esto a menos que esté escribiendo un depurador Perl.
...
Como es un error normal usar C<exec> en lugar de C<system>, Perl le avisará si hay alguna sentencia a continuación que no sea C<die>, C<warn> o C<exit> (si C<-w> está puesto -pero usted, siempre lo tiene así, ¿verdad?-).   Si I<verdaderamente> quiere continuar un C<exec> con otra sentencia, puede usar algunas de estas formas para evitar el aviso:
    ...
    exec ('foo')   or print STDERR "no puedo lanzar foo: $!";
    { exec ('foo') }; print STDERR "no puedo lanzar foo: $!";
...
Si sólo hay un argumento escalar o un array con un elemento dentro de él, se comprueba si el argumento tiene metacaracteres, y si hay alguno, el argumento entero es pasado al shell de comandos para que sea interpretado (suele ser C</bin/sh -c> en plataformas Unix, pero varía en otras plataformas).
...
Ejemplos:
    ...
    exec '/bin/echo', 'Sus argumentos son: ', @ARGV;
...
Cuando los argumentos son ejecutados vía el shell del sistema, los resultados están sujetos a sus peculiaridades y capacidades.  Ver L<perlop/"`STRING`"> para los detalles.
...
Usar un objeto indirecto con C<exec> o C<system> es también más seguro.  Este uso (que también funciona bien con system()) fuerza la interpretación de los argumentos como una lista multievaluada, incluso si la lista sólo tiene un argumento.  De esta forma estará protegido de la expansión de los comodines del shell o de la división en palabras con un espacio en blanco dentro de ellas.
    ...
    exec @args;               # sujeto a los escapes del shell
                                # if @args == 1
    exec { $args[0] } @args;  # seguro incluso con un solo argumento

La primera versión, sin el objeto indirecto, ejecuta el programa I<echo>, pasándole C<"sorpresa"> como argumento.  La segunda versión no; intentó ejecutar un programa llamado literalmente I<"echo sorpresa">, no lo encontró, y puso C<$?> a un valor distinto de cero para indicar un fallo.

A partir de la versión v5.6.0, Perl intenta vaciar todos los ficheros abiertos para salida antes de ejecutar exec, pero esto puede no estar soportado en algunas plataformas (ver L<perlport>).  Para estar seguros, puede necesitar poner C<$|> ($AUTOFLUSH en English) o llamar el método C<autoflush()> de C<IO::Handle> en cualquiera de los gestores de archivo abiertos para evitar la pérdida de datos en la salida.
...
exists() también pueden ser llamado sobre elementos de array, pero su comportamiento es mucho menos evidente, y está fuertemente ligada a la utilización de L</delete> en los arreglos.  B<Queda avisado> que borrar elementos de un array mediante delete() es una funcionalidad obsoleta y es probable que sea eliminada en una futura versión de Perl.
...
[parcial] Aunque el array o hash más profundamente anidado no se autovivifique sólo por que su existencia sea comprobada, cualquiera de los niveles intermedios sí lo hará.
...
=item fcntl GESTOR,FUNCIÓN,ESCALAR
X<fcntl>
...
Aquí hay un ejemplo de poner un gestor de fichero llamado C<REMOTE> a no bloqueante a nivel de sistema.  Aunque, tendrá que negociar también con C<$|>.

    use Fcntl qw(F_GETFL F_SETFL O_NONBLOCK);

    $flags = fcntl(REMOTE, F_GETFL, 0)
                or die "No puedo obtener las banderas para este socket: $!\n";

    $flags = fcntl(REMOTE, F_SETFL, $flags | O_NONBLOCK)
                or die "No puedo poner las banderas para este socket: $!\n";

=item fileno GESTOR
X<fileno>

Devuelve el descriptor de fichero para un gestor, o indefinido si el gestor no está abierto.  Esto es principalmente útil para construir bitmaps para C<select> y operaciones POSIX de bajo nivel sobre tty.
Si el GESTOR es una expresión, el valor es tomado como un gestor indirecto, generalmente su nombre.

Puede usar esto para encontrar cuándo dos gestores se refieren al mismo descriptor subyacente:

    if (fileno(ESTE) == fileno(ESE)) {
	print "ESTE y ESE son duplicados\n";

(Gestores conectados a objetos en memoria vía las nuevas posibilidades de C<open> pueden devolver indefinido incluso si están abiertos).


=item flock GESTOR, OPERACIÓN
X<flock> X<lock> X<bloqueo>

Llama a flock(2), o una emulación de ella, sobre GESTOR.  Devuelve verdadero para éxito, falso en fallo.  Produce un error fatal si se usa en una máquina que no implemente flock(2), bloqueo fcntl(2), o lockf(3).
C<flock> es el interfaz portable de Perl de bloqueo de fichero, aunque sólo bloquea ficheros enteros, no registros.
...
OPERACIÓN es una de LOCK_SH, LOCK_EX, o LOCK_UN, posiblemente combinado con LOCK_NB.  Estas constantes tienen, tradicionalmente, los valores 1, 2, 8 y 4, pero puede usar los nombres simbólicos si los importa con el módulo Fcntl, tanto de forma indivual o como un grupo usando la marca ':flock'.  LOCK_SH solicita un bloqueo compartido, LOCK_EX solicita un bloqueo exclusivo y LOCK_UN devuelve un bloqueo solicitado previamente.  Si LOCK_NB se combina a nivel de bit con 'or' con LOCK_SH o LOCK_EX entonces C<flock> regresa inmediatamente en vez de esperar por el bloqueo; compruebe el valor de estado devuelto para ver si lo ha conseguido.

Para evitar la posibilidad de descoordinación, Perl ahora vacía el GESTOR antes de bloquearlo o desbloquearlo.

Note que la emulación integrada con lockf(3) no provee de bloqueos compartidos y requiere que el GESTOR sea abierto con intención de escribir en el.  Estas son las semánticas que lockf(3) implementa.  La mayor parte si no todos los sistemas implementan lockf(3) en términos de bloqueo con fcntl(2), aunque, las diferencias en la semántica no llame la atención a mucha gente.

Note que la emulación de fcntl(2) de flock(3) requiere que GESTOR sea abierto con intención de leer para usar LOCK_SH y requiere ser abierto con intención de escribir para usar LOCK_EX.

Note también que algunas versiones de C<flock> no pueden bloquear cosas sobre la red; necesitará usar un C<fcntl> más específico del sistema para hacer esto.  Si le gusta la idea, puede forzar a Perl a ignorar la función del sistema flock(2) y proveer de su propia emulación basada en fcntl(2), pasando la opción C<-Ud_flock> al programa F<Configure> cuando configure la compilación de Perl.
...
A partir de la versión v5.6.0, Perl intentará vaciar todos los ficheros abiertos para salida antes de crear un proceso hijo, pero esto puede no estar soportado en algunas plataformas (ver L<perlport>).  Para estar seguros, puede necesitar poner C<$|> ($AUTOFLUSH en English) o llamar el método C<autoflush()> de C<IO::Handle> en cualquiera de los gestores de archivo abiertos para evitar duplicación de salida.
...
=item getc GESTOR
X<getc> X<getchar> X<character> X<carácter> X<file, read> X<fichero, lectura>

=item getc

Devuelve el siguiente carácter desde el fichero de entrada asociado a GESTOR, o el valor indefinido al final del fichero, o si ocurrió un error (en este último caso, se actualiza C<$!>).  Si GESTOR se omite, se lee desde STDIN.  Esto no es particularmente eficiente.  Sin embargo, esto no puede ser usado para extraer caracteres sueltos sin esperar a que el usuario pulse la tecla de I<Enter>.  Para esto, intente algo como esto:
...
Implementa la función de la librería C del mismo nombre, que en la mayoría de los sistemas devuelve el login actual desde F</etc/utmp>, si existe.  Si devuelve la cadena vacía, utilice C<getpwuid>.
...
Nota para usuarios Linux: en Linux, las funciones C C<getpid()> y C<getppid()> devuelven valores diferentes para hilos (threads) diferentes. Para ser portable, este comportamiento no está reflejado en la función  C<getppid()>, que devuelve un valor consistente a través de los hilos. Si quiere llamar a la subyacente C<getppid()>, puede usar el módulo de CPAN C<Linux::Pid>.
...
(Ver L<getpriority(2)>). Lanzará una excepción fatal si se usa en una máquina que no implemente getpriority(2).
...
Cerciorarse de que <gethostbyname()> es llamado en contexto ESCALAR y que se comprueba que su valor de retorno está definido.
...
Si está cansado de recordar qué elemento de la lista devuelta contiene qué valor, existen interfaces por nombre, en módulos estándar: C<File::stat>, C<Net::hostent>, C<Net::netent>, C<Net::protoent>, C<Net::servent>, C<Time::gmtime>, C<Time::localtime> y C<User::grent>.  Estas sobreescriben a las integradas, con versiones que devuelven objetos con los nombres apropiados para cada campo.  Por ejemplo:
   ...
   use File::stat;
   use User::pwent;
   $is_his = (stat($fichero)->uid == pwent($loquesea)->uid);
...
Un ejemplo para comprobar si el algoritmo de Nagle está activado en un socket:
    ...
    print "El algoritmo de Nagle está ", $nodelay ? "off\n" : "on\n";
...
Si los únicos caracteres comodín en C<glob> son llaves, no se coincidirá con ningún fichero, pero, potencialmente, se devuelven muchas cadenas.  Por ejemplo, esto produce nueve cadenas, una por cada par de frutas y colores:

    @muchos =  glob "{manzana,tomate,cereza}={verde,amarilla,roja}";

Comenzando con v5.6.0, este operador está implementado usando la extensión estándar C<File::Glob>.  Ver L<File::Glob> para los detalles, incluyendo C<bsd_glob> que no trata al espacio en blanco como separador de patrones.
...
La forma C<goto-ETIQUETA> busca la sentencia etiquetada con ETIQUETA y continua la ejecución desde allí. No se puede utilizar para salir de un bloque o una subrutina pasada a C<sort>.  Puede ser utilizada para ir a casi cualquier sitio que esté dentro del ámbito dinámico, incluyendo fuera de las subrutinas, pero es usualmente mucho mejor usar otras construcciones como C<last> o C<die>.
...
=item ioctl GESTOR,FUNCIÓN,ESCALAR
X<ioctl>
...
El valor devuelto de C<ioctl> (y C<fcntl>) es como sigue:
    ...
    si OS devuelve:    entonces Perl devuelve:
        -1               el valor indefinido
         0               la cadena "0 but true"
...
Devuelve una lista consistente de todas las claves del hash indicado, o los índices de un array. (En contexto escalar, devuelve el número de claves o índices).
...
Como efecto colateral, llamando a keys() resetea el iterador interno del HASH o ARRAY (ver L</each>).  De forma particular, llamando a keys() en contexto nulo (void) resetea el iterador sin otro efecto.
...
Aquí hay otra forma de imprimir su entorno:
    ...
    @valores = values %ENV;
    while (@claves) {
        print pop(@claves), '=', pop(@valores), "\n";
    }

o de forma ordenada por la clave:

    foreach $clave (sort(keys %ENV)) {
...
Esto es una ordenación numérica descendente de un hash por sus valores:
    ...
    foreach $clave (sort { $hash{$b} <=> $hash{$a} } keys %hash) {
        printf "%4d %s\n", $hash{$clave}, $clave;
    }

Usado como valor a la izquierda (lvalue), C<keys> le permite incrementar el número de alojamientos de memoria hash para el hash indicado.  Puede hacerle ganar más eficiencia si sabe que el hash va a tener un gran tamaño.  (Esto es similar a pre-extender un array asignándole un gran número a $#array). Si dice

    keys %hash = 200;

entonces C<%hash> tendrá al menos 200 alojamientos de memoria reservados para él -- 256 de ellos, de hecho, ya que se redondea a la siguiente potencia de dos.  Estos alojamientos serán mantenidos incluso si hace C<%hash = ()>. Use C<undef %hash> si quiere liberar el almacenamiento mientras mantiene a C<%hash> en el ámbito.
No puede reducir el número de alojamientos reservados para el hash usando C<keys> de esta manera (pero no necesita preocuparse si hace esto por accidente; intentarlo no tiene ningún efecto). C<keys @array> en un contexto a la izquierda es un error de sintaxis.
...
Si la SEÑAL es cero, no se envía señal al proceso, pero C<kill> comprueba si I<posible> enviar una señal a la misma (lo que significa, para ser breve, que el proceso es propiedad del mismo usuario, o somos el super-usuario).  Esto es útil para comprobar que un proceso hijo aún está vivo (aunque sólo sea como un zombie) y no ha cambiado su UID.  Ver L<perlport> para notas sobre la portabilidad de esta construcción.

A diferencia del shell, si SEÑAL es negativo, mata el grupo de procesos en lugar de a los procesos. Eso significa que desea utilizar, generalmente, señales positivas, y no negativas.
Puede usar también nombres de señales entrecomilladas.

El comportamiento de kill cuando el I<PROCESO> es el número cero o negativo depende del sistema operativo.  Por ejemplo, en los sistemas conformes a POSIX, un cero señalará al grupo del proceso actual y -1 señalará todos los procesos.
...
C<last> no puede usarse para salir de un bloque que devuelve un valor, como C<eval {}>, C<sub {}> o C<do {}>, y no debe ser usado para salir de una operación grep() o map().
...
Devuelve una versión de EXPR, en minúsculas.  Esta es la función interna que implementa el escapado C<\L> en las cadenas entre comillas dobles.  Respeta la actual configuración regional LC_CTYPE si C<use locale> está en vigor.  Ver L<perllocale> y L<perlunicode> para más detalles sobre la configuración regional y el soporte de Unicode.
...
Devuelve el valor de EXPR con el primer carácter en minúsculas.  Esta es la función interna que implementa el escapado C<\l> en las cadenas entre comillas dobles.  Respeta la actual configuración regional LC_CTYPE si C <use locale> está en vigor.  Ver L<perllocale> y L<perlunicode> para más detalles sobre la configuración regional y el soporte de Unicode.
...
=item link FICHEROVIEJO,FICHERONUEVO
X<link>

Crea un nuevo nombre de fichero enlazado con el nombre del fichero antiguo.  Devuelve verdadero en caso de éxito, o falso en caso contrario.

=item listen SOCKET,TAMAÑOCOLA
X<listen>
...
Un local() modifica las variables listadas para que sean locales en el bloque que les rodea, un archivo o un eval().  Si se lista más de un valor, esa lista debe ser puesta entre paréntesis.  Ver L<perlsub/"Valores temporales con local()"> para más detalles, incluidas las cuestiones relativas a arrays y hash enlazadas.
...
Todos los elementos de la lista son numéricos y derivan de la 'struct tm' del C.  $seg, $min y $hora son los segundos, minutos y horas del tiempo especificado.

C<$mdia> es el día del mes, y C<$mes> es el propio mes, en el rango C<0..11> con 0 indicando enero y 11, diciembre.
...
C<$anno> es el número de años desde 1900, no sólo los dos últimos dígitos del año.  Es decir, $anno es C<123> para el año 2023.  La forma apropiada para obtener un año con los 4 dígitos es simplemente:

    $anno += 1900;

De lo contrario está creando programas Y2K no compatibles -y usted no querría hacer eso, ¿verdad?-
...
Este valor escalar B<no> es dependiente de la región, sino que viene de una función interna de Perl. Para GMT, en lugar de la hora local, use la primitiva L</gmtime>. Véase también el módulo C<Time::Local> (para convertir los segundos, minutos, horas, ... de nuevo al valor entero devuelto por time()), y las funciones strftime(3) y mktime(3) del módulo L<POSIX>.

Para obtener algo similar, pero con cadenas de fecha dependiente de la configuración región, ajuste apropiadamente sus variables de entorno a la región (véase L<perllocale>) e intente, por ejemplo:
...
Devuelve el logaritmo natural (base I<e>) de EXPR.  Si EXPR se omite, devuelve el logaritmo de C<$_>.  Para obtener el logaritmo de otra base, uso álgebra básica: el logaritmo de base N de un número es igual al logaritmo natural de ese número dividido por el logaritmo natural de N. Por ejemplo:
...
hace lo mismo que la función C<stat> (incluyendo el establecimiento del gestor especial C<_>) pero de un enlace simbólico en lugar del archivo apuntado por el enlace simbólico.  Si los enlaces simbólicos no están implementados en su sistema, se hace un C<stat> normal.  Para más información más detallada, por favor consulte la documentación de C<stat>.
...
Note que C<$_> es un alias al valor de la lista, por lo que puede ser usado para modificar los elementos de la LISTA.  Mientras que esto suele ser útil, puede causar resultados extraños si los elementos de la LISTA no son variables.
El uso de un bucle C<foreach> normal para este propósito sería más claro en la mayoría de los casos.  Véase también L</grep> para un array compuesto por los elementos de la lista original en que el BLOQUE o EXPR se evalúa como verdadero.

Si C<$_> es léxico en el ámbito cuando el C<map> aparece (porque se ha declarado con C<my $_>), entonces, además de ser un alias local a los elementos de la lista, C<$_> sigue siendo léxico dentro del bloque, es decir, no puede ser visto desde el exterior, evitando cualquier potencial efecto secundario.

C<{> comienza tanto una referencia a hash como bloques, por lo que C<map { ...> podría ser tanto el comienzo de map BLOQUE LISTA o map EXPR, LISTA. Debido a que Perl no mira por adelantado por el cierre C<}>, tiene que hacer una conjetura por la que tiene que lidiar en base a lo que encuentra justo después de la <C{>. Por lo general, lo hace bien, pero si no, no se dará cuenta de que algo anda mal hasta que llega a la C<}> y se encuentra con la omisión de (o inesperada) coma. El error de sintaxis será reportado cerca de C<}>, pero tendrá que cambiar algo cerca de C<{>, como en el uso de un unario C<+>, para darle un poco de ayuda a Perl:

    %hash = map {  "\L$_" => 1  } @array  # perl lee mal la EXPR.  
    %hash = map { +"\L$_" => 1  } @array  # perl lee bien el BLOQUE. 
...
Crea el directorio especificado por el nombre de FICHERO, con permisos especificados por la MÁSCARA (y modificada por C<umask>).  Si tiene éxito, devuelve verdadero, en caso contrario devuelve falso y asigna C<$!> (errno).
Si se omite, por defecto MÁSCARA es 0777. Si se omite, por defecto FICHERO se toma desde <$_>.

En general, es mejor crear directorios con una MÁSCARA permisiva y dejar que el usuario lo modifique que con su C<umask>, que suministrar una MÁSCARA restrictiva y no darle al usuario una forma de ser más permisivo.
Las excepciones a esta regla son cuando el fichero o directorio debe mantenerse privados (archivos de correo, por ejemplo).  La entrada perlfunc(1) sobre C<umask> discute la elección de la máscara con más detalles.

Tenga en cuenta que de acuerdo con POSIX 1.003.1-1996, el nombre de fichero puede tener cualquier número de barras inclinadas.  Algunos sistemas operativos y de sistemas de ficheros no interpretan esto de forma correcta, por lo que Perl automáticamente elimina todas las barras inclinadas finales para tener a todo el mundo feliz.

Para, recursivamente, crear una estructura de directorios, mire la función C<mkpath> del módulo L<File::Path>.
...
primero para obtener las definiciones de las constantes.  Si CMD es C<IPC_STAT>, entonces ARG debe ser una variable que contendrá la estructura C<msqid_ds> devuelta.  Devuelve lo mismo que C<ioctl>: el valor indefinido para error, C<"0 but true"> para el cero, o el valor de retorno real, en caso contrario.  Véase también la documentación de L<perlipc/"SysV IPC">, C<IPC::SysV>, y C<IPC::Semaphore>.

=item msgget KEY,FLAGS
X<msgget>

Llama a la función System V IPC msgget(2).  Devuelve el identificador de una cola de mensaje, o el valor indefinido si hay un error.  Véase también la documentación de L<perlipc/"SysV IPC"> y C<IPC::SysV> y C<IPC::Msg>.
...
Contamina la variable.  Devuelve verdadero si tiene éxito, o falso si hay un error.  Véase también la documentación de L<perlipc/"SysV IPC">, C<IPC::SysV>, y C<IPC::SysV::Msg>.

=item msgsnd ID,MSG,FLAGS
X<msgsnd>

Llama a la función msgsnd de System V IPC para enviar el mensaje MSG a la cola de mensajes ID.  MSG debe comenzar con un tipo de mensaje, contenido en un entero largo nativo, seguido por la longitud del mensaje, y finalmente el mensaje mismo.  Este tipo de empaquetado se puede lograr con C<pack("l! a*", $tipo, $mensaje)>.  Devuelve verdadero si tiene éxito, o falso si hay un error.  Véase también la documentación C<IPC::SysV> y C<IPC::SysV::Msg>.
...
Un C<my> declara las variables enumeradas como locales (léxicamente) en el bloque, archivo, o C<eval>.  Si se indica más de un valor, esa lista debe ser puesta entre paréntesis.
...
Note que si hubiera un bloque C<continue> a continuación del anterior, se ejecutaría incluso en las líneas descartadas.  Si la ETIQUETA se omite, el comando se refiere al bucle más interior en que se encuentre.
...
Interpreta EXPR como una cadena en octal y devuelve el valor correspondiente.  (Si EXPR comienza con C<0x>, se interpreta como una cadena en hexadecimal.  Si EXPR empieza con C<0b>, se interpreta como una cadena binaria.  El espacio en blanco que le rodea es ignorado en los tres casos). Lo que le sigue se interpretará como un decimal, binario, octal y hexadecimal en la notación estándar de Perl:

    $val = oct($val) if $val =~ /^0/;

Si se omite EXPR, se usa C<$_>.   Para hacer lo contrario (producir un número en octal), use sprintf() o printf():

    $dec_perms = (stat("fichero"))[2] & 07777;
    $oct_perm_str = sprintf "%o", $permisos;

La función oct() es de uso común cuando una cadena como C<644> tiene que ser convertida en un modo de fichero, por ejemplo.  Aunque Perl convierte automáticamente las cadenas en números según sea necesario, esta conversión automática asume que es en base 10.
...
=item open GESTOR,EXPR
X<open> X<pipe> X<file, open> X<fopen> X<abrir>

=item open GESTOR,MODO,EXPR

=item open GESTOR,MODO,EXPR,LISTA

=item open GESTOR,MODO,REFERENCIA

=item open GESTOR

Abre el archivo cuyo nombre viene dado por EXPR, y lo asocia con GESTOR.

Ejemplos sencillos para abrir un fichero en modo lectura:
...
Si GESTOR es un variable escalar indefinida (o elemento de un array o hash) la variable es asignada a una referencia a un nuevo gestor de fichero anónimo; de lo contrario, si GESTOR es una expresión, su valor se utiliza como el nombre del gestor de archivo real buscado.  (Esto se considera una referencia simbólica, por lo que C<use strict 'refs'> I<no> debe estar activada).

Si EXPR se omite, la variable escalar del mismo nombre que el GESTOR contiene el nombre del fichero.  (Note que las variables léxicas -las declaradas con C<my>- no servirán para este propósito; así que si está usando C<my>, especifique EXPR en su llamada a open).

Si se especifican tres o más argumentos entonces el modo de apertura y el nombre del fichero están separados. Si MODO es C<< '<' >> o nada, el archivo se abre para lectura.  Si MODO es C<< '>' >>, el archivo es truncado y abierto para salida, siendo creado si es necesario.  Si MODO es C<<< '>>' >>>, el archivo se abre en modo añadir, y de nuevo, será creado si es necesario.

Puede poner un C<'+'> en frente de C<< '>' >> o C<< '<' >> para indicar que desea tanto acceder al fichero en lectura como en escritura; C<< '+<' >> casi siempre es lo preferido para la lectura/escritura de actualizaciones -el modo C<< '+>' >> primero borraría el fichero-.  Por lo general, no puede utilizar el modo de lectura y escritura para la actualización de ficheros de texto, ya que tienen registros de longitud variable.  Ver la opción B<-i> en L<perlrun> para una mejor aproximación.  El archivo se crea con permisos de C<0666> modificados por el valor de C<umask> del proceso.

Estos prefijos se corresponden con los modos de la función fopen(3) de C<'r'>, C<'r+'>, C<'w'>, C<'w+'>, C<'a'> y C<'a+'>.

En la forme de dos argumentos (y un argumento) de la llamada, el modo y el nombre del fichero debe ser concatenados (en ese orden), opcionalmente separados por espacios.  Puede omitir el modo en estos formatos cuando el modo sea C<< '<' >>.

Si el nombre del fichero empieza por C<'|'>, el nombre del fichero es interpretado como un comando a quien le entubamos nuestra salida, y si el nombre del fichero termina con un C<'|'>, el nombre del fichero es interpretado como un comando que entuba su salida hacia nosotros.  Ver L<perlipc/"Usando open() por IPC"> para más ejemplos de sobre esto.  (No se le permite hacer un C<open> a un comando que entube tanto en la entrada I<como> en la salida, pero véase L<IPC::Open2>, L<IPC::Open3>, y L<perlipc/"Comunicación bidireccional con otro proceso"> para alternativas sobre esto).

Para tres o más argumentos, cuando MODO es C<'|-'>, el nombre del fichero se interpreta como un comando cuya salida va a ser entubada, y si MODO es <'-|'>, el nombre del fichero es interpretado como un comando que entuba la salida hacia nosotros.  En la forma de dos argumentos (y un argumento), debe sustituir el guión (C<'-'>) con el comando.
Ver L<perlipc/"Usando open() por IPC"> para más ejemplos de sobre esto.
(No se le permite hacer un C<open> a un comando que entube tanto en la entrada I<como> en la salida, pero véase L<IPC::Open2>, L<IPC::Open3>, y L<perlipc/"Comunicación bidireccional"> para alternativas sobre esto).
...
Puede usar la forma de tres argumentos de open() para especificar capas  E/S (algunas veces referidas como "disciplinas") para aplicar al gestor que afecta al modo en que la entrada y salida son procesadas (véase L<open> y L<PerlIO> para más detalles). Por ejemplo:

  open(my $fh, "<:encoding(UTF-8)", "fichero")
    || die "no puedo abrir fichero codificado en UTF-8: $!";

abre el fichero codificado en UTF-8 que contiene caracteres Unicode; ver L<perluniintro>. Tenga en cuenta que si las capas se especifican en la forma de tres argumentos, entonces las capas por defecto almacenadas en ${^OPEN} (ver L<perlvar>; normalmente fijados por el pragma B<open> o la opción B<-CioD>) son ignoradas.
...
Como un caso especial, de la forma de tres argumentos con un modo de lectura/escritura y el tercer argumento siendo C<undef>:

    open(my $tmp, "+>", undef) or die ...

abre un gestor de archivo a un archivo anónimo temporal.  También usando "+<" trabaja de forma simétrica, pero debe considerar, realmente, en primer lugar, escribir algo en el archivo temporal.  Necesitará usar seek() para hacer la lectura.

Desde v5.8.0, Perl ha sido construido para que use PerlIO por defecto.  A menos que haya cambiado esto (es decir, Configure -Uuseperlio), puede abrir gestores de archivo directamente a escalares Perl de esta forma:

    open($fh, '>', \$variable) || ..

Para (re)abrir C<STDOUT> o C<STDERR> como un fichero en memoria, primero, cierrelos:
...
Ejemplos generales:
    ...
    # ficheros en memoria
    open(MEMORIA,'>', \$var)
        or die "No puedo abrir el fichero en memoria: $!";
    print MEMORIA "foo!\n";              # la salida aparecerá en $var

    # procesa la lista de ficheros pasados como argumentos junto con cualquier fichero a incluir

    foreach $fichero (@ARGV) {
        procesa($fichero, 'fh00');
    }

    sub procesa {
        my($fichero, $entrada) = @_;
        $entrada++;    # esto es un incremento de cadena
        unless (open($entrada, $fichero)) {
            print STDERR "No puedo abrir $fichero: $!\n";
            return;
        }
...
Puede también, en la tradición de Bourne shell, especificar una EXPR comenzando con C<< '>&' >>, en cuyo caso el resto de la cadena se interpreta como el nombre de un gestor de archivo (o descriptor de fichero, si es numérico) para que sea duplicado (con C<dup(2)> ) y luego abierto.  Puede usar C<&> después de C<< > >>,
C<<< >> >>>, C<< < >>, C<< +> >>, C<<< +>> >>>, y C<< +< >>.
El modo que especifique debe coincidir con el modo del gestor de archivo original.
(Duplicar un gestor de archivo no tiene en cuenta cualquier contenido existente de los buffers de E/S). Si utiliza la forma de tres arg. entonces puede pasar un número, el nombre de un gestor de archivo o la normal "referencia a un glob".
...
Si especifica C<< '<&=X' >>, donde C<X> es un número de descriptor de archivo o un gestor de archivo, entonces Perl hará un equivalente de C de C<fdopen> de ese descriptor de archivo (y no llamar a C<dup(2)>).  Por ejemplo:
...
Ser parco en gestores de archivo también es útil (además de ser parsimoniosos), por ejemplo cuando algo depende de descriptores de fichero, como por ejemplo, el uso del bloqueo flock().  Si hace sólo C<< open(A, '>>&B') >>, el gestor de archivo A no tendrá el mismo descriptor de archivo que B, y por lo tanto flock(A) no hará un flock(B), y viceversa.  Pero con C<< open(A, '>>&=B') >> los gestores de archivo de compartirán el mismo descriptor de fichero.

Tenga en cuenta que si está usando Perls más viejos que v5.8.0, Perl usará las bibliotecas estándar C de fdopen() para ejecutar la funcionalidad de "=".
En muchos sistemas Unix fdopen() falla cuando los descriptores de archivo exceden de un valor determinado, por lo general 255.  Para Perls v5.8.0 y posteriores, PerlIO suele ser el predeterminado.

Podrá saber si Perl se ha elaborado con PerlIO o no mediante la ejecución de C<perl -V> y buscando por la línea C<useperlio=>.  Si C<useperlio> está C<definido>, tiene PerlIO activado, de lo contrario, no.

Si abre una tubería en el comando C<'-'>, es decir, ya sea C<'|-'> o C<'-|'> con la forma de 2 argumentos (o un argumento) de open(), entonces se realiza un fork() implícito, y el valor de retorno del open() es el PID del hijo dentro del proceso padre, y C<0> en el proceso hijo.  (Use C<defined($pid)> para determinar si open() se ha realizado correctamente). El gestor de archivo se comporta normalmente para el padre, pero la E/S del gestor de archivo se entuba desde/hacia el STDOUT/STDIN del proceso hijo.
En el proceso hijo, el gestor de archivo no se abre: la E/S sucede desde/hasta el nuevo STDOUT/STDIN.  Habitualmente esto se usa como el open entubado cuando quiere ejercer más control sobre cómo el comando entubado es ejecutado, como cuando se ejecuta con setuid y no quiero tener que escanear comandos del shell buscando metacaracteres.
...
Las siguientes tripletas son, más o menos equivalentes:
    ...
    open(FOO, "cat -n '$fichero'|");
    open(FOO, '-|', "cat -n '$fichero'");
    open(FOO, '-|') || exec 'cat', '-n', $fichero;
    open(FOO, '-|', "cat", '-n', $fichero);

El último ejemplo de cada bloque muestra la tubería como "forma de lista", que todavía no está soportado en todas las plataformas.  Una buena regla general es que si la plataforma tiene un verdadero C<fork()> (en otras palabras, si su plataforma es Unix), puede utilizar la forma de lista.

Ver L<perlipc/"Aperturas seguras de tuberías"> para más ejemplos sobre esto.

Empezando con v5.6.0, Perl intentará vaciar todos los archivos abiertos para la salida antes de cualquier operación que pueda hacer un fork, pero esto quizás no esté soportado en algunas plataformas (véase L <perlport>).  Para estar seguro, es posible que necesite el conjunto C<|> ($AUTOFLUSH en English) o llame al método C<autoflush()> de C<IO::Handle> en cualquier identificador abierto.

En sistemas que apoyan la bandera close-on-exec en ficheros, esta bandera se establecerá para el descriptor de fichero abierto recientemente según lo determinado por el valor de $^F.  Ver L<perlvar/$^F>.

Cerrando cualquier gestor de archivo entubado hace que el proceso padre espere que el hijo termine, y devuelva el valor de estado en C<$?> y en C<${^CHILD_ERROR_NATIVE}>.

El nombre del fichero pasado a las formas de dos argumentos (o de un argumento) de open() terminará con los espacios en blanco iniciales y finales eliminados, y la normal redirección de los caracteres activada.  Esta propiedad, conocida como "open() mágico", a menudo se puede utilizar con buenos resultados.  Un usuario podría especificar un nombre de fichero de F<"rsh cat fichero |">, o usted puede cambiar ciertos nombres de fichero, según lo que necesite:

    $fichero =~ s/(.*\.gz)\s*$/gzip -dc < $1|/;
    open(FH, $fichero) or die "No puedo abrir $fichero: $!";

Use la forma de 3 argumentos para abrir un fichero con una cantidad arbitraria de caracteres extraños en él,
...
(esto quizás no funcione en algunos sistemas de archivos extraños).  Uno debe elegir escrupulosamente entre la I<magia> y la forma de 3 argumentos de open():

    open IN, $ARGV[0];

permitirá al usuario especificar un argumento de la forma C<"rsh cat fichero |">, pero no funcionará en un nombre de fichero que tenga un espacio al final, mientras que

    open IN, '<', $ARGV[0];

tendrá exactamente las restricciones contrarias.

Si busca un "auténtico" C<open> como en C (ver C<open(2)> en su sistema), entonces debe usar la función C<sysopen>, que no supone tal magia (pero puede utilizar sutilmente diferentes modos de fichero que el open() de Perl, que es asignado a la función en C fopen()).  Esta es otra manera de proteger los nombres de archivo de la interpretación.  Por ejemplo:
    ...
    print HANDLE "algo $$\n";
    seek(HANDLE, 0, 0);
    print "El fichero contiene: ", <HANDLE>;

Utilizando el constructor del paquete C<IO::Handle> (o una de sus subclases, como C<IO::File> o C<IO::Socket>), puede generar gestores de archivo anónimos que tienen el ámbito de cualquiera de las variables que contengan referencias a ellas, y se cierran automáticamente cuando salgan del ámbito de aplicación:
    ...
    sub leer_mifichero {
        ...
        open($handle, "mifichero") or die "mifichero: $!";
        $primero = <$handle>
            or return ();     # Cerrado automáticamente aquí.
...
GESTOR_DIR puede ser una expresión cuyo valor puede ser usado como un gestor indirecto, normalmente el nombre real del gestor.  Si GESTOR_DIR es una variable escalar indefinida (o array o elemento hash), a la variable se le asigna una referencia a un nuevo gestor de directorio anónimo.
Los GESTOR_DIR tienen su propio espacio de nombres separados de los GESTORes.
...
Devuelve el valor numérico (en la codificación nativa de 8 bits, como ASCII o EBCDIC, o Unicode) del primer carácter de EXPR.  Si se omite EXPR, se usa C<$_>.

Para lo contrario, véase L </chr>.
Ver L<perlunicode> para más información acerca de Unicode.
...
C<our> asocia un nombre sencillo con una variable de paquete en el paquete actual para ser usado en el ámbito actual.  Cuando C<use strict 'vars'> esté activo, C<our> le permite usar variables globales declaradas sin calificarlas con nombres de paquetes, en el ámbito léxico de la declaración C<our>.
De esta manera, C<our> difiere de la C<use vars>, en que su ámbito es el paquete.

A diferencia de C<my>, que tanto asigna almacenamiento para una variable y asocia un nombre sencillo con este almacenamiento para su uso dentro del ámbito actual, C<our> asocia un nombre sencillo con una variable de paquete en el paquete actual, para su uso en el ámbito actual.  En otras palabras, C<our> tiene el alcance de las mismas reglas que C<my>, pero no crea necesariamente una variable.
...
La PLANTILLA es una secuencia de caracteres que dan el orden y el tipo de los valores, de la siguiente manera:
    ...
    w  Un entero comprimido en BER (no en ASN.1 BER, ver perlpacktut para los
        ...
        primero el dígito más significativo, con el menor número posible de dígitos.         El octavo bit (el bit más alto) es puesto en cada byte excepto el último.
...
Uno o más de los modificadores siguientes pueden, opcionalmente, seguir a ciertas letras en la PLANTILLA (la segunda columna enumeran las letras para las cuales el modificador es válido):
    ...
    !   sSlLiI     Fuerza tamaños nativos (corto, largo, entero) en lugar
                      de tamaños (16/32 bit) fijos.

        xX         Hace x y X actuar como comandos de alineamiento.

        nNvV       Trata a enteros como con signo, en lugar de sin signo.

        @.         Especifica una posición como desplazamiento byte en la
...
Cada letra puede ser seguida opcionalmente por un número que indica el número de repeticiones.  Un número de repeticiones podría estar opcionalmente encerrada entre paréntesis, como en C<pack("C[80]", @arr)>.  El número de repeticiones engulle a tantos valores de la LISTA cuando es usada con todos los tipos de formato que no sean C<a>, C<A>, C<Z>, C<b>,
C<B>, C<h>, C<H>, C<@>, C<.>, C<x>, C<X> y C<P>, donde significa otra cosa, descrita abajo.  El suministro de un C<*> para el número de repeticiones en lugar de un número significa usar tantos elementos como queden, a excepción de:
...
Los tipos C<a>, C<A>, y C<Z> absorben un solo valor, pero se empaqueta como una cadena de longitud indicada, rellenada con nulos o espacios según sea necesario.  Al desempaquetar, C<A> elimina el espacio en blanco y nulos sobrantes al final, C<Z> elimina todo después del primer nulo, y C<a> devuelve los datos sin ningún tipo de recorte.

Si el valor a empaquetar es demasiado largo, el resultado se trunca.  Si es demasiado largo y se indica un contador explícito, C<Z> empaqueta solo C<$contador-1> bytes, seguido por un byte nulo.  Por lo tanto C<Z> siempre empaqueta un valor nulo a continuación, excepto cuando el contador es 0.

=item *

Del mismo modo, los formatos C<b> y C<B> empaquetan una cadena con una dada longitud de bits.
Cada uno de estos formatos genera un bit del resultado.
...
Al desempaquetar, los bits son convertidos en una cadena de C<"0"> y C<"1">.
...
Con caracteres no-alfabéticos, el resultado se basa en los 4 bits menos significativos del carácter de entrada, es decir, en C<ord($carácter)%16>.  En particular, los caracteres C<"0"> y C<"1"> generan los nibles 0 y 1, como los bytes C<"\0"> y C<"\1">.  Para los caracteres C<"a" .. "f"> y C<"A" .. "F">, el resultado es compatible con los dígitos hexadecimales de costumbre, de modo que C<"a"> y C<"A"> generan el nible C<0xa==10>.  No utilice más que estos caracteres con este formato.
...
Para C<pack>, I<secuencia-item> puede tener un contador de repeticiones, en cuyo caso el mínimo de él y del número de elementos disponibles es usado como el argumento para <longitud-item>. Si no tiene contador de repeticiones o utiliza un '*', se utiliza el número de elementos disponibles.
...
Indicar un contador al formato I<longitud-item> solo es útil con 
C<A>, C<a>, o C<Z>.  Empaquetando con un I<longitud-item> de C<a> o C<Z> puede introducir caracteres C<"\000">, que Perl no considerará como legal en cadenas numéricas.
...
Básicamente, Intel y VAX son little-endian, mientras que todos los demás, incluyendo Motorola m68k/88k, PPC, Sparc, HP PA, Power y Cray, son big-endian.  Alpha y MIPS pueden ser: Digital/Compaq utilizados/usados en modo little-endian, pero SGI/Cray los usa en modo big-endian.
...
Debido a la multiplicidad de formatos de punto flotante y la falta de una norma de representación en la "red" para ellos, no hay ninguna facilidad para el intercambio de datos.  Esto significa que datos empaquetados de punto flotante escritos en una máquina quizás no puedan ser leídos en otra, aunque ambas utilizan aritmética IEEE de punto flotante (porque la ordenación de bytes de la representación en memoria no es parte de la especificación IEEE).  Véase también L<perlport>.
...
Para los comandos de alineación, un C<contador> de 0 es equivalente a un C<contador> de 1; ambos son sentencias no efectivas.
...
Ejemplos:
    ...
    $bar = pack('s@4l', 12, 34);
    # un 12 short, relleno con ceros en la posición 4, un 34 long
    # $foo eq $bar
...
Ver L<IPC::Open2>, L<IPC::Open3>, y L<perlipc/"Comunicación Bidireccional"> para ejemplos de estas cosas.
...
Devuelve el valor indefinido si el array está vacío, aunque esto también puede suceder en otras situaciones.  Si ARRAY se omite, se extrae desde el array C<@ARGV> en el
programa principal, y el array C<@_> en subrutinas, igual que C<shift>.
...
Devuelve el desplazamiento donde la última búsqueda C<m//g> terminó para la variable en cuestión (C<$_> es usada cuando no se especifica la variable).  Tenga en cuenta que 0 es un desplazamiento válido de la coincidencia.  C<undef> indica que la posición de búsqueda se restablece (generalmente debido a un fallo de coincidencia, pero también puede ser porque aún no ha sido ejecutada la coincidencia sobre el escalar). C<pos> accede directamente a la ubicación utilizada por el motor de expresiones regulares para almacenar el desplazamiento, por lo que asignar a C<pos> cambiará ese desplazamiento, por lo que también influirá en la aserción de ancho cero C<\G> en las expresiones regulares. Debido a que una coincidencia C<m//gc> fallida no reinicia el desplazamiento, el retorno de C<pos> no cambiará en ese caso.  Ver L<perlre> y L<perlop>.

=item print GESTOR_FICHERO LISTA
X<print>
...
GESTOR_FICHERO puede ser una variable escalar conteniendo el nombre o una referencia al gestor de archivo, introduciendo así un nivel de indirección.  (NOTA: Si GESTOR_FICHERO es una variable y el siguiente símbolo es un término, puede ser mal interpretado como un operador a menos que se interponga un C<+> o poner entre paréntesis los argumentos). GESTOR_FICHERO, si se omite, se imprime en la salida estándar por defecto, o al canal de salida seleccionado la última vez, ver L</select>.  Si la LISTA también se omite, se imprime C<$_> al actual controlador de salida seleccionado.
Para configurar el controlador de salida predeterminada a otro valor distinto de STDOUT, use el operador select.  El valor actual de C<$,> (si lo tiene) se imprime entre cada elemento de la lista.  El valor actual de C<$\> (si lo tiene) se imprime después de que toda la lista haya sido impresa.  Debido a que print toma una LISTA, cualquier cosa en la LISTA se evalúa en contexto de lista, y cualquier subrutina que llame tendrá una o más de sus expresiones evaluadas en contexto de lista.  También tenga cuidado de no seguir la palabra clave print con un paréntesis de apertura a menos que desee que el correspondiente paréntesis de cierre ponga fin a los argumentos de la impresión; poner paréntesis alrededor de todos los argumentos (o interponer un C<+>, pero esto no parece tan bonito).

Tenga en cuenta que si usted está almacenando el GESTOR_FICHERO en un array, o si está usando cualquier otra expresión más compleja que una variable escalar para recuperarlo, tendrá que utilizar un bloque que retorne el valor del gestor de archivo:

    print { $ficheros[$i] } "cosas\n";
...
=item printf GESTOR_FICHERO FORMATO, LISTA
X<printf>

=item printf FORMATO, LISTA

Equivalente a C<print GESTOR_FICHERO sprintf(FORMATO, LISTA)>, excepto que C<$\> (el separador de registro a la salida) no es añadida.  El primer argumento de la lista se interpreta como el formato para C<printf>. Ver C<sprintf> para una explicación del argumento formato.  Si C<use locale> está activo, y POSIX::setlocale() ha sido llamada, el carácter utilizado para el separador decimal en números formateados en punto flotante se ve afectado por la configuración regional de LC_NUMERIC.  Ver L<perllocale> y L<POSIX>.
...
Si FUNCIÓN es una cadena que comienza con C<CORE::>, el resto es tomado como un nombre para una orden interna de Perl.  Si la orden interna no es yo <sobreescribible> (como lo es C<qw//>) o si sus argumentos no pueden ser adecuadamente expresados por un prototipo (como C<system>), prototype() devuelve C<undef>, porque la orden interna en realidad no se comporta como una función de Perl.  De lo contrario, se devuelve la cadena que describe el prototipo equivalente.

=item push ARRAY, LISTA
X<push> X<stack> X<pila>

Considera a ARRAY como una pila, e inserta los valores de LISTA al final de ARRAY.  La longitud de ARRAY se incrementa por la longitud de LISTA.  Tiene el mismo efecto que
...
Devuelve el valor de EXPR con todos los caracteres que no sean "palabra" con una barra invertida.  (Es decir, todos los caracteres que no se ajusten a C</[A-Za-z_0-9]/> serán precedidos por una barra invertida en la cadena devuelta, con independencia de cualquier configuración regional). Esta es la función interna que implementa el escapado de C<\Q> en las cadenas con doble entrecomillado.
...
Devuelve un número decimal aleatorio mayor o igual a C<0> y menor que el valor de EXPR.  (EXPR debe ser positiva). Si EXPR se omite, se usa el valor C<1>.  Actualmente EXPR con el valor de C<0> es también un caso especial como el de C<1> (no estaba documentado esto antes de Perl 5.8.0
...
=item read GESTOR_FICHERO,ESCALAR,LONGITUD,DESPLAZAMIENTO
X<read> X<file, read> X<fichero, lectura>

=item read GESTOR_FICHERO,ESCALAR,LONGITUD

Intento de leer LONGITUD I<caracteres> de datos en la variable ESCALAR del GESTOR_FICHERO especificado.  Devuelve el número de caracteres actualmente leídos, C<0> al final del fichero, o undef si hubo un error (en este último caso se actualiza el valor de C<$!>).  ESCALAR crecerá o se reducirá de manera que el último carácter leído es el último carácter del escalar después de la lectura.

Un DESPLAZAMIENTO puede ser especificado para colocar los datos leídos en algún otro lugar de la cadena distinto del principio.  Un DESPLAZAMIENTO negativo especifica el desplazamiento en número de caracteres contando desde el final de la cadena.  Un DESPLAZAMIENTO positivo mayor que la longitud de ESCALAR resulta en un cadena rellenada al tamaño requerido con bytes C<"\0"> antes de que el resultado de la lectura sea añadido.

La llamada es implementada en términos de Perl o de la función nativa fread(3) de su sistema.  Para obtener una auténtica llamada del sistema read(2), ver C<sysread>.

Note los I<caracteres>: dependiendo del estado del gestor de archivo, son leídos bytes (8-bit) o caracteres.  Por defecto todos los gestores de archivo operan en bytes, pero por ejemplo, si el gestor ha sido abierto con la capa de E/S C<:utf8> (ver L</open>, y el pragma C<open>, L<open>), la E/S operará en caracteres codificados en UTF-8, no bytes.  Del mismo modo, para el pragma C<:encoding>: en este caso casi todos los caracteres se pueden leer.

=item readdir GESTOR_DIRECTORIO
X<readdir>
...
Si está planeando hacer test de ficheros de los valores retornados por un C<readdir>, más le vale anteponer el directorio en cuestión.  De lo contrario, porque no hicimos un C<chdir> antes, se estará probando en un archivo incorrecto.

    opendir(my $dh, $algun_dir) || die "no puedo opendir $algun_dir: $!";
    @dots = grep { /^\./ && -f "$algun_dir/$_" } readdir($dh);
    closedir $dh;

A partir de Perl 5.11.2 puede usar un simple C<readdir> en un bucle C<while>, que actualizará C<$_> en cada iteración.
...
Lee del gestor de archivo cuyo typeglob esté contenido en EXPR (o desde *ARGV si se omite EXPR).  En contexto escalar, cada llamada lee y devuelve la siguiente línea hasta que se alcance el final del fichero, en cuyo caso la ulterior llamada devuelve C<undef>.  En contexto de lista, lee hasta el final de fichero y devuelve una lista de líneas.  Note que la noción de "línea" usada aquí es según tenga definido C<$/> o C<$INPUT_RECORD_SEPARATOR>.  Ver L<perlvar/"$/">.

Cuando C<$/> tenga un valor de C<undef>, cuando C<readline> esté en contexto escalar (es decir, modo de absorción completa de fichero), y cuando se esté leyendo un fichero vacío, devuelve C<''> la primera vez, seguido a continuación de C<undef>.
...
Si C<readline> encuentra un error del sistema operativo, C<$!> será puesto al correspondiente mensaje de error.  Puede ser útil comprobar C<$!> cuando está leyendo de gestores de archivo en los que no confía, como tty o un socket.  En el ejemplo siguiente se utiliza la forma del operador de C<readline> y muere si el resultado no está definido.
...
Note que no puede manejar errores de C<readline> de esta manera con el gestor C<ARGV>. En este caso, tiene que abrir cada elemento de C<@ARGV> debido a que C<eof> maneja C<ARGV> de otra manera.
...
Recibe un mensaje de un socket.  Intenta leer LONGITUD I<caracteres> de datos en la variable ESCALAR del gestor SOCKET especificado.
...
Note los I<caracteres>: dependiendo del estado del socket, son leídos bytes (8-bit) o caracteres.  Por defecto todos los sockets operan en bytes, pero por ejemplo, si el socket ha sido cambiado usando binmode() para operar con la capa de E/S C<:encoding(utf8)> (ver el pragma C<open>, L<open>), la E/S operará en caracteres codificados en UTF-8, no bytes.  Del mismo modo, para el pragma C<:encoding>: en este caso casi todos los caracteres se pueden leer.
...
El comando C<redo> reinicia la iteración sin evaluar de nuevo la condición.  El bloque C<continue>, si está, no es ejecutado:  Si la ETIQUETA se omite, el comando se refiere al bucle más interior en que se encuentre.  Los programas que quieren engañarse a sí mismos acerca de lo que se acaba de introducir suelen utilizar este comando:
...
Si el objeto referenciado ha sido bendecido en un paquete, entonces es el nombre de este paquete el que es devuelto en su lugar.  Puede pensar en C<ref> como un operador C<typeof>.
...
El valor de retorno C<LVALUE> indica una referencia a un valor izquierdoso que no es una variable. Obtiene esto de la referencia a las llamadas a funciones como C<pos()> o C<substr()>. C<VSTRING> se devuelve si la referencia apunta a una L<version string|perldata/"Cadenas de versión">.
...
X<rename> X<move> X<mv> X<ren> X<renombrar> X<mover>
...
De lo contrario, C<require> demanda que un archivo de biblioteca sea incluido si no se hubiera incluido ya.  El archivo es incluido a través del mecanismo do FICHERO, que es esencialmente lo mismo que una variedad de C<eval> con la advertencia de que las variables léxicas en la secuencia de comandos del programa serán invisibles para el código incluido.  Tiene una semántica similar a la subrutina siguiente:

    sub require {
       my ($nombrefichero) = @_;
       if (exists $INC{$nombrefichero}) {
           return 1 if $INC{$nombrefichero};
           die "Falló la compilación en require";
       }
       my ($realnombrefichero,$resultado);
       ITER: {
           foreach $prefijo (@INC) {
               $realnombrefichero = "$prefijo/$nombrefichero";
               if (-f $realnombrefichero) {
                   $INC{$nombrefichero} = $realnombrefichero;
                   $resultado = do $realnombrefichero;
                   last ITER;
               }
           }
           die "No encuentro $nombrefichero en \@INC";
       }
       if ($@) {
           $INC{$nombrefichero} = undef;
           die $@;
       } elsif (!$resultado) {
           delete $INC{$nombrefichero};
           die "$nombrefichero no devolvió un valor verdadero";
       } else {
           return $resultado;
       }
    }
...
El archivo debe devolver verdadero como última sentencia para indicar una ejecución exitosa de cualquier código de inicialización, por lo que es habitual a tal efecto que un archivo termine con un C<1;> a menos que esté seguro de que devolverá cierto, de otra manera.  Pero es mejor sólo para poner C<1;>, en caso de que añada más sentencias.

Si EXPR es una simple palabra, el require asume una extensión "F<.pm>" y le reemplaza "F<::>" con "F</>" en el nombre del fichero, para que sea fácil cargar módulos estándar.  Esta forma de cargar módulos no tiene el riesgo de alterar el espacio de nombres propio.
...
También puede insertar enganches en la facilidad import, poniendo código Perl directamente en el array C<@INC>.  Hay tres formas de enganches: referencias a subrutinas, referencias a arrays y objetos bendecidos.

Referencias a subrutinas son el caso más simple.  Cuando el sistema de inclusión recorre C<@INC> y se encuentra con una subrutina, esta subrutina es llamada con dos parámetros; el primero, una referencia a sí mismo; y el segundo, el nombre del fichero que se incluirá (por ejemplo, "F<Foo/Bar.pm>").  La subrutina debe devolver, o nada, o bien una lista de hasta tres valores en el orden siguiente:

=over

=item 1

Un gestor de fichero, desde el cual, el fichero será leído.  

=item 2

Una referencia a una subrutina. Si no hay un gestor de archivo (punto anterior), entonces esta subrutina se espera que genere una línea de código fuente por llamada, escribiendo en la línea C &lt;$ _&gt; y devolver 1, a continuación, volver 0 al final del archivo.  Si hay un gestor de archivo, a continuación, la subrutina será llamada a actuar como un simple filtro de código fuente, con la línea leída en C<$_>.
...
Tenga en cuenta que este gestor de archivo debe ser un gestor de archivo real (en sentido estricto un typeglob o referencia a un typeglob, bendecido o no); controladores de archivo enlazados serán ignorados y el procesado del valor de retorno terminará ahí.
...
En otras palabras, puede escribir:
    ...
    push @INC, \&mi_sub;
    sub mi_sub {
        my ($coderef, $fichero) = @_;  # $coderef is \&mi_sub
        ...
    }
...
Regresa de una subrutina, C<eval> o C<do FICHERO>, con el valor indicado en EXPR.  Evaluación de EXPR puede estar en contextos lista, escalar, o contexto vacío, dependiendo de cómo el valor de retorno será utilizado, y el contexto puede variar de una ejecución a la siguiente (ver C<wantarray>).  Si no se indica EXPR, devuelve una lista vacía en contexto de lista, el valor indefinido en el contexto escalar, y (por supuesto) nada en absoluto en el contexto vacío.

(En ausencia de un C<return> explícito, una subrutina, eval, o do FICHERO, automáticamente devuelve el valor de la última expresión evaluada).
...
Tenga en cuenta que invertir un array en sí mismo (como en C<@a = reverse @a>) preservará elementos inexistentes cuando le sea posible, es decir, para arrays no mágicos o arrays enlazados con métodos C<EXISTS> y C<DELETE>.

Este operador también es útil para invertir un hash, aunque existen algunas salvedades.  Si un valor está duplicado en el hash original, sólo uno de ellos puede ser representado como clave en el hash invertido.  Además, esto tiene el efecto de desmontar un hash y construir uno nuevo, algo que puede tardar algún tiempo en un hash grande, como en un archivo DBM.

    %por_nombre = reverse %por_direccion;  # Invierte el hash

=item rewinddir GESTOR_DIRECTORIO
X<rewinddir>

Establece la posición actual al comienzo del directorio para la rutina C<readdir> en GESTOR_DIRECTORIO.
...
Igual que C<print>, pero implícitamente añade un carácter de nueva línea.
C<say LISTA> es simplemente una abreviatura de C<{ local $\ = "\n"; print LISTA }>.

Esta palabra clave sólo está disponible cuando la característica "say" está habilitada: ver L<feature> .

=item scalar EXPR
X<scalar> X<context> X<contexto>
...
Dado que C<scalar> es un operador unario, si, accidentalmente, usa para EXPR una lista entre paréntesis, esto se comportará como una expresión escalar con comas, evaluando todos menos el último elemento en contexto vacío, y devolviendo el elemento final evaluado en contexto escalar.  Esto es muy raro que quiera hacerlo.
...
=item seek GESTOR_FICHERO,POSICIÓN,DONDE
X<seek> X<fseek> X<filehandle, position> X<gestor fichero, posición>

Establece la posición de GESTOR_FICHERO, igual que la llamada C<fseek> de C<stdio>.
GESTOR_FICHERO puede ser una expresión cuyo valor da el nombre del gestor de archivo.  Los valores de DONDE son C<0> para poner la nueva posición I<en bytes> a POSICIÓN, C<1> para poner la posición actual más POSICIÓN, y C<2> para ponerlo a EOF más POSICIÓN (normalmente negativo).  Para DONDE puede usar las constantes C<SEEK_SET>,
C<SEEK_CUR>, y C<SEEK_END> (inicio de fichero, posición actual, y fin de fichero) desde el módulo Fcntl.  Devuelve C<1> en caso de éxito; de otra manera, C<0>.

Nota sobre lo de I<en bytes>: incluso si el gestor de fichero se ha puesto para operar en caracteres (por ejemplo usando la capa de open C<:encoding(utf8)>), tell() devolverá desplazamientos en bytes, no en caracteres (porque implementando esto convertirían en lentas a seek() y tell()).

Si desea posicionar el fichero para las funciones C<sysread> o C<syswrite>, no utilice C<seek>, porque hace impredecible y no portátil la posición de lectura/escritura del fichero a causa del efecto del buffer del fichero.  Use en su lugar C<sysseek>.
...
Si eso no funciona (algunas implementaciones de E/S son particularmente irritables), puede que tenga que hacer algo como esto:
    ...
    for (;;) {
        for ($curpos = tell(FILE); $_ = <FILE>;
             $curpos = tell(FILE)) {
            # buscar por algo y ponerlo en el fichero
        }
        sleep($por_un_rato);
        seek(FILE, $curpos, 0);
    }
...
Establece la posición actual de la rutina C<readdir> en GESTOR_DIR.  POS debe ser un valor devuelto por C<telldir>.  C<seekdir> también tiene las mismas advertencias sobre la posible compactación del directorio como la correspondiente rutina del sistema.

=item select GESTOR_FICHERO
X<select> X<filehandle, default>

=item select

Devuelve el gestor de archivo seleccionado.  Si se indica un GESTOR_FICHERO, se establece un nuevo gestor de archivo predeterminado para la salida.  Esto tiene dos efectos: primero, un C<write> o un C<print> sin un gestor de archivo será por defecto este GESTOR_FICHERO.  En segundo lugar, las referencias a variables relacionadas con la salida se referirán a este canal de salida.  Por ejemplo, si tiene que fijar la parte superior del formato de un formulario para más de un canal de salida, puede hacer lo siguiente:
...
GESTOR_FICHERO puede ser una expresión cuyo valor da el nombre del gestor de archivo real.  Por lo tanto:

    $oldfh = select(STDERR); $| = 1; select($oldfh);

Algunos programadores prefieren pensar en gestores de archivo como objetos con métodos, prefiriendo escribir el último ejemplo como:
...
Si desea seleccionar en muchos gestores de archivo, puede desear escribir una subrutina de esta manera:
...
Tenga en cuenta que cuando C<select> es reiniciado después de señales (por ejemplo, SIGALRM) es algo dependiente de la implementación.  Véase también L<perlport> para las notas sobre la portabilidad de C<select>.

En caso de error, C<select> se comporta como select(2): devuelve -1 y actualiza C<$!>.

En algunos sistemas Unix, select(2) puede informar de un descriptor de fichero socket como "listo para leer" cuando no hay datos disponibles, y por lo tanto una lectura posterior produce un bloqueo. Esto puede evitarse si siempre utiliza O_NONBLOCK en el socket. Ver select(2) y fcntl(2) para más detalles.
...
Véase también la documentación de L<perlipc/"SysV IPC">, C<IPC::SysV>, y C<IPC::Semaphore>.

=item semget KEY,NSEMS,FLAGS
X<semget>

Llama a la función System V IPC semget(2).  Devuelve el identificador del semáforo, o el valor indefinido si hay un error.  Véase también la documentación de L<perlipc/"SysV IPC">, C<IPC::SysV>, y C<IPC::Semaphore>.

=item semop KEY,OPSTRING
X<semop>

Llama a la función System V IPC semop(2) para operaciones de semáforo como señalización y espera.  OPSTRING debe ser un array empaquetado de estructuras semop.  Cada estructura semop se puede generar con C<pack("s!3", $semnum, $semop, $semflag)>.  La longitud de OPSTRING implica el número de operaciones de semáforos.  Devuelve verdadero si tiene éxito, o falso si hay un error.  A modo de ejemplo, el siguiente código espera en un semáforo $semnum del semáforo con identificador $semid:

    $semop = pack("s!3", $semnum, -1, 0);
    die "Problema son semáforo: $!\n" unless semop($semid, $semop);

Para señalizar el el semáforo, reemplace C<-1> con C<1>.  Véase también la documentación de L<perlipc/"SysV IPC">, C<IPC::SysV>, y C<IPC::Semaphore>.
...
Envía un mensaje sobre un socket.  Intenta enviar el escalar MSG al gestor de archivo SOCKET.  Toma las mismas banderas que la llamada al sistema del mismo nombre.  En sockets desconectados, debe especificar un destino I<adonde> enviarlo, en cuyo caso hace una llamada del sistema sendto(2).  Devuelve el número de caracteres enviados, o el valor indefinido en caso de error.  La llamada del sistema sendmsg(2) no está, actualmente, implementada.  Ver L<perlipc/"UDP: Paso de mensajes"> para ejemplos.
...
Establece el grupo de procesos en curso para el PID especificado, C<0> para el proceso actual.  Eleva una excepción si se usa en una máquina que no implemente POSIX setpgid(2) o BSD setpgrp(2).  Si los argumentos son omitidos, el valor predeterminado es C<0,0>.  Tenga en cuenta que la versión de BSD 4.2 de C<setpgrp> no acepta ningún argumento, por lo que sólo C<setpgrp(0,0)> es portátil.  Véase también C<POSIX::setsid()>.

=item setpriority QUÉ,QUIÉN,PRIORIDAD
X<setpriority> X<priority> X<nice> X<renice> X<prioridad>

Devuelve la prioridad actual de un proceso, un grupo de procesos o un usuario.
(Ver setpriority(2)). Lanza una excepción cuando se utiliza en una máquina que no implementa setpriority(2).

=item setsockopt SOCKET,NIVEL,OPCIÓN_NOMBRE,OPCIÓN_VALOR
X<setsockopt>

Establece la opción de socket solicitada.  Devuelve indefinido si hay un error.  Utilice constantes enteras proporcionadas por el módulo C<Socket> para NIVEL y OPCIÓN_NOMBRE.  Los valores para NIVEL también se pueden obtener de getprotobyname.  OPCIÓN_VALOR podría ser una cadena empaquetada o un entero.
...
Extrae el primer valor del array y lo devuelve, acortando el array en uno y moviendo todos los demás hacia arriba.  Si no hay elementos en el array, devuelve el valor undef.  Si ARRAY se omite, desplaza el array C<@_> en el ámbito léxico de subrutinas y formatos, y el array C<@ARGV> fuera de una subrutina y también dentro de los ámbitos léxicos establecidos por construcciones como C<eval CADENA>, C<BEGIN {}>, C<INIT {}>, C<CHECK {}>, C<UNITCHECK {}> y C <END {}>.

Véase también C<unshift>, C<push>, y C<pop>.  C<shift> y C<unshift> hacen lo mismo con el extremo izquierdo de un array que C<pop> y C<push> hacen para el extremo derecho.
...
primero para obtener las definiciones de las constantes.  Si CMD es C<IPC_STAT>, entonces ARG debe ser una variable que contendrá la estructura C<shmid_ds> devuelta.  Devuelve lo mismo que C<ioctl>: el valor indefinido para error, C<"0 but true"> para el cero, o el valor de retorno real, en caso contrario.
Véase también la documentación de L<perlipc/"SysV IPC"> y C<IPC::SysV>.

=item shmget CLAVE,TAMAÑO,BANDERAS
X<shmget>

Llama a la función System V IPC shmget(2).  Devuelve el identificador de la memoria compartida, o el valor indefinido si hay un error.
Véase también la documentación de L<perlipc/"SysV IPC"> y C<IPC::SysV>.
...
Lee o escribe en el segmento ID de memoria compartida System V a partir de la posición POSICIÓN para el tamaño TAMAÑO, añadiéndose a ella, copiando la entrada/salida, y desuniéndose de ella.  Al leer, VAR debe ser una variable que contendrá los datos leídos.  Al escribir, si CADENA es demasiado larga, sólo se utilizan TAMAÑO bytes; si CADENA es demasiado corta, se escriben nulos para llenar TAMAÑO bytes.  Regresa verdadero si tiene éxito, o falso si hay un error.
shmread() contamina a la variable. Véase también la documentación L<perlipc/"SysV IPC">, C<IPC::SysV>, y el módulo C<IPC::Shareable> en CPAN.
...
Devuelve C<1> para el éxito; en caso de error, devuelve C<undef> si el primer argumento no es un gestor de archivo válido, o devuelve C<0> y actualiza C<$!> por cualquier otro fallo.

=item sin EXPR
X<sin> X<sine> X<asin> X<arcsine> X<seno>
...
Abre un socket de la clase especificada y lo adhiere al gestor de archivo SOCKET.  DOMINIO, TIPO, y PROTOCOLO son especificados de la misma manera que la llamada del sistema del mismo nombre.  Debe usar C<use Socket> primero para obtener las apropiadas definiciones.  Ver los ejemplos en L<perlipc/"Sockets: Comunicación Cliente/Servidor">.
...
En sistemas que apoyan la bandera close-on-exec en ficheros, esta bandera se establecerá para los nuevos descriptores de fichero abiertos, según lo determinado por el valor de $^F.  Ver L<perlvar/$^F>.
...
X<sort> X<qsort> X<quicksort> X<mergesort> X<ordenar>
...
Si NOMBRE_SUB O BLOQUE se omite, se ordena siguiendo una comparación estándar de cadenas de caracteres.  Si se ha especificado NOMBRE_SUB, da el nombre de la subrutina que devuelve un entero menor que, igual que, o mayor que C<0>, dependiendo de cómo los elementos de la lista tienen que ser ordenados.  (Los operadores C<< <=> >> y C<cmp> son muy útiles en este tipo de rutinas). NOMBRE_SUB puede ser el nombre de una variable escalar, en cuyo caso el valor provee el nombre de (o la referencia a) la actual subrutina a usar.  En lugar de un NOMBRE_SUB, puede proporcionar un BLOQUE como si fuera una subrutina de ordenación anónima.

Si el prototipo de la subrutina es C<($$)>, los elementos a ser comparados son pasados por referencia en C<@_>, como en una subrutina normal.  Esto es más lento que las subrutinas no prototipadas, donde los elementos a comparar son pasados a la subrutina en las variables globales del paquete actual $a y $b (ver ejemplo abajo).  Tenga en cuenta que en este último caso, que suele ser contraproducente declarar $a y $b como léxicas.
...
Cuando C<use locale> esté activo, C<sort LIST> ordena LISTA de acuerdo con la actual configuración regional de intercalación.  Ver L<perllocale>.
...
Ejemplos:
    ...
    # ordenación léxica
    @articulos = sort @ficheros;
    
    # lo mismo, pero con una rutina de ordenación explícita
    @articulos = sort {$a cmp $b} @ficheros;
    
    # ahora, independiente del tamaño de caja
    @articulos = sort {uc($a) cmp uc($b)} @ficheros;
    
    # lo mismo, en orden inverso
    @articulos = sort {$b cmp $a} @ficheros;
    
    # ordenación numérica ascendente
    @articulos = sort {$a <=> $b} @ficheros;
    
    # ordenación numérica descendente
    @articulos = sort {$b <=> $a} @ficheros;
...
Advertencia: es requerido un cuidado sintáctico cuando se ordena la lista devuelta desde una función. Si quiere ordenar la lista devuelta por la llamada a una función
C<busca_registros(@claves)>, puede utilizar:
...
Si está usando strict, I<no debe> declarar $a
y $b como léxicas.  Son variables globales del paquete.  Esto significa que si usted está en el paquete C<main> y escribe

    @articulos = sort {$b <=> $a} @ficheros;

entonces C<$a> y C<$b> son C<$main::a> y C<$main::b> (o C<$::a> y C<$::b>), pero si está en el paquete C<FooPack>, es lo mismo que escribir

    @articulos = sort {$FooPack::b <=> $FooPack::a} @ficheros;
...
Dado que C<< <=> >> devuelve C<undef> cuando alguno de los operandos es C<NaN> (I<not-a-number>, no es un número), y porque C<sort> lanza una excepción a menos que el resultado de la comparación esté definido, al ordenar con una función de comparación como C<< $a <=> $b >>, tenga cuidado acerca de las listas que pueden contener un C<NaN>.
En el ejemplo siguiente se aprovecha de que C<NaN != NaN> para eliminar cualquier C<NaN> de la lista de entrada.
...
Si LONGIIUD es negativa, elimina los elementos desde DESPLAZAMIENTO hacia adelante, excepto -LONGITUD elementos al final del array.
Si ambos DESPLAZAMIENTO y LONGITUD se omiten, quita todo. Si el DESPLAZAMIENTO es más allá del final del array, Perl lanza una advertencia, y extrae al final del array.

Las siguientes equivalencias se mantienen (suponiendo que C<< $[ == 0 y $#a >= $i >> )
...
Divide la cadena EXPR en una lista de cadenas y devuelve esa lista.  Por defecto, los campos vacíos precedentes son preservados, y los campos vacíos consecuentes son eliminados.  (Si todos los campos están vacíos, se consideran que están al final).

En contexto escalar, devuelve el número de campos que se han encontrado.

Si EXPR es omitido, divide la cadena C<$_>.  Si PATRÓN es también omitido, se divide por el espacio en blanco(después de saltarse cualquier espacio en blanco precedente).  Cualquier cosa concordante con el PATRÓN se toma como un delimitador que separa los campos.  (Tenga en cuenta que el delimitador puede ser más largo que un carácter).

Si LÍMITE está especificado y es positivo, representa el máximo número de campos que EXPR será dividido, aunque el número real de campos devueltos depende del número de veces que PATRÓN concuerda en EXPR.  Si LÍMITE no está especificado o es cero, los campos vacíos finales son descartados (que los potenciales usuarios de C<pop> harían bien en recordar).
Si LÍMITE es negativo, se trata como si se hubiera especificado un LÍMITE arbitrariamente grande.  Tenga en cuenta que la división de una EXPR que evalúe a una cadena vacía siempre devuelve una lista vacía, sin importar el LÍMITE especificado.

Un patrón de coincidencia con la cadena vacía (que no debe confundirse con un patrón vacío C<//>, que es sólo un miembro del conjunto de patrones coincidentes con la cadena vacía), divide EXPR en caracteres individuales.  Por ejemplo:

    print join(':', split(/ */, 'hola por aqui')), "\n";

produce la salida 'h:o:l:a:p:o:r:a:q:u:i'.

Como un caso especial para C<split>, el patrón vacío C<//> específicamente coincide con la cadena vacía; esto no ha de ser confundido con el uso normal de un patrón vacío que significa la última coincidencia exitosa.  Así que para dividir una cadena en caracteres individuales, lo siguiente:

    print join(':', split(//, 'hola por aqui')), "\n";

produce la salida 'h:o:l:a: :p:o:r: :a:q:u:i'.

Campos vacíos antecedentes son producidos cuando hay coincidencias de ancho positivas al principio de la cadena; una coincidencia de ancho cero al principio de la cadena no produce un campo vacío. Por ejemplo:

   print join(':', split(/(?=\w)/, 'hola por aqui!'));

produce la salida 'h:o:l:a :p:o:r :a:q:u:i!'. Campos vacíos consecuentes, por el contrario, se producen cuando hay una coincidencia al final de la cadena (y cuando LÍMITE se ha dado y no es 0), independientemente de la longitud de la coincidencia.
Por ejemplo:

   print join(':', split(//,   'hola aqui!', -1)), "\n";
   print join(':', split(/\W/, 'hola aqui!', -1)), "\n";

produce la salida 'h:o:l:a: :a:q:u:i:!:' y 'hola:aqui:', respectivamente, ambos con un campo vacío final.

El parámetro LÍMITE puede usarse para dividir una línea de forma parcial

    ($login, $contraseña, $resto) = split(/:/, $_, 3);

Al asignar a una lista, si LÍMITE es omitido, o cero, Perl proporciona un LÍMITE uno mayor que el número de variables en la lista, para evitar trabajo innecesario.  Para la lista de arriba LÍMITE hubiera sido 4 por defecto.  En aplicaciones de tiempo crítico no es conveniente dividir en más campos de los que realmente necesita.

Si el PATRÓN contiene paréntesis, se crean elementos de lista adicionales por cada subcadena coincidente con el delimitador.
...
Si usted tuviera el encabezado completo de un mensaje de correo electrónico Unix normal en $encabezado, puede dividirlo en sus campos y valores de esta manera:

    $encabezado =~ s/\n(?=\s)//g;  # arregla la continuación de líneas
    %cabeceras  =  (UNIX_FROM => split /^(\S*?):\s*/m, $encabezado);

El patrón C</PATRÓN/> puede ser sustituido por una expresión para especificar patrones que varían en tiempo de ejecución.  (Para hacer la compilación en tiempo de ejecución sólo una vez, utilice C</$variable/o>.)

Como caso especial, especificando un PATRÓN de un espacio (S<C<' '>>) dividirá en espacios en blanco al igual que lo hace C<split> sin argumentos.  Por lo tanto, S<C<split(' ')>> puede ser utilizado para emular el comportamiento por defecto de B<awk>, mientras que S<C<split(/ /)>> le dará tantos campos nulos iniciales (cadenas vacías) como espacios haya allí.
Un C<split> con C</\s+/> es como un S<C<split(' ')>> excepto que cualquier espacio en blanco inicial produce un primer campo nulo.  Un C<split> sin argumentos realmente es internamente un S<C<split(' ', $_)>>.

Un PATRÓN C</^/> es tratado como si fuera C</^/m>, ya que de lo contrario no serviría de mucho.
...
Al igual que con la coincidencia de patrones regulares, cualquier paréntesis de captura que no coincida en un C<split()> se establecerá a C<undef>:
...
Devuelve una cadena formateada por los habituales convenios de C<printf> de la función de biblioteca C<sprintf>.  Ver abajo para más detalles y ver C<sprintf(3)> o C<printf(3)> en su sistema para obtener una explicación de los principios generales.
...
A diferencia de C<printf>, C<sprintf> no hace lo que probablemente quiere decir cuando se le pasa un array como primer argumento. El array se le da en contexto escalar, y en lugar de utilizar el elemento 0 del array como el formato, Perl usará el número de elementos del array como el formato, que casi nunca es útil.
...
uno o más de:
   ...
   #       garantiza el "0" precedente en cualquier octal,
           prefija los hexadecimales distintos de cero con "0x" o "0X",
           prefija los binarios distintos de cero con "0b" o "0B"
...
Si un ancho de campo obtenidos a través de C<*> es negativo, tiene el mismo efecto que la bandera C<->: alineación a la izquierda.
...
Para formatos en punto flotante, excepto 'g' y 'G', especifica cuántos lugares a la derecha del punto decimal debe mostrar (el valor predeterminado es 6).
Por ejemplo:
...
Para "g" y "G", especifica el número máximo de dígitos para mostrar, incluyendo los anteriores del punto decimal y los posteriores a él, por ejemplo:
...
Este último elevará una excepción si Perl no entiende "quads" en su instalación. (Esto requiere que la plataforma, ya sea que soporte quads de forma nativa, o que Perl fuera compilado específicamente para apoyar quads). Usted puede averiguar si su Perl soporta quads a través de L<Config>:
...
Si C<use locale> está en vigor y POSIX::setlocale() ha sido llamado, el carácter utilizado para el separador decimal en formato de punto flotante de los números se ve afectada por la configuración regional LC_NUMERIC.  Ver L<perllocale> y L<POSIX>.

=item sqrt EXPR
X<sqrt> X<root> X<square root> X<raíz> X<raíz cuadrado>
...
=item srand EXPR
X<srand> X<seed> X<randseed> X<semilla>

=item srand

Establece la semilla de números aleatorios para el operador C<rand>.

La misión de la función es "alimentar" a la función C<rand> para que C<rand> pueda producir una secuencia diferente cada vez que ejecute su programa.

Si srand() no se llama de forma explícita, es llamada implícitamente en el primer uso del operador C<rand>.  Sin embargo, esto no fue cierto para las versiones de Perl anteriores a la 5.004, así que si su programa se va a ejecutar en versiones anteriores de Perl, debería llamar a C<srand>.

La mayoría de los programas ni siquiera llaman a srand(), excepto los que necesitan un fuerte punto de partida criptográfico en lugar del valor por defecto, que se basa en la hora del día, el ID del proceso, la asignación de memoria, o el dispositivo F</dev/urandom> si está disponible.

Puede llamar a srand($semilla) con la misma $semilla para reproducir la I<misma> secuencia desde rand(), pero esto generalmente se reserva para la generación de resultados predecibles para pruebas o depuración.
De lo contrario, no llame a srand() más de una vez en su programa.

B<No> llame a srand() (e.d., sin un argumento) más de una vez en su script.  El estado interno del generador de números aleatorios debería contener más entropía que la ofrecida por cualquier semilla, así que volver a llamar a srand() realmente hace I<perder> la aleatoriedad.
...
En las versiones de Perl anteriores a 5.004 la semilla por defecto era sólo el valor actual de C<time>.  Esto no es una buena semilla, por lo que demasiados viejos programas tienen sus propias semillas (a menudo C<time ^ $$> o C<time ^ ($$ + ($$ << 15))>), pero esto ya no es necesario.

Para propósitos criptográficos, necesita algo mucho más aleatorio para el valor por defecto de la semilla.  El método usual es la suma de control de la salida comprimida de uno o más programas, rápidamente cambiantes, de estado del sistema operativo.  Por ejemplo:

    srand (time ^ $$ ^ unpack "%L*", `ps axww | gzip -f`);

Si está especialmente preocupado con esto, vea el módulo en CPAN C<Math::TrulyRandom>.
...
=item stat GESTOR
X<stat> X<file, status> X<ctime> X<fichero, estado>
...
Devuelve una lista de 13 elementos dando la información de estado de un archivo, tanto si el archivo ha sido abierto a través de GESTOR o GESTOR_DIR, o nombrado por la EXPR.  Si EXPR se omite, ve el estado de C<$_>.  Devuelve la lista vacía si C<stat> falla.  Típicamente se usa como sigue:

    ($dev,$ino,$mode,$nlink,$uid,$gid,$rdev,$size,
       $atime,$mtime,$ctime,$blksize,$blocks)
           = stat($fichero);

No todos los campos están soportados por todos los sistemas de ficheros.  Aquí están los significados de los campos:

  0 dev      número de dispositivo del sistema de ficheros
  1 ino      número de inodo
  2 mode     modo de fichero (tipo y permisos)
  3 nlink    número de enlaces (duros) al fichero
  4 uid      ID numérico del usuario propietario del fichero
  5 gid      ID numérico del grupo del propietario del fichero
  6 rdev     identificador del dispositivo (solo ficheros especiales)
  7 size     tamaño total del fichero, en bytes
 ...
 11 blksize  tamaño preferido del bloque de E/S para el sistema de ficheros
 12 blocks   número actual de bloques reservados

(El epoch fue a las 00:00 GMT del 1 de enero de 1970)

(*) No todos los campos están soportados por todos los sistemas de ficheros. En particular, el campo ctime no es portátil.  En particular, no puede esperar que sea el "tiempo de creación", véase L<perlport/"Ficheros y sistemas de ficheros"> para más detalles.

Si a C<stat> se le pasa el gestor de archivo especial consistente en un subrayado, no se hacen estadísticas, sino que se devuelven el contenido actual de la estructura de estadísticas de la última llamada C<stat>, C<lstat>, o test de ficheros.  Ejemplo:

    if (-x $fichero && (($d) = stat(_)) && $d < 0) {
        print "$fichero es un fichero NFS ejecutable\n";
    }
...
En contexto escalar, C<stat> devuelve un valor booleano que indica el éxito o el fracaso y, si tiene éxito, establece la información asociada con el gestor de archivo especial C<_>.

El módulo <File::stat> proporciona un conveniente mecanismo de acceso por nombre:

    use File::stat;
    $sb = stat($fichero);
    printf "Fichero es %s, tamaño es %s, permisos %04o, mtime %s\n",
           $fichero, $sb->size, $sb->mode & 07777,
...
y las funciones C<S_IF*> son
    ...
    S_IMODE($modo)    la parte de $modo conteniendo los bits de permisos
            y los bits setuid/setgid/sticky

    S_IFMT($modo)    la parte de $modo conteniendo el tipo de archivo
            que puede ser bit-andeado con (por ejemplo) S_IFREG
...
C<state> variables sólo se activan cuando el pragma C<use feature "state"> está en vigor.  Ver L<feature>.
...
Tenga en cuenta que si usted tiene que buscar en cadenas que no sabe hasta el tiempo de ejecución, puede crear un bucle completo como una cadena y luego hacerle un C<eval>, para evitar tener que volver a recompilar todos los patrones todo el tiempo.  Junto con la indefinición de C<$/> para la entrada de archivos completos como un único registro, esto puede ser bastante rápido, a menudo más rápido que los programas especializados como fgrep(1).  Lo siguiente examina una lista de archivos (C<@ficheros>) por una lista de palabras (C<@palabras>), e imprime los nombres de los archivos que contienen una coincidencia:
    ...
    foreach $fichero (sort keys(%visto)) {
        print $fichero, "\n";
    }
...
Esta es la definición de subrutina, no una función real I<per se>.
Sin un BLOQUE es sólo una declaración previa.  Sin un NOMBRE, es una declaración de una función anónima, y devuelve un valor: la referencia al CÓDIGO de la clausura que acaba de crear.

Ver L<perlsub> y L<perlref> para los detalles acerca de subrutinas y referencias, y L<attributes> y L<Attribute::Handlers> para obtener más información acerca de los atributos.

=item substr EXPR,DESPLAZAMIENTO,LONGITUD,REEMPLAZO
X<substr> X<substring> X<mid> X<left> X<right> X<subcadena> X<medio> X<izquierda> X<derecha>

=item substr EXPR,DESPLAZAMIENTO,LONGITUD

=item substr EXPR,DESPLAZAMIENTO

Extrae una subcadena de EXPR y la devuelve.  El primer carácter está en la posición C<0>, o lo que sea que haya configurado en C<$[> (pero no lo haga).
Si DESPLAZAMIENTO es negativo (o más precisamente, menor que C<$[>), cuenta la posición desde el final de la cadena.  Si se omite LONGITUD, devuelve todo hasta el final de la cadena.  Si LONGITUD es negativa, deja fuera a tantos caracteres del final de la cadena.
...
Puede utilizar la función substr() como un valor izquierdoso, en cuyo caso la EXPR debe ser en si mismo un valor izquierdoso.  Si asigna algo más corto que LONGITUD, la cadena se reducirá, y si asigna algo más largo que LONGITUD, la cadena crecerá para acogerlo.  Para mantener la cadena de la misma longitud, puede ser necesario rellenar o recortar su valor con C<sprintf>.

Si DESPLAZAMIENTO y LONGITUD especifican una subcadena que está parcialmente fuera de la cadena, sólo la parte de dentro de la cadena se devuelve.  Si la subcadena está más allá de cada extremo de la cadena, substr() devuelve el valor indefinido y produce una advertencia.  Cuando se utiliza como un valor izquierdoso, especificando una subcadena que está totalmente fuera de la cadena lanza una excepción.
...
Una alternativa al uso substr() como un valor izquierdoso es especificar la cadena de reemplazo como el cuarto argumento.  Esto le permite reemplazar partes de la EXPR y devolver lo que había antes, en una operación, tal como se puede hacer con splice().

    my $s = "The black cat climbed the green tree";
    my $z = substr $s, 14, 5, "saltó desde";    # subió
    # $s es ahora "El gato negro saltó desde el árbol verde"

Tenga en cuenta que el valor izquierdoso devuelto por la versión de 3 argumentos de substr() actúa como una "bala mágica"; cada vez que se le asigna, recuerda qué parte de la cadena original se ha modificado; por ejemplo:
...
Antes de la versión Perl 5.9.1, no estaba especificado el resultado de usar varias veces un valor izquierdoso.

=item symlink FICHEROVIEJO,FICHERONUEVO
X<symlink> X<link> X<symbolic link> X<link, symbolic> X<enlace> X<enlace, simbólico> X<enlace simbólico>
...
Tenga en cuenta que algunas llamadas al sistema legítimamente pueden devolver C<-1>.  La forma correcta de manejar llamadas de ese tipo es el de asignar C<$!=0;> antes de la llamada y comprobar el valor de C<$!> si syscall devuelve C<-1>.

Hay un problema con C<syscall(&SYS_pipe)>: devuelve el número de fichero de la lectura final de la tubería que crea.  No hay manera de recuperar el número de fichero del otro extremo.  Puede evitar este problema mediante el uso, en su lugar, de C<pipe>.

=item sysopen GESTOR,FICHERO,MODO
X<sysopen>

=item sysopen GESTOR,FICHERO,MODO,PERMISOS

Abre el archivo cuyo nombre viene dado por FICHERO, y lo asocia con GESTOR.  Si GESTOR es una expresión, el valor es tomado como el nombre real del gestor buscado.  Esta función llama a la función subyacente del sistema operativo C<open> con los parámetros FICHERO, MODO, PERMISOS.

Los valores posibles y bits de bandera del parámetro MODO son dependientes del sistema; están disponibles en el módulo estándar C<Fcntl>.
Consulte la documentación de C<open> de su sistema operativo para ver qué valores y bits de bandera están disponibles.  Puede combinar varias banderas con el operador C<|>.
...
Por razones históricas, algunos valores funcionan en casi todos los sistemas soportados por Perl: 0 significa solo lectura, 1 significa sólo escritura y 2 significa lectura y escritura.  Sabemos que estos valores I<no> funcionan bajo OS/390 y VM/ESA Unix y en el Macintosh; es probable que no desea utilizarlos en nuevos programas.

Si el archivo con el nombre FICHERO no existe y la llamada C<open> lo crea (por lo general porque el modo incluye la bandera C<O_CREAT>), entonces el valor de PERMISOS especifica los permisos del archivo recién creado.  Si se omite el argumento PERMISOS a C<sysopen>, Perl utiliza el valor octal C<0666>.
...
En muchos sistemas Unix, fdopen() se conoce que falla cuando los descriptores de archivo exceden un valor determinado, por lo general 255. Si necesita más descriptores de archivo que esa cantidad, contemple la posibilidad de reconstruir Perl para que use la biblioteca C<sfio>, o tal vez usar la función estándar POSIX::open().

Ver L<perlopentut> para una más amable, más suave explicación de apertura de los archivos.

=item sysread GESTOR,ESCALAR,LONGITUD,DESPLAZAMIENTO
X<sysread>

=item sysread GESTOR,ESCALAR,LONGITUD

Intenta leer LONGITUD bytes de datos en la variable ESCALAR desde el GESTOR especificado, utilizando read(2).  Omite E/S con buffer, por lo que mezclar esto con otros tipos de lecturas, C<print>, C<write>, C<seek>, C<tell>, o C<eof> puede causar confusión porque las capas Perlio o stdio por lo general usan búferes de datos.  Devuelve el número de bytes realmente leídos, C<0> al final del archivo, o undef si hubo un error (en este último caso, también se establece C<$!>).  ESCALAR crecerá o se reducirá de manera que el último byte leído sea el último byte del escalar después de la lectura.

Un DESPLAZAMIENTO puede ser especificado para colocar los datos leídos en algún otro lugar de la cadena distinto del principio.  Un DESPLAZAMIENTO negativo especifica el desplazamiento en número de caracteres contando desde el final de la cadena.  Un DESPLAZAMIENTO positivo mayor que la longitud de ESCALAR resulta en un cadena rellenada al tamaño requerido con bytes C<"\0"> antes de que el resultado de la lectura sea añadido.

No hay función syseof(), que está bien, ya que, de todos modos, eof() no funciona bien con los ficheros de dispositivo (como los ttys).  Use sysread() y compruebe si el valor devuelto es 0 para decidir si ha llegado al final.

Tenga en cuenta que si el gestor de archivo ha sido marcado como  C<:utf8>, son leídos caracteres Unicode en lugar de bytes (la LONGITUD, el DESPLAZAMIENTO y el valor de retorno de sysread() están descritos en forma de número de caracteres Unicode).
La capa C<:encoding(...)> implícitamente introduce la capa C<:utf8>.
Ver L</binmode>, L</open>, y el pragma C<open>, L<open>.

=item sysseek GESTOR,POSICIÓN,DESDEDÓNDE
X<sysseek> X<lseek>

Establece la posición del GESTOR en bytes usando lseek(2).  GESTOR puede ser una expresión cuyo valor da el nombre del gestor de archivo.  Los valores de DESDEDÓNDE son C<0> para establecer la nueva posición a POSICIÓN, C<1> para establecer la posición actual más POSICIÓN, y C<2> para establecerlo a EOF más POSICIÓN (normalmente negativo).

Nota sobre lo de I<en bytes>: incluso si el gestor de fichero se ha puesto para operar en caracteres (por ejemplo usando la capa de E/S C<:encoding(utf8)>), tell() devolverá desplazamientos en bytes, no en caracteres (porque implementando esto convertirían en inaceptablemente lentas a seek() y tell()).

sysseek() omite E/S buffer normal, por lo que mezclar esto con lecturas (que no sean C<sysread>, por ejemplo, C<< <> >> o read()) C<print>, C<write>, C<seek>, C<tell>, o C<eof> puede causar confusión.

Para DESDEDÓNDE puede usar las constantes C<SEEK_SET>,
C<SEEK_CUR>, y C<SEEK_END> (inicio de fichero, posición actual, y fin de fichero) desde el módulo Fcntl.  El uso de las constantes también es más portátil que confiar en 0, 1 y 2.  Por ejemplo, para definir una función "systell":

    use Fcntl 'SEEK_CUR';
    sub systell { sysseek($_[0], 0, SEEK_CUR) }

Devuelve la nueva posición, o el valor indefinido en caso de fallo.  Una posición de cero es devuelto como cadena C<"0 but true"> (I<"0 pero verdadero">), por lo que C<sysseek> devuelve verdadero o falso en caso de fallo, sin embargo, usted puede determinar fácilmente la nueva posición.
...
El valor devuelto es el estado de salida del programa devuelto por la llamada C<wait>.  Para obtener el valor real de la salida, se debe desplazar ocho a la derecha (ver más abajo). Ver también L</exec>.  Esto I<no> es lo que desea utilizar para capturar la salida de un comando; para eso debe utilizar solamente las comillas invertidas o C<qx//>, como se describe en L<perlop/"`CADENA`">.  Un valor devuelto de -1 indica un fallo en el arranque del programa o un error de la llamada al sistema wait(2) (inspeccionar $! para ver la razón).
...
Al igual que C<exec>, C<system> permite reposar a un programa bajo un nombre, si se utiliza la sintaxis C<system PROGRAMA LISTA>.  Una vez más, véase L</exec>.
...
Si desea inspeccionar manualmente el fallo de C<system>, puede comprobar todas las posibles causas del fallo mediante la inspección de C<$?> como esto:
...
=item syswrite GESTOR,ESCALAR,LONGITUD,DESPLAZAMIENTO
X<syswrite>

=item syswrite GESTOR,ESCALAR,LONGITUD

=item syswrite GESTOR,ESCALAR

Intenta escribir LONGITUD bytes de datos desde la variable ESCALAR al GESTOR especificado, utilizando write(2).  Si LONGITUD no se especifica, escribe todo el ESCALAR.  Omite E/S con buffer, por lo que mezclar esto con otros tipos de lecturas, C<print>, C<write>, C<seek>, C<tell>, o C<eof> puede causar confusión porque las capas Perlio o stdio por lo general usan búferes de datos.  Devuelve el número de bytes realmente escritos, o C<undef> si hubo un error (en este caso, también se establece la variable C<$!>).  Si LONGITUD es mayor que los datos disponibles en ESCALAR después del DESPLAZAMIENTO, sólo se escribirán los datos que estén disponibles.
...
B<Advertencia>: Si el gestor está marcado como C<:utf8>, caracteres Unicode codificados en UTF-8 son escritos en lugar de bytes, y la LONGITUD, DESPLAZAMIENTO y el valor de retorno de syswrite() estarán codificados en caracteres (en Unicode UTF-8).
La capa C<:encoding(...)> implícitamente introduce la capa C<:utf8>.
Ver L</binmode>, L</open>, y el pragma C<open>, L<open>.
...
Devuelve la posición actual I<en bytes> para GESTOR, o -1 en caso de error.  GESTOR_FICHERO puede ser una expresión cuyo valor da el nombre del gestor de archivo real.  Si GESTOR se omite, se asume que es el último fichero leído.
...
No utilice tell() (u otras operaciones de E/S con buffer) en un gestor de archivo que ha sido manipulado por sysread(), syswrite() o sysseek().
Esas funciones ignoran el sistema de buffer, mientras que tell(), no.

=item telldir GESTOR_DIR
X<telldir>

Devuelve la posición actual de la rutina C<readdir> sobre GESTOR_DIR.
...
Esta función une una variable a una clase de paquete que ofrecerá la implementación para esa variable.  VARIABLE es el nombre de la variable que será enlazada.  CLASE es el nombre de la clase que implementa objetos del tipo correcto.  Cualesquiera argumentos adicionales son pasados al método C<new> de la clase (es decir, C<TIESCALAR>, C<TIEHANDLE>, C<TIEARRAY>, o C<TIEHASH>).  Por lo general estos son argumentos que pudieran ser transmitidos a la función del C C<dbm_open()>. El objeto devuelto por el método C<new> también es devuelto por la función C<tie>, que será útil si desea tener acceso a otros métodos en CLASE.

Note que las funciones como C<keys> y C<values> pueden devolver largas listas como cuando se usan con grandes ficheros DBM.  Puede preferir usar la función C<each> para iterar sobre tales listas.  Ejemplo:

    # imprime los desplazamientos históricos
    use NDBM_File;
...
Una clase que implementa un gestor de archivo debe tener los siguientes métodos:
...
A diferencia de C<dbmopen>, la función C<tie> no usará C<use> o C<require> de un módulo por usted; necesitará hacerlo de forma explícita.  Ver L<DB_File> o el módulo F<Config> para interesantes implementaciones de C<tie>.
...
Devuelve una referencia al objeto subyacente en VARIABLE (el mismo valor que fue devuelto originalmente por la llamada C<tie> que unía a la variable a un paquete). Devuelve el valor indefinido si la VARIABLE no está vinculada a un paquete.
...
Para medir el tiempo con una granularidad mayor de un segundo, puede usar el módulo L<Time::HiRes> (de CPAN, y a partir de Perl 5.8, como parte de la distribución estándar), o si tiene gettimeofday(2), puede ser capaz de utilizar el interfaz C<syscall> de Perl.
Ver L<perlfaq8> para más detalles.
...
Devuelve una lista de cuatro elementos, dando los tiempos de usuario y sistema, en segundos, para este proceso y los hijos de este proceso.
...
El operador de transliteración.  Igual que el C<y///>.  Ver L<perlop/"Operadores entrecomillados">.

=item truncate GESTOR,LONGITUD
X<truncate>

=item truncate EXPR,LONGITUD

Trunca el archivo abierto en GESTOR, o nombrado por EXPR, a la longitud especificada.  Lanza una excepción si truncate no está implementada en el sistema.  Devuelve verdadero si tiene éxito, sino, el valor indefinido.

El comportamiento no está definido si LONGITUD es mayor que la longitud del archivo.

La posición en el fichero de GESTOR no cambia.  Puede llamar a L<seek> antes de escribir en el archivo.
...
Devuelve una versión de EXPR, en mayúsculas.  Esta es la función interna del escapado C<\U> en las cadenas doblemente entrecomilladas.  Respeta la actual configuración regional LC_CTYPE si C<use locale> está en vigor.  Ver L<perllocale> y L<perlunicode> para más detalles sobre la configuración regional y el soporte de Unicode.
No intentará hacer un mapeo de mayúsculas con tilder en las letras iniciales.  Ver C<ucfirst> para eso.

Si se omite EXPR, se usa C<$_>.

=item ucfirst EXPR
X<ucfirst> X<uppercase> X<mayúsculas>

=item ucfirst

Devuelve el valor de EXPR con el primer carácter en mayúsculas.  Esta es la función interna del escapado C<\u> en las cadenas doblemente entrecomilladas.  Respeta la actual configuración regional LC_CTYPE si C<use locale> está en vigor.  Ver L<perllocale> y L<perlunicode> para más detalles sobre la configuración regional y el soporte de Unicode.
...
Los permisos Unix C<rwxr-x---> son representados como tres conjuntos de tres bits, o tres dígitos octales: C<0750> (el 0 precedente indica que es octal, y que no es uno de los dígitos).  El valor de C<umask> es un número que representa los bits de lso permisos que quedan desactivados.  El valor de permiso (o "modo") que se pasa a C<mkdir> o C<sysopen> son modificados por su umask, así que incluso si usted dice C<sysopen> para crear un archivo con permisos de C<0777>, y su umask es C<0022>, entonces el fichero realmente es creado con permisos C<0755>.  Si su C<umask> fuera C<0027> (el grupo no puede escribir, los otros no pueden leer, escribir o ejecutar), entonces pasando a C<sysopen> C<0666> creará un archivo con el modo C<0640> (C<0666 &~ 027> es C<0640>).

He aquí algunos consejos: suministre un modo de creación de C<0666> para ficheros regulares (en C<sysopen>) y uno de C<0777> para directorios (en C<mkdir>) y los archivos ejecutables.  Esto ofrece a los usuarios la libertad de elección: si quieren archivos protegidos, podrían elegir un umasks para el proceso de C<022>, C<027>, o incluso la máscara, particularmente antisocial, de C<077>.
Los programas rara vez o nunca deben tomar decisiones políticas, que es mejor dejar al usuario.  La excepción a esto es cuando se escriben archivos que deben mantenerse de forma privada: archivos de correo, los cookies del navegador web, ficheros I<rhosts>, y así sucesivamente.
...
=item undef EXPR
X<undef> X<undefine> X<indefinido>

=item undef

Indefine el valor de EXPR, que debe ser un valor izquierdoso.  Use solamente sobre un valor escalar, un array (usando C<@>), un hash (con C<%>), una subrutina (con C<&>), o un typeglob (usando C<*>).  Decir C<undef $hash{$key}> probablemente no vaya a hacer lo que usted espera en la mayoría de las variables predefinidas o lista de valores DBM, así que no haga eso; vea L<delete>.  Siempre devuelve el valor indefinido.  Puede omitir la EXPR, en tal caso, nada queda indefinido, pero aún obtiene un valor indefinido que podría, por ejemplo, devolver de una subrutina, asignar a una variable, o pasar como un parámetro.  Ejemplos:
...
=item unlink LISTA
X<unlink> X<delete> X<remove> X<rm> X<del> X<borrar>

=item unlink

Elimina una lista de archivos. En caso de éxito, devuelve el número de archivos que se han eliminado correctamente. En caso de fallo, se devuelve falso y establece C<$!> (errno):
...
Rompe el enlace entre una variable y un paquete.  (Ver C<tie>). No tiene efecto si la variable no está enlazada.
...
Además, si la versión especificada de Perl es mayor o igual a 5.9.5, C<use VERSIÓN> cargará el pragma C<feature> y activará todas las características disponibles en la versión solicitada.  Ver L<feature>.
Del mismo modo, si la versión especificada de Perl es mayor o igual a 5.11.0, el modo de trabajo estricto quedará activado léxicamente como con C<use strict> (excepto que el archivo F<strict.pm>, realmente, no se cargará).

El C<BEGIN> fuerza a ejecutar a C<require> e C<import> en tiempo de compilación.  El C<require> se asegura que el módulo esté cargado en memoria, si todavía no lo está.  El C<import> no es una orden interna; es sólo una llamada al método estático ordinario en el paquete C<Módulo> para indicar al módulo que importe la lista de características en el paquete actual.  El módulo puede implementar su propio método C<import> de la forma que desee, aunque la mayoría de los módulos sólo eligen derivar su método C<import> vía herencia desde la clase C<Exporter> que se define en el módulo C<Exporter>.  Ver L<Exporter>.  Si no se puede encontrar ningún método C<import> entonces la llamada se pasa por alto, incluso si hay un método AUTOLOAD.
...
Cambia los tiempos de acceso y modificación de cada archivo de una lista de archivos.  Los dos primeros elementos de la lista deben ser el tiempo de acceso y modificación, NUMÉRICOS, y en ese orden.  Devuelve el número de ficheros cambiados con éxito.  El tiempo de cambio de inodo de cada archivo se establece en la hora actual.  Por ejemplo, este código tiene el mismo efecto que el comando touch(1) de Unix cuando los archivos <realmente existen> y pertenecen al usuario que ejecuta el programa:

    #!/usr/bin/perl
    $atime = $mtime = time;
    utime $atime, $mtime, @ARGV;

Desde Perl 5.7.2, si los dos primeros elementos de la lista son C<undef>, se llama a la librería del sistema C utime(2) con un segundo argumento nulo. En la mayoría de los sistemas, esto hará que los tiempos de acceso y modificación del archivo se establezcan a la hora actual (es decir, equivalente al ejemplo anterior) y funcionará incluso en archivos de los cuales usted no tenga permisos de escritura:

    for $fichero (@ARGV) {
	utime(undef, undef, $fichero) 
	    || warn "no puedo tocar $fichero: $!";
    } 
...
En sistemas que soporten futimes(2), puede pasar gestores de archivo entre los archivos.  En sistemas que no admiten futimes(2), pasarle un gestor de archivo lanza una excepción.  Gestores de archivo se deben pasar como comodines o referencias a comodines para que sean reconocidos; las palabras sueltas se consideran como nombres de archivo.
...
Devuelve una lista consistente de todas las claves del hash indicado, o los índices de un array. (En contexto escalar, devuelve el número de valores).

Los valores son devueltos en un aparente orden aleatorio.  El actual orden aleatorio está sujeto a cambio en futuras versiones de Perl, pero se garantiza que sea el mismo orden que las funciones C<keys> o C<each> producirían (mientras que el hash no haya sido modificado).  Desde Perl 5.8.1 la ordenación es diferente incluso entre diferentes ejecuciones de Perl por razones de seguridad (ver L<perlsec/"Ataques por Complejidad Algorítmica">).

Como efecto colateral, llamando a values() resetea el iterador interno del HASH o ARRAY (ver L</each>). (De forma particular, llamando a values() en contexto nulo (void) resetea el iterador sin otro efecto. Además de restablecer el iterador, C<values @array> en contexto lista es la mismo que un simple C<@array>.
Le recomendamos que utilice el contexto vacío C<keys @array> para esto, pero razonando que hacer C<values @array> exigirá más comentarios que dejarlo tal cual)


Tenga en cuenta que los valores no son copiados, lo que significa que modificarles modificará el contenido del hash:
...
X<vec> X<bit> X<bit vector> X<vectores bit>

Se usa para tratar la cadena en EXPR como un vector de bits compuesta de elementos de tantos BITS de ancho, y devuelve el valor del elemento especificado por el DESPLAZAMIENTO como un entero sin signo.  BITS, por tanto, especifica el número de bits que son reservados para cada elemento en el vector de bits.  Esto debe ser una potencia de dos desde 1 a 32 (o 64, si la plataforma lo admite).
...
Ver L<perlop/"Operadores binarios"> .
...
Este es un ejemplo para ilustrar cómo los bits están amparados efectivamente en su lugar:
    ...
    for $w (0..3) {
        ...
        for ($shift=0; $shift < $ancho; ++$shift) {
            for ($despl=0; $despl < 32/$ancho; ++$despl) {
                $cadena = pack("B*", "0"x32);
                $bits = (1<<$shift);
                vec($cadena, $despl, $ancho) = $bits;
                $res = unpack("b*",$cadena);
                $val = unpack("V", $cadena);
                write;
            }
...
El estado se devuelve en C<$?> y C<${^CHILD_ERROR_NATIVE}>.  Si usted dice
...
=item warn LISTA
X<warn> X<warning> X<STDERR> X<advertencia>
...
Ver L<perlvar> para más detalles sobre la configuración de las entradas C<%SIG>, y para más ejemplos.  Véase el módulo Carp para otros tipos de avisos mediante sus funciones carp() y cluck().
...
Escribe un registro formateado (posiblemente de varias líneas) al GESTOR especificado, utilizando el formato asociado con ese archivo.  Por defecto el formato de un archivo es el que tiene el mismo nombre que el gestor de archivo, pero el formato para el canal de salida (véase la función C<select>) se puede establecer de forma explícita mediante la asignación del nombre del formato a la variable C<$~>.

El principio de procesamiento de formularios se maneja de forma automática: si no hay suficiente espacio en la página actual para el registro formateado, la página se avanza escribiendo un avance de página, un formato especial de principio de página se utiliza para dar formato al encabezado de la página nueva, y entonces el registro se escribe.
Por defecto, el formato de principio de página es el nombre del gestor de archivo con "_TOP" añadido al final, pero puede ser configurada de forma dinámica con el formato de su elección mediante la asignación del nombre a la variable C<$^> mientras que el gestor de archivo esté seleccionado.  El número de líneas restantes de la página actual está en la variable C<$->, que se puede fijar a C<0> para forzar una nueva página.

Si GESTOR no está especificado, la salida va al canal predeterminado de salida actual, que suele ser STDOUT, pero puede ser modificado por el operador C<select>.  Si el GESTOR es una EXPR, entonces la expresión se evalúa y la cadena resultante se utiliza para buscar el nombre del GESTOR en tiempo de ejecución.  Para más información sobre formatos, véase L<perlform>.

Tenga en cuenta que write I<no> es lo contrario de C<read>.  Por desgracia.

=item y///

El operador de transliteración.  Igual que C<tr///>.  Ver L<perlop/"Operadores entrecomillados">.
