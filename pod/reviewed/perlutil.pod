...
Al redactar documentación POD puede usar la utilidad F<podchecker>
para detectar errores de marcas POD.

=item L<splain|splain>

F<splain> es una interfaz de L<perldiag>; si pega un mensaje de error, mostrará una descripción.
...
Perl incluye un perfilador, el módulo F<Devel::DProf>. La utilidad F<dprofpp> analiza los resultados del perfilador e indica cuáles son las subrutinas que consumen más tiempo. Consulte L<Devel::DProf> para obtener más información.

=item L<prove>

F<prove> es una interfaz de línea de comandos de la funcionalidad de ejecución de tests de F<Test::Harness>.  Es una alternativa a C<make test>.
...
F<cpan2dist> es una herramienta que permite crear distribuciones (o paquetes) compatibles con distintos administradores de paquetes a partir de módulos de CPAN. El código de compatibilidad con los distintos formatos específicos se encuentra en los módulos C<CPANPLUS::Dist::*> de CPAN.
