...
Véase también L<"Términos y Operadores de listas (por la izquierda)">.
...
(Véase también L<Aritmética entera> y L<Operadores de bit en cadenas>).
...
Sólo se puede utilizar el blablablá en lugar de una instrucción completa.
...
El blablablá no puede sustituir a una expresión que forma parte de una instrucción más grande debido a que C<...> es también la versión de tres puntos del operador rango (véase L<Operadores de rango>). Estos ejemplos de blablablá siguen siendo errores de sintaxis:
...
Véase también la discusión de los Operadores de lista en L<Términos y Operadores de listas (por la izquierda)>.
...
Si no hay dígitos válidos entre las llaves, el carácter generado es el carácter NULL (C<\x{00}>).  Sin embargo, una explícita pareja de llaves vacía (C<\x{}>) no provocará ninguna advertencia.
...
Solo están permitidos dígitos hexadecimales si van a continuación de C<\x>.  Cuando C<\x> esté seguido por menos de dos dígitos válidos, los dígitos válidos serán rellenados con ceros.  Esto significa que C<\x7> será interpretado como C<\x07> y C<\x> será interpretado como un C<\x00>.  Salvo en el final de una cadena, con menos de dos dígitos válidos, dará lugar a una advertencia.  Tenga en cuenta que, si bien la advertencia dice que el carácter ilegal no se tiene en cuenta, sólo es ignorado como parte del escapado y aún así es utilizado como el carácter siguiente en la cadena.
Por ejemplo:
...
El resultado es el carácter especificado por el número octal de tres dígitos en el rango 000 a 777 (pero es mejor no superar el 077, ver siguiente párrafo).  Véase L</[8]> más abajo, para detalles sobre este carácter.

Algunos contextos permiten dos o incluso un dígito, pero cualquier otro uso, sin exactamente tres dígitos, siendo el primero un cero, puede dar resultados inesperados.  (Por ejemplo, véase L<perlrebackslash/Secuencias de escape octales>.) A partir de Perl v5.14, puede utilizar en su lugar C<\o{}> toda vez que evita todos estos problemas.  De lo contrario, es mejor utilizar esta construcción sólo para los ordinales C<\077> e inferiores, recordando que hay que rellenar a la izquierda con ceros, para tener los tres dígitos.  Para ordinales más grandes, puede usar C<\o{}>, o convertirlo a otra cosa, como por ejemplo, a hexadecimal y usar C<\x{}> en su lugar.
...
Si se usa Unicode (por ejemplo, C<\N{}> o códigos de punto de 0x100 o superiores), el mapeo de tamaño de caja usado por C<\l>, C<\L>, C<\u> y C<\U> está definido por Unicode.
...
Si un patrón precompilado se incrusta en un patrón más grande entonces los efectos de 'msixpdual' serán propagados apropiadamente.  El efecto del modificador 'o' no es propagado, se limita a los patrones que lo utilicen de forma explícita.
...
=item m?PATRÓN?
X<?> X<operator, match-once> X<operador, coincidir una vez>
...
Por razones históricas, la C<m> precedente en C<m?PATRÓN?> es opcional, pero la sintaxis resultante <?PATRÓN?> es obsoleta, que generará una advertencia sobre su uso y que puede ser removida de una próxima versión estable de Perl sin previo aviso.
...
Si la opción C</r> (no destructiva) es utilizada, entonces se llevará a cabo la sustitución de una copia de la cadena y en lugar de devolver el número de sustituciones, devuelve la copia, haya o no ocurrido una sustitución. En este caso, la cadena original siempre se mantendrá sin variación. La copia siempre será una cadena normal, incluso si la entrada es un objeto o una variable ligada.

Si no se especifica una cadena a través de los operadores C<=~> o C<!~>, se utiliza la variable C<$_> para la búsqueda y la modificación.  (La cadena especificada con C<=~> debe ser una variable escalar, un elemento de un array, un elemento de un hash, o una asignación a una de éstos, es decir, un I<ivalor>).
...
Por ejemplo:
    ...
    open SPLAT, "stuff"    or die "no puedo abrir stuff: $!";
    open STDIN, "<&SPLAT"  or die "no puedo duplicar SPLAT: $!";
    print STDOUT `sort`;
...
Translitera todas las apariciones de los caracteres que se encuentran en la lista de búsqueda con el carácter correspondiente en la lista de reemplazo.  Devuelve el número de caracteres sustituidos o eliminados.  Si no se especifica una cadena vía los operadores C<=~> o C<!~>, se translitera la cadena $_.  (La cadena especificada con C<=~> debe ser una variable escalar, un elemento de un array, un elemento de un hash, o una asignación a una de éstos, es decir, un I<ivalor>).

Si la opción C</r> (no destructiva) es utilizada, entonces se llevará a cabo el reemplazo de una copia de la cadena y devuelve la copia, haya o no ocurrido una modificación. En este caso, la cadena original siempre se mantendrá sin variación. La copia siempre será una cadena normal, incluso si la entrada es un objeto o una variable ligada.
...
Ejemplos:
    ...
    $HOST = $host =~ tr/a-z/A-Z/r    # encadenado con s///
                  =~ s/:/ -p/r;
...
Si un <GESTOR_ARCHIVO> es utilizado en un contexto en el que está esperando una lista, es devuelta una lista que contenga todas las líneas de entrada, una línea por cada elemento de la lista.  De esta manera, es fácil que el espacio ocupado por los datos crezca a un gran tamaño, así que tenga cuidado.
...
puede indicar al compilador que utilice operaciones con enteros (véase L<integer> para una explicación más detallada) desde aquí hasta el final del BLOQUE actual.  Un BLOQUE interno puede revocar esto diciendo
