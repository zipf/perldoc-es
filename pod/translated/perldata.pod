...
Debido a una rareza histórica, si no se está bajo C<use utf8>, los 128 controles extras en el rango C<[0x80-0xff]> también se pueden utilizar en variables de un solo carácter.  Es obsoleto el uso de un carácter de control literal.  El soporte de esto se eliminará en una futura versión de perl.

=item De forma similar a lo anterior, un sigilo, seguido por un texto simple, entre llaves, donde el primer carácter es tanto un circunflejo seguido por letra mayúscula, o un carácter de control literal, como C<${^GLOBAL_PHASE}> o C<${\7LOBAL_PHASE}>.  Es obsoleto el uso de un carácter de control literal.  El soporte de esto se eliminará en una futura versión de perl.

=item Un sigilo seguido de un único carácter coincidente con la propiedad C<\p{POSIX_Punct}>, como C<$!> o C<%+>.

=back

Note que a partir de Perl 5.20, es obsoleto usar caracteres de control literales en nombres de variables.
...
De forma más general, una porción produce una lista vacía sólo si se indexa más allá del final de la lista.

    @a = (1)[  1,2];        # @a no tiene elementos
    @b = (1)[0,1,2];        # @b tiene tres elementos
