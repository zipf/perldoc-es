...
Se usa el carácter "@" para indicar que una variable es un array completo (o una porción de array o hash). Funciona de manera parecida a las palabras "estos/estos" o "estas/esas" del español, que indican que se esperan múltiples valores.
X<array>
...
También debe hacer esto al interpolar una variable en una cadena para separar el nombre de una variable de un signo de dos puntos doble o un apóstrofo, a fin de evitar que se traten como separadores de paquete:
X<interpolation> X<interpolación>
...
Es decir, se debe usar C<*ARCHIVO> para crear nuevas entradas de en la tabla de símbolos; no se puede hacer con C<*foo{COSA}>.  En caso de duda, use C<*ARCHIVO>.
