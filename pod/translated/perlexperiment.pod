...
En este documento se muestran las funcionalidades experimentales actuales y anteriores del núcleo de perl. Aunque estas funcionalidades están documentadas en los temás correspondientes de la documentación, esta lista abreviada presenta una introducción y datos básicos sobre su estado de desarrollo actual.
...
C<Getopt::Long> actualizado a la versión 2.35
...
La clase de caracteres C<\N>, que no debe confundirse con la secuencia de caracteres con nombre C<\N{NAME}>, se usa en en una expresiones regulares para denotar cualquier carácter que no sea de nueva línea.
...
También les hemos concedido 5 por estabilidad y 3 por carisma.
...
Estas funcionalidades ya no se consideran experimental y se han suprimido. Si después de que le indicaramos explícitamente que estas funcionalidades no se deben usar en programas de producción decidió hacerlo, la responsabilidad es suya (vea L<perlpolicy>).
