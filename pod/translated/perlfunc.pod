...
=item Funciones para controladores de archivo, ficheros y directorios
X<file> X<filehandle> X<directory> X<pipe> X<link> X<symlink> X<fichero> X<controlador de archivo> X<enlace>
...
=item binmode GESTOR_FICHERO, CAPA
X<binmode> X<binary> X<text> X<DOS> X<Windows> X<binario> X<texto>

=item binmode GESTOR_FICHERO

Ordena que el modo de lectura o escritura para el GESTOR_FICHERO sea en "binario" o "texto" en sistemas donde las bibliotecas en tiempo de ejecución distingan entre ficheros binarios y de texto.  Si GESTOR_FICHERO es una expresión, el valor es tomado como el nombre del gestor.  Devuelve verdadero en éxito; de lo contrario devuelve C<undef> y pone C<$!> (errno).
...
Para indicar GESTOR_FICHERO como UTF-8, use C<:utf8> o C<:encoding(utf8)>.
...
Este identificador se activa con la característica C<"switch">: vea L<feature> para más información
...
No tiene porqué cerrar el GESTOR_FICHERO si inmediatamente va a hacer otro C<open> sobre él mismo, porque C<open> lo cierra por usted.  (Ver C<open>). Sin embargo, un C<close> explícito en un fichero de entrada reinicia el contador de líneas (C<$.>), mientras que un C<close> implícito hecho por C<open>, no.
...
GESTOR_FICHERO puede ser una expresión cuyo valor puede ser usado como un gestor indirecto, normalmente el nombre real del gestor.
...
C<die> lanza una excepción. Dentro de un C<eval()>, el mensaje de error es colocado en C<$@> y el C<eval> termina con el valor I<undef> (indefinido).
Si la excepción está fuera de un C<eval>, entonces la excepción no capturada imprime LISTA al C<STDERR> y sale con un valor no nulo. Si necesita terminar el proceso con un valor de salida específico, véase L<exit>.
...
=item eof GESTOR_FICHERO
X<eof> X<end of file> X<end-of-file> X<fin de fichero>

=item eof ()

=item eof

Devuelve 1 si la siguiente lectura sobre GESTOR_FICHERO devolverá un fin de fichero, o si GESTOR_FICHERO no está abierto.  GESTOR_FICHERO puede ser una expresión cuyo valor da el gestor real.  (Note que esta función realmente lee un carácter y luego hace un C<ungetc>, así que no es útil en un contexto interactivo). No lea de un fichero del que se ha llegado al final (o vuelva a llamar a C<eof(GESTOR_FICHERO)> sobre él) después de haber alcanzado un fin-de-fichero.  Tipos de ficheros como por ejemplo terminales pueden perder la condición de fin-de-fichero si lo hace.
...
A partir de la versión v5.6.0, Perl intenta vaciar todos los ficheros abiertos para salida antes de ejecutar exec, pero esto puede no estar soportado en algunas plataformas (ver L<perlport>).  Para estar seguros, puede necesitar poner C<$|> ($AUTOFLUSH en el módulo English) o llamar el método C<autoflush()> de C<IO::Handle> en cualquiera de los gestores de archivo abiertos para evitar la pérdida de datos en la salida.
...
=item fcntl GESTOR_FICHERO,FUNCIÓN,ESCALAR
X<fcntl>
...
Si el GESTOR_FICHERO es una expresión, el valor es tomado como un gestor indirecto, generalmente su nombre.
...
=item flock GESTOR_FICHERO,OPERACIÓN
X<flock> X<lock> X<locking>

Llama a flock(2), o una emulación de ella, sobre GESTOR_FICHERO.  Devuelve verdadero para éxito, falso en fallo.  Produce un error fatal si se usa en una máquina que no implemente flock(2), bloqueo fcntl(2), o lockf(3).
...
Para evitar la posibilidad de descoordinación, Perl ahora vacía el GESTOR_FICHERO antes de bloquearlo o desbloquearlo.

Note que la emulación integrada con lockf(3) no provee de bloqueos compartidos y requiere que el GESTOR_FICHERO sea abierto con intención de escribir en el.  Estas son las semánticas que lockf(3) implementa.  La mayor parte si no todos los sistemas implementan lockf(3) en términos de bloqueo con fcntl(2), aunque, las diferencias en la semántica no llame la atención a mucha gente.

Note que la emulación de fcntl(2) de flock(3) requiere que GESTOR_FICHERO sea abierto con intención de leer para usar LOCK_SH y requiere ser abierto con intención de escribir para usar LOCK_EX.
...
=item getc GESTOR_FICHERO
X<getc> X<getchar> X<character> X<file, read> X<fichero, lectura> X<carácter>

=item getc

Devuelve el siguiente carácter desde el fichero de entrada asociado a GESTOR_FICHERO, o el valor indefinido al final del fichero, o si ocurrió un error (en este último caso, se actualiza C<$!>).  Si GESTOR_FICHERO se omite, se lee desde STDIN.  Esto no es particularmente eficiente.  Sin embargo, esto no puede ser usado para extraer caracteres sueltos sin esperar a que el usuario pulse la tecla de I<Enter>.  Para esto, intente algo como esto:
...
=item glob EXPR
X<glob> X<wildcard> X<filename, expansion> X<expand> X<fichero, expansión> X<comodín> X<expandir>
...
=item ioctl GESTOR_FICHERO,FUNCIÓN,ESCALAR
X<ioctl>
...
Los resultados son los que devuelve la llamada del sistema en lenguaje C C<tolower()>.

=item En plataformas ASCII

Los resultados siguen la semántica ASCII.  Sólo los caracteres C<A-Z> cambian, a C<a-z>, respectivamente.

=back

=item De lo contrario, si EXPR tiene la bandera UTF8 puesta

Si el paquete actual tiene un subrutina denominada C<ToLower>, será utilizada para cambiar el tamaño de caja (ver L<perlunicode/Mapeos de caja definidos por el usuario>). De lo contrario se utiliza semántica Unicode para el cambio de tamaño de caja.
...
Semántica Unicode es utilizada para el cambio de tamaño de caja.  Cualquier subrutina denominada C<ToLower> no será usada.
...
Semántica ASCII es utilizada para el cambio de tamaño de caja.  La minúscula de cualquier carácter fuera del rango ASCII es el propio carácter.
...
Esta función se comporta de la misma manera bajo varios I<pragma>, como por ejemplo en una configuración regional, como lo hace L</lc>.
...
=item open GESTOR_FICHERO,EXPR
X<open> X<pipe> X<file, open> X<fopen> X<fichero, open>

=item open GESTOR_FICHERO,MODO,EXPR

=item open GESTOR_FICHERO,MODO,EXPR,LISTA

=item open GESTOR_FICHERO,MODO,REFERENCIA

=item open GESTOR_FICHERO

Abre el archivo cuyo nombre viene dado por EXPR, y lo asocia con GESTOR_FICHERO.
...
Si GESTOR_FICHERO es un variable escalar indefinida (o elemento de un array o hash) la variable es asignada a una referencia a un nuevo gestor de fichero anónimo; de lo contrario, si GESTOR_FICHERO es una expresión, su valor se utiliza como el nombre del gestor de archivo real buscado.  (Esto se considera una referencia simbólica, por lo que C<use strict 'refs'> I<no> debe estar activada).

Si EXPR se omite, la variable escalar del mismo nombre que el GESTOR_FICHERO contiene el nombre del fichero.  (Note que las variables léxicas -las declaradas con C<my>- no servirán para este propósito; así que si está usando C<my>, especifique EXPR en su llamada a open).
...
Puede también, en la tradición de Bourne shell, especificar una EXPR comenzando con C<< '>&' >>, en cuyo caso el resto de la cadena se interpreta como el nombre de un gestor de archivo (o descriptor de fichero, si es numérico) para que sea duplicado (con C<dup(2)> ) y luego abierto.  Puede usar C<&> después de C<< > >>, C<<< >> >>>, C<< < >>, C<< +> >>, C<<< +>> >>>, y C<< +< >>.
...
La PLANTILLA es una secuencia de caracteres que dan el orden y el tipo de los valores, de la siguiente manera:
    ...
    I  Un valor entero sin signo.
      (Este 'entero' es _al_menos_, de 32 bit de ancho.  Su exacto
           tamaño depende de lo que un compilador C local llama 'int').
...
C<x> y C<X> aceptan el modificar C<!> para actuar como comando de alineamiento: saltan hacia adelante o atrás a la posición más cercana alineada a un múltiplo de C<contador> caracteres. Por ejemplo, para pack() o unpack() una estructura en C como
    ...
    struct {
	char   c;    /* un carácter de 8-bit, con signo */
	double d; 
	char   cc[2];
    }
...
quotemeta (y C<\Q> ... C<\E>) son útiles cuando se interpolan cadenas en expresiones regulares, porque por defecto una variable interpolada será considerada una mini expresión regular. Por ejemplo:
    ...
    my $sentencia = 'El rápido zorro marrón saltó sobre el perezoso perro';
    my $subcadena = 'rápido.*?zorro';
    $sentencia =~ s{$subcadena}{gran lobo malo};

Hará que C<$sentencia> se convierta en C<'El gran lobo malo saltó sobre ...'>.

De otra forma:

    my $sentencia = 'El rápido zorro marrón saltó sobre el perezoso perro';
    my $subcadena = 'rápido.*?zorro';
    $sentencia =~ s{\Q$subcadena\E}{gran lobo malo};
...
o:
    ...
    $sentencia =~ s{$subcadena_escapada}{gran lobo malo};

Ambas dejará la sentencia como estaba. Normalmente, deben ser usados quotemeta() o C<\Q> al aceptar cadenas de entrada provenientes del usuario.
...
X<read> X<file, read> X<fichero, leer>
...
El valor de retorno C<LVALUE> indica una referencia a un I<ivalor> (valor que se pone a la izquierda) que no es una variable. Obtiene esto de la referencia a las llamadas a funciones como C<pos()> o C<substr()>. C<VSTRING> se devuelve si la referencia apunta a una L<version string|perldata/"Cadenas de versión">.
...
Establece la posición actual al comienzo del directorio para la rutina C<readdir> en GESTOR_DIR.
...
=item seek GESTOR_FICHERO,POSICIÓN,DESDE
...
puede averiguar si Perl considera "long double" como el valor por defecto para el tamaño del punto flotante para su plataforma a través de L<Config>:
	...
	print "dobles largos por defecto\n";
    }
...
La mayoría de los programas ni siquiera llaman a srand(), excepto los que necesitan un fuerte punto de partida criptográfico en lugar del valor por defecto, que se basa en la hora del día, el ID del proceso, la asignación de memoria, o el dispositivo F</dev/urandom> si está disponible. También puede llamar a srand() después de un fork() para evitar que los procesos hijos compartan la misma semilla de inicialización que con el padre (y por lo tanto entre sí).
...
Si está especialmente preocupado por esto, busque en CPAN por módulos generadores de números aleatorios en lugar de desarrollar el suyo propio.
...
Puede utilizar la función substr() como un I<ivalor>, en cuyo caso la EXPR debe ser en si mismo un I<ivalor>.  Si asigna algo más corto que LONGITUD, la cadena se reducirá, y si asigna algo más largo que LONGITUD, la cadena crecerá para acogerlo.  Para mantener la cadena de la misma longitud, puede ser necesario rellenar o recortar su valor con C<sprintf>.

Si DESPLAZAMIENTO y LONGITUD especifican una subcadena que está parcialmente fuera de la cadena, sólo la parte de dentro de la cadena se devuelve.  Si la subcadena está más allá de cada extremo de la cadena, substr() devuelve el valor indefinido y produce una advertencia.  Cuando se utiliza como un I<ivalor>, especificando una subcadena que está totalmente fuera de la cadena lanza una excepción.
...
Una alternativa al uso substr() como un I<ivalor> es especificar la cadena de reemplazo como el cuarto argumento.  Esto le permite reemplazar partes de la EXPR y devolver lo que había antes, en una operación, tal como se puede hacer con splice().

    my $s = "El gato negro subió el árbol verde";
    my $z = substr $s, 14, 5, "saltó desde";    # subió
    # $s es ahora "El gato negro saltó desde el árbol verde"

Tenga en cuenta que el I<ivalor>> devuelto por la versión de 3 argumentos de substr() actúa como una "bala mágica"; cada vez que se le asigna, recuerda qué parte de la cadena original se ha modificado; por ejemplo:
...
Antes de la versión Perl 5.9.1, no estaba especificado el resultado de usar varias veces un I<ivalor>.
...
=item sysopen GESTOR_FICHERO,FICHERO,MODO
X<sysopen>

=item sysopen GESTOR_FICHERO,FICHERO,MODO,PERMISOS

Abre el archivo cuyo nombre viene dado por FICHERO, y lo asocia con GESTOR_FICHERO.  Si GESTOR_FICHERO es una expresión, el valor es tomado como el nombre real del gestor buscado.  Esta función llama a la función subyacente del sistema operativo C<open> con los parámetros FICHERO, MODO, PERMISOS.
...
=item sysseek GESTOR_FICHERO,POSICIÓN,DESDEDÓNDE
X<sysseek> X<lseek>

Establece la posición del GESTOR_FICHERO en bytes usando lseek(2).  GESTOR_FICHERO puede ser una expresión cuyo valor da el nombre del gestor de archivo.  Los valores de DESDEDÓNDE son C<0> para establecer la nueva posición a POSICIÓN, C<1> para establecer la posición actual más POSICIÓN, y C<2> para establecerlo a EOF más POSICIÓN (normalmente negativo).
...
Debido a que C<system> hace un C<fork> y un C<wait> puede afectar al controlador C<SIGCHLD>. Véase L<perlipc> para más detalles.

=item syswrite GESTOR_FICHERO,ESCALAR,LONGITUD,DESPLAZAMIENTO
X<syswrite>

=item syswrite GESTOR_FICHERO,ESCALAR,LONGITUD

=item syswrite GESTOR_FICHERO,ESCALAR

Intenta escribir LONGITUD bytes de datos desde la variable ESCALAR al GESTOR_FICHERO especificado, utilizando write(2).  Si LONGITUD no se especifica, escribe todo el ESCALAR.  Omite E/S con buffer, por lo que mezclar esto con otros tipos de lecturas, C<print>, C<write>, C<seek>, C<tell>, o C<eof> puede causar confusión porque las capas Perlio o stdio por lo general usan búferes de datos.  Devuelve el número de bytes realmente escritos, o C<undef> si hubo un error (en este caso, también se establece la variable C<$!>).  Si LONGITUD es mayor que los datos disponibles en ESCALAR después del DESPLAZAMIENTO, sólo se escribirán los datos que estén disponibles.
...
=item tell GESTOR_FICHERO
X<tell>

=item tell

Devuelve la posición actual I<en bytes> para GESTOR_FICHERO, o -1 en caso de error.  GESTOR_FICHERO puede ser una expresión cuyo valor da el nombre del gestor de archivo real.  Si GESTOR_FICHERO se omite, se asume que es el último fichero leído.
...
Esta función une una variable a una clase de paquete que ofrecerá la implementación para esa variable.  VARIABLE es el nombre de la variable que será enlazada.  CLASE es el nombre de la clase que implementa objetos del tipo correcto.  Cualesquiera argumentos adicionales son pasados al método C<new> de la clase (es decir, C<TIESCALAR>, C<TIEHANDLE>, C<TIEARRAY>, o C<TIEHASH>).  Por lo general estos son argumentos que podrían ser transmitidos a la función del C C<dbm_open()>.  El objeto devuelto por el método C<new> también es devuelto por la función C<tie>, que será útil si desea acceder a otros métodos en la CLASE.
...
=item truncate GESTOR_FICHERO,LONGITUD
X<truncate>

=item truncate EXPR,LONGITUD

Trunca el archivo abierto en GESTOR_FICHERO, o nombrado por EXPR, a la longitud especificada.  Lanza una excepción si truncate no está implementada en el sistema.  Devuelve verdadero si tiene éxito, sino, el valor indefinido.

El comportamiento no está definido si LONGITUD es mayor que la longitud del archivo.

La posición en el fichero de GESTOR_FICHERO no cambia.  Puede llamar a L<seek> antes de escribir en el archivo.
...
Se debe tener cuidado cuando se usa la forma C<no VERSIÓN> de C<no>.  I<Sólo> es para ser usado para afirmar que el perl que se está ejecutando es de una versión anterior a su argumento y I<no> para deshacer los efectos secundarios activados por C<use VERSION>.
...
Si utiliza wait en su controlador de $SIG{CHLD} puede, accidentalmente hacer esperar por el hijo creado por qx() o system(). Véase L<perlipc> para más detalles.
...
Si GESTOR_FICHERO no está especificado, la salida va al canal predeterminado de salida actual, que suele ser STDOUT, pero puede ser modificado por el operador C<select>.  Si el GESTOR_FICHERO es una EXPR, entonces la expresión se evalúa y la cadena resultante se utiliza para buscar el nombre del GESTOR_FICHERO en tiempo de ejecución.  Para más información sobre formatos, véase L<perlform>.
