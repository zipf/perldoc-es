...
C<evalbytes> sólo está disponible con la característica C<"evalbytes"> (vea L<feature>) o si es prefijada con C<CORE::>.  C<__SUB__> sólo está disponible con la característica C<"current_sub"> o si está prefijada con C<CORE::>. Tanto la característica C<"evalbytes"> como la C<"current_sub"> son activadas de forma automática con una declaración C<use v5.16> (o superior) en el ámbito actual.
...
Esta palabra clave se activa con la característica C<"switch">: vea L<feature> para más información  También puede acceder a ella anteponiendo C<CORE::>.  O de otra forma, incluya un C<use v5.10> o posterior en el contexto actual.
...
En la primera forma, el valor devuelto por EXPR es interpretado y ejecutado como si fuera un pequeño programa Perl.  El valor de la expresión (que es en sí mismo determinada en contexto escalar) es primero interpretada, y si no hay ningún error, ejecutada como un bloque dentro del contexto léxico del actual programa Perl.  Esto significa que, en particular, cualquier otra variable léxica externa es visible dentro de ella, y la configuración de variable de paquete o de subrutinas y las definiciones de formato siguen permaneciendo.

Note que el valor es interpretado cada vez que se ejecuta C<eval>.
...
Si la característica C<unicode_eval> está habilitada (que lo es por defecto con una declaración C<use 5.16> o superior), EXPR o C<$_> es tratado como una cadena de caracteres, así que las declaraciones C<use utf8> no tienen efecto, y los filtros de código fuente quedan prohibidos.  En ausencia de la característica C<unicode_eval>, la cadena a veces será tratada como caracteres y, otras veces como bytes, dependiendo de la codificación interna, y los filtros de código fuente activados dentro de C<eval> muestran el comportamiento errático, pero histórico, comportamiento de afectar a algún archivo externo que todavía se está compilando.  Vea también la palabra clave L</evalbytes>, que siempre trata su entrada como un flujo de bytes y funciona correctamente con filtros de código fuente, y el I<pragma> L<feature>.
...
Si hay algún error de sintaxis o en tiempo de ejecución, o se ejecuta una instrucción C<die>, C<eval> devuelve C<undef> en contexto escalar o una lista vacía en contexto lista, y C<$@> es inicializado al mensaje de error.  (Antes de 5,16, un error causaba que C<undef> era devuelto en contexto de lista para errores de sintaxis, pero no para errores en tiempo de ejecución).
...
Perl sólo implementa la forma completa de plegado de caja.
...
Cerciórese de que C<gethostbyname()> es llamado en contexto ESCALAR y compruebe que su valor de retorno está definido.
...
A diferencia del shell, si SEÑAL es negativo, mata el grupo de procesos en lugar de a los procesos.  Eso significa que deseara utilizar, generalmente, señales positivas, y no negativas.
...
En algunas plataformas como Windows, donde la llamada del sistema fork() no está disponible.
Perl puede ser compilado para emular fork() a nivel del intérprete.
...
Semántica Unicode es utilizada para el cambio de tamaño de caja.
...
El formato de este valor escalar B<no> es dependiente de la configuración regional, sino que viene integrado en Perl.  Para GMT, en lugar de la hora local, use la primitiva L</gmtime>.  Vea también el módulo C<Time::Local> (para convertir los segundos, minutos, horas, etc, de nuevo al valor entero devuelto por time()), y las funciones strftime(3) y mktime(3) del módulo L<POSIX>.
...
La semántica exacta e interfaz de TIPO y ATRIBUTOS están aún en evolución.  TIPO está vinculado al uso del I<pragma> C<fields>, y los atributos se manejan con el I<pragma> C<attributes>, o a partir de Perl 5.8.0 también a través del módulo C<Attribute::Handlers>.  Ver L<perlsub/"Variables privadas vía my()"> para más detalles, y L<fields>, L<attributes>, y L<Attribute::Handlers>.
...
La PLANTILLA es una secuencia de caracteres que dan el orden y el tipo de los valores, de la siguiente manera:
    ...
    Q  Un valor cuádruplo sin signo.
         (Quads están disponibles sólo si su sistema soporta valores
          enteros de 64 bit _y_ si Perl ha sido compilado para
          soportarlos.  En caso contrario, lanza una excepción).

    i  Un valor entero con signo.
    I  Un valor entero sin signo.
         (Este 'entero' es _al_menos_, de 32 bit de ancho.  Su tamaño exacto depende de lo que un compilador C local llame 'int').
    ...
    D  Un flotante en doble-largo precisión en formato nativo.
         (Dobles largos están disponibles sólo si su sistema soporta
          valores doble largos _y_ si Perl ha sido compilado para
          soportarlos.  En caso contrario, lanza una excepción).
...
Uno o más de los modificadores siguientes pueden, opcionalmente, seguir a ciertas letras en la PLANTILLA (la segunda columna enumeran las letras para las cuales el modificador es válido):
    ...
    !   sSlLiI     Fuerza a tamaños nativos (short, long, int) en lugar
        ...
        @.         Especifica una posición como desplazamiento de byte en la
                   representación interna de la cadena empaquetada.  Eficiente
                   pero peligroso.
...
Equivalente a C<print IDENTIFICADOR_ARCHIVO sprintf(FORMATO, LISTA)>, excepto que C<$\> (el separador de registro a la salida) no es añadida.  El primer argumento de la lista será interpretado como el formato para C<printf>.  Vea L<sprintf|/sprintf FORMATO, LISTA> para una explicación del argumento de formato.  Si omite la LISTA, se usará C<$_>; para usar un IDENTIFICADOR_ARCHIVO sin una LISTA, debe usar un identificador de archivo real, como C<FH>, no uno indirecto como C<$fh>.  Si C<use locale> (incluyendo C<use locale ':not_characters'>) está activo y POSIX::setlocale() ha sido llamada, el carácter utilizado para el separador decimal en números formateados en punto flotante se ve afectado por la configuración regional de LC_NUMERIC.  Ver L<perllocale> y L<POSIX>.
...
Devuelve el valor de EXPR con todos los caracteres ASCII que no formen parte de una "palabra" escapados con una barra invertida.  (Es decir, todos los caracteres ASCII que no se ajusten a C</[A-Za-z_0-9]/> serán precedidos por una barra invertida en la cadena devuelta, con independencia de cualquier configuración regional).
...
En Perl 5.14, todos los caracteres no ASCII son escapados en cadenas no codificadas como UTF-8, pero sí son escapadas en cadenas UTF-8.
...
La llamada es implementada en términos de Perl o de la función nativa fread(3) de su sistema.  Para obtener una auténtica llamada del sistema read(2), vea L<sysread|/sysread IDENTIFICADOR_ARCHIVO,ESCALAR,LONGITUD,DESPLAZAMIENTO>.
...
En aplicaciones críticas con el tiempo, no es conveniente dividir en más campos de los que realmente se necesita.  Por lo tanto, cuando se asigna a una lista, si LÍMITE es omitido (o cero), entonces LÍMITE es tratado como si fuera uno mayor que el número de variables en la lista; para lo siguiente, LÍMITE es implícitamente 4:

    ($login, $contraseña, $resto) = split(/:/);
...
Si el PATRÓN contiene L<grupos de captura|perlretut/Agrupando cosas y coincidencia jerárquica>, entonces, por cada separador, se produce un campo adicional para cada subcadena capturada por un grupo (en el orden en que los grupos son especificados, como lo hacen las L<contrareferencias|perlretut/Contrareferencias>); si algún grupo no coincide, entonces captura el valor C<undef> en lugar de una subcadena.  Además, note que se produce un campo adicional cada vez que exista un separador (es decir, cuando se produce una división), y tales campos adicionales B<no> cuentan para el LÍMITE.  Considere las siguientes expresiones evaluadas en contexto de lista (se indica cada lista devuelta en el comentario correspondiente):
...
Para conversiones numéricas, se puede especificar el tamaño del número a interpretar usando C<l>, C<h>, C<V>, C<q>, C<L>, o C<ll>.  Para las conversiones de entero (C<d u o x X b i D U O>), se asume que los números son, por lo general, de un tamaño, por defecto, de un entero de su plataforma (normalmente 32 or 64 bits), pero esto se puede reemplazar usando en su lugar uno de los tipos estándares de C, soportados por el compilador usado para construir Perl:
   ...
   j           interpreta un entero como un tipo C "intmax_t" en Perl 5.14 
...
y las funciones C<S_IF*> son
    ...
    S_IFMT($modo)    la parte de $modo conteniendo los bits del tipo
            de archivo que puede ser bit-I<andeado> con (por ejemplo) S_IFREG
...
Variables C<state> sólo se activan cuando el I<pragma> C<use feature "state"> está en vigor, a menos que el identificador está escrito como C<CORE::state>.
...
Antes de la versión Perl 5.10, no estaba especificado el resultado de usar varias veces un I<ivalor> (valor indicado en la parte izquierda).  Antes de 5,16, el resultado con desplazamientos negativos no estaba especificado.
...
No tiene efecto si la variable no está enlazada.
...
C<use VERSIÓN> también activará todas las características disponibles en la versión solicitada tal como son definidas por el I<pragma> C<feature>, desactivando cualquier característica no incluida en la versión solicitada.  Ver L<feature>.
