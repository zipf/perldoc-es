...
La forma normal de ejecutar un programa de Perl es haciéndolo directamente ejecutable, o bien pasando el nombre del código fuente como un argumento en la línea de comandos.  (Un entorno Perl interactivo también es posible -vea L<perldebug> para más detalles de cómo hacer esto-.) En el arranque, Perl busca por su programa en uno de los siguientes lugares:
...
Contenido en el fichero especificado por el primer nombre de fichero en la línea de comandos.
...
Pasado implícitamente a través de la entrada estándar.  Esto sólo funciona si no hay nombres de fichero como argumentos -para pasar argumentos a un programa que lee desde el STDIN debe, explícitamente, especificar un "-" para el nombre del programa-.

=back

Con los métodos 2 y 3, Perl inicia el análisis del fichero de entrada desde el comienzo, a menos que haya especificado una opción B<-x>, en cuyo caso escanea por la primera línea que comience por C<#!> y contenga la palabra "perl", y arrancará desde allí.  Esto es útil para ejecutar un programa incorporado en un correo grande.  (En este caso se indicará el final del programa, utilizando el I<token> C<__END__>.)
...
Los ejemplos anteriores usan una ruta relativa al intérprete perl, accediendo al primero que se encuentre en la ruta del usuario, sin fijarse en qué versión sea.  Si necesita una versión específica de Perl, por ejemplo, la perl5.005_57, debe colocarla directamente en la ruta de la línea  C<#!>.

Si la línea C<#!> no contiene la palabra "perl", el programa nombrado después del C<#!> es ejecutado en lugar del intérprete de Perl.  Esto es algo extraño, pero ayuda a personas en máquinas que no pueden trabajar con C<#!>, porque ellas pueden indicar a un programa que su SHELL es F</usr/bin/perl>, y Perl, entonces, despachará, por ellos, el programa al intérprete correcto.
...
Ponga
    ...
    extproc perl -S -sus_opciones

como primera línea en el archivo C<*.cmd> (B<-S> si sale un error en el manejo de `extproc' del cmd.exe).

=item MS-DOS

Cree un fichero batch para ejecutar su programa, y codifique en él un C<ALTERNATE_SHEBANG> (vea el archivo F<dosish.h> en el código fuente de su distribución para obtener más información).
...
Cualquier valor igual o superior a 0400 hará que Perl succione todo el fichero, pero, por convención, para este propósito se usa de forma normal el valor 0777.
...
A partir de 5.8.1, el B<-C> puede ser seguido tanto por un número como una lista de letras de opciones.  Estas letras, sus valores numéricos, y efectos, son como sigue; listar las letras es lo mismo que sumar los números.
    ...
    D    24   i + o
    A    32   los elementos @ARGV se espera que sean cadenas codificados
              en UTF-8
    L    64   normalmente, el "IOEioA" es incondicional.
              La L las hace condicionales según las variables de
              entorno de configuración local (LC_ALL, LC_TYPE, y LANG,
              en orden de precedencia decreciente) -- si las variables
              indican UTF-8, entonces toman efecto las opciones "IOEioA"
    a   256   Pone ${^UTF8CACHE} a -1, para ejecutar el código de caché
              UTF-8 en modo depuración.
...
Un solitario B<-C> (no seguido por ningún número ni lista de opciones), o la cadena vacía C<""> para la variable de entorno C<PERL_UNICODE>, tiene el mismo efecto que B<-CSDL>.  En otras palabras, los gestores de archivo estándares de E/S y la capa por defecto para C<open()> están en modo UTF I<pero> solo si las variables de entorno de configuración local indican que se está en un entorno local UTF-8.  Este comportamiento sigue el I<implícito> (y problemático) comportamiento de UTF-8 de Perl 5.8.0.
...
Como alternativa, especifique un número en lugar de una lista de letras (ej., B<-D14> es equivalente a B<-Dtls>):
     ...
     4096  U  No oficial, para hackeo del usuario (reservado para uso privado, y no distribuible)
     8192  H  Volcado de Hash -- usurpa values()
    16384  X  Reserva del Scratchpad
    32768  D  Limpieza
   131072  T  Simbolización
   262144  R  Incluye el contador de referencias de las variables volcadas (ej. cuando se usa -Ds)
   524288  J  muestra s,t,P-debug (no salta afuera) en códigos de operación dentro del paquee DB
  ...
  8388608  q  silencio - actualmente solo suprime el mensaje "EXECUTING"
 16777216  M  traza la resolución de la coincidencia inteligente
 33554432  B  vuelca las definiciones de suBrutinas, incluyendo los Bloques especiales como BEGIN
...
especifica el patrón a split si B<-a> también está en activo.  El patrón puede estar rodeado de C<//>, C<""> o C<''>, o de lo contrario se pondrá entre comillas simples. Puede usar espacio en blanco literal dentro del patrón.
...
Si no se indica ninguna extensión, no se realiza ninguna copia de seguridad y el archivo actual es
sobreescrito.
...
un sufijo:
    ...
    $ perl -pi'orig_*' -e 's/bar/baz/' fileA	# copia de seguridad a 'orig_fileA'
...
O incluso para colocar copias de seguridad de los archivos originales en otro directorio (suponiendo que el directorio ya existe):
    ...
    $ perl -pi'old/*.orig' -e 's/bar/baz/' fileA # copia de seguridad a 'old/fileA.orig'
...
excepto que la opción B<-i> no necesita comparar $ARGV a $oldargv para saber cuándo el nombre de archivo ha cambiado.  Aun así, usa ARGVOUT para el gestor de archivo seleccionado.  Tenga en cuenta que STDOUT es restaurado como el gestor de archivo por defecto, después del bucle.
...
Un poco de endulzamiento sintáctico interno puede ayudarle para decir B<-mI<MÓDULO>=foo,bar> o B<-MI<MÓDULO>=foo,bar> como una forma abreviada de B<'-MI<MÓDULO> qw(foo bar)'>.  Esto evita la necesidad de usar comillas cuando se importan símbolos.  El código real generado por B<-MI<MÓDULO>=foo,bar> es C<use módulo split(/,/,q{foo,bar})>.  Tenga en cuenta que la forma C<=> quita la distinción entre B<-m> y B<-M>.
...
Note que las líneas, por defecto, no son impresas.  Vea B<-p> para tener las líneas impresas.  Si un archivo nombrado por un argumento no puede ser abierto por alguna razón, Perl se lo advierte y pasa al archivo siguiente.
...
Tenga en cuenta que una opción como B<--help> crea la variable C<${-help}>, el cual no es acorde con C<use strict "refs">.  También, cuándo utilice esta opción en un guión con avisos habilitados, puede obtener avisos espurios de la forma "utilizado sólo una vez (I<used only once>)".
...
B<Nota: Esto no es un sustituto de C<-T>!> Esta a ser destinada a ser utilizada I<sólo> como una ayuda temporal para el desarrollo, mientras asegura código heredado: para el código en producción real y para nuevo código seguro escrito desde cero, utilice siempre el verdadero B<-T>.
...
imprime advertencias sobre construcciones dudosas, como nombres de variables mencionadas sólo una vez y variables escalares utilizadas antes de ser declaradas; subrutinas redefinidas; referencias a gestores de archivo indefinidos; gestores de archivo abiertos en sólo lectura mientras que está intentando escribir en ellos; valores utilizados como un número que no I<parecen> números; utilizando un array como si se tratara de un escalar; si sus subrutinas tienen más de 100 niveles de profundidad de recursión; y un sinnúmero de otras cosas.
...
Vea también L<perldiag> y L<perltrap>.  Una muy detallada facilidad de alertas también está disponible si quiere manipular clases enteras de advertencias; vea L<warnings> o L<perllexwarn>.
...
Todas las referencias a números de línea en el programa (avisos, errores, ...) serán tratados como si la línea C<#!> fuera la primera línea.
...
Una lista de directorios en los que buscar archivos de bibliotecas Perl antes de buscar en la biblioteca estándar y en el directorio actual.  Cualesquiera directorios específicos de la arquitectura, bajo las ubicaciones indicadas, son automáticamente incluidos, si existen, realizando la búsqueda en el arranque del intérprete.
...
Opciones en línea de comandos.  Las opciones en esta variable son tratadas como si estuvieran en cada línea de comandos Perl.  Solo se permiten las opciones B<-[CDIMUdmtwW]>.  Cuando se ejecutan test de contaminación (ya sea porque el programa se estaba ejecutando setuid o setgid, o especificadas las opciones B<-T> o B<-t>), esta variable es ignorada.  Si PERL5OPT comienza con B<-T>, la contaminación es activada, y las siguientes opciones ignoradas.  Si PERL5OPT comienza con B<-t>, la contaminación será activada, un modificable punto eliminado de @INC, y las posteriores opciones tenidas en cuenta.
...
La lista se convierte en el valor predeterminado para I<toda> E/S de Perl. En consecuencia, sólo capas incluidas en el código pueden aparecer en esta lista, ya que las capas externas (tales como C<encoding()>) necesitan E/S para poder ser leídas! Vea L<"open pragma"|open> para saber cómo añadir codificaciones externas por defecto.
...
En plataformas Win32 esta capa I<experimental> usa "gestores" de E/S nativos en lugar de una capa de descriptor de fichero numérico tipo Unix. Se sabe que tiene errores en esta versión (5.14).
...
Perl normalmente busca un LSP compatible IFS, porque es necesario para la emulación de los I<sockets> de Windows como gestores de archivo real.  Sin embargo, esto puede causar problemas si usted tiene un cortafuegos como I<McAfee Guardian>, que requiere que todas las aplicaciones utilicen su LSP, pero que no es compatible con IFS, porque es evidente que Perl normalmente evita el uso de ese LSP.
...
(Desde Perl 5.8.1.) Se utiliza para aleatorizar la función hash interna de Perl.
Para emular el comportamiento pre-5.8.1, establezca a un número entero; C<"0"> significa exactamente el mismo orden que en 5.8.0.  "Pre-5.8.1" significa, entre otras cosas, que las claves de hash siempre tendrá el mismo orden entre las diferentes ejecuciones de Perl.

La mayoría de los hashes, por defecto, devuelven elementos en el mismo orden que en Perl 5.8.0.
En un trabajo normal con un hash, si se detectan datos patológicos durante una inserción de clave hash, entonces ese hash pasará a una alternativa de semillas de hash aleatorias.

El comportamiento por defecto es el azar a menos que se establezca PERL_HASH_SEED.
Si Perl se ha compilado con B<-DUSE_HASH_SEED_EXPLICIT>, el comportamiento por defecto es I<no> seleccionar al azar a menos que se establezca PERL_HASH_SEED.

Si PERL_HASH_SEED no está definida o está establecida a una cadena no numérica, Perl utiliza la semilla pseudoaleatoria suministrada por el sistema operativo y las bibliotecas.
...
Vea L<perlsec/"Ataques de Complejidad Algorítmica"> y L</PERL_HASH_SEED_DEBUG> para más información.

=item PERL_HASH_SEED_DEBUG
X<PERL_HASH_SEED_DEBUG>

(Desde Perl 5.8.1.) Puesto a C<"1"> para mostrar (en STDERR) el valor de la semilla de hash al comienzo de la ejecución.  Esto, combinado con L</PERL_HASH_SEED> está destinado a ayudar a depurar el comportamiento no determinista causado por la aleatorización de los hash.

B <Note que la semilla hash es información sensible>: conociéndolo, se puede elaborar un ataque de denegación de servicio contra el código Perl, incluso remotamente; vea L<perlsec/"Ataques de Complejidad Algorítmica"> para más información.
B<No muestre la semilla hash> a personas que no necesiten conocerla.
Vea también hash_seed() en L<Hash::Util>.

=item PERL_MEM_LOG
X<PERL_MEM_LOG>

Si su Perl se ha compilado con B<-Accflags=-DPERL_MEM_LOG>, poniendo la variable de entorno C<PERL_MEM_LOG> permite el registro de mensajes de depuración. El valor tiene la forma C<< <I<número>>[m][s][t] >>, donde C<I<número>> es el número de descriptor de archivo en donde desea escribir (2, por defecto), y la combinación de letras especifica que desea información acerca de (m)moria y/o (s)v, opcionalmente con (t)iempos. Por ejemplo, C<PERL_MEM_LOG=1mst> registra toda la información a la salida estándar. Puede escribir a otros descriptores de fichero abiertos en una variedad de formas:
...
Sin embargo, la ejecución de programas setuid harían bien en ejecutar las siguientes líneas antes de hacer nada, sólo para seguir manteniendo a la gente honesta:
