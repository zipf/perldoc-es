...
Las que no se pueden usar dentro de una clase de caracteres entre corchetes (como C<[\da-z]>) se marcan como C<No en [].>.
 ...
 \b                Límite de palabra/no-palabra. (Retroceso en []).
 \B                No es límite de palabra/no-palabra.  No en [].
 \cX               Control-X.
...
C<\b> es el carácter de retroceso solo dentro de una clase carácter. Fuera de una clase carácter, C<\b> es un límite de palabra/no-palabra.
...
Al igual que los escapes octales, existen dos formas de escapes hexadecimales, pero ambas empiezan por lo mismo: C<\x>.  A esto le sigue, exactamente, dos dígitos hexadecimales formando un número, o un número hexadecimal de longitud arbitraria rodeado con llaves. El número hexadecimal es el código del carácter que quiere expresar.
...
C<\w> es una clase de carácter que coincide con cualquier carácter que forme parte de una I<palabra> (letras, dígitos, marcas Unicode, caracteres de puntuación de unión (como el guión bajo)).  C<\d> es una clase de carácter que coincide con cualquier dígito decimal, mientras que la clase carácter C<\s> coincide con cualquier carácter espacio en blanco.
...
C<\b> coincide con cualquier lugar entre un carácter de palabra y otro que no lo es; C<\B> coincide con cualquier lugar entre aquellos caracteres en los que no coincidan con un C<\b>. C<\b> y C<\B> asumen que hay un carácter de no palabra antes del comienzo y final de la cadena fuente; así C<\b> coincidirá con el comienzo (o final) de la cadena fuente si la cadena comienza (o termina) con un carácter de palabra. De lo contrario, coincidirá C<\B>.   
...
Todas las determinaciones de frontera miran por caracteres de palabra sueltos, no por caracteres de no palabra, ni por finales de cadena.  Puede ayudar a entender cómo funcionan <\b> y <\B> equiparándolos a lo que sigue:
