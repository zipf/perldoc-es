...
Algunas plataformas tienen otras categorías, tales como unidades de medida y tamaños de papel.  Ninguno de estos se utiliza directamente por Perl, pero operaciones externas con las que Perl interacciona puede utilizarlos.  Vea L</Fuera del ámbito de cualquier variante de "use locale"> más abajo.
...
Perl no usará las configuraciones regionales a menos que lo solicite (pero, de nuevo, note que Perl puede interactuar con código que las utiliza).  Incluso si existe tal petición, B<todo> lo siguiente debe darse para que funcione correctamente:

=over 4

=item *

B<Su sistema operativo debe soportar el sistema de configuración regional>.  Si lo hace, debe encontrar que la función C<setlocale()> está documentada en alguna parte de su biblioteca de C.
...
B<Las variables de entorno que determinan la configuración regional (ver L</"ENTORNO">) deben estar correctamente ajustadas> en el momento del arranque de la aplicación, ya sea por usted mismo o por quien configuró la cuenta en el sistema; o
...
Por defecto, el propio Perl ignora la configuración regional en vigor.  El pragma S<C<use locale>> le pide a Perl que utilice la configuración regional actual para algunas operaciones.
A partir de v5.16, hay un parámetro opcional para este pragma:

    use locale ':not_characters';

Este parámetro permite una mejor mezcla de las configuraciones regionales y Unicode (menos útil en v5.20 y siguientes), y se describen detalladamente en L</Unicode y UTF-8> pero, en pocas palabras, le dice a Perl que no utilice las partes que describen a los caracteres en la definición de la configuración regional, es decir, las categorías C<LC_CTYPE> y C<LC_COLLATE>.  En su lugar, utilizará el juego de caracteres nativo (ampliado por Unicode).  Cuando use este parámetro, usted es el responsable de que el conjunto de caracteres externo sea traducido al nativo/Unicode (que ahora, realmente, se tratará de uno de las cada vez más populares configuraciones regionales UTF-8).  Hay varias formas convenientes de hacer esto, como se describe en L</Unicode y UTF-8>.
...
=item B<Fuera del ámbito de cualquier variante de "use locale">

Solo deben quedar afectadas las operaciones originadas fuera de Perl, como las que sigue:

=over 4

=item *

Las variables L<$!|perlvar/$ERRNO> (y sus sinónimas C<$ERRNO> y C<$OS_ERROR>) y L<$^E|perlvar/$EXTENDED_OS_ERROR> (y su sinónima C<$EXTENDED_OS_ERROR>) cuando se usan como cadenas de caracteres siempre es en función de la configuración regional actual, y como si estuvieran dentro del ámbito de L<"use bytes"|bytes>.  Esto probablemente cambie en Perl v5.22.

=item *

La configuración regional actual también se usa cuando se va hacia afuera de Perl con operaciones como L<system()|perlfunc/system LIST> o L<qxE<sol>E<sol>|perlop/qxE<sol>STRINGE<sol>>, si aquellas operaciones son sensibles a la configuración regional.

=item *

También Perl da acceso a varias funciones de biblioteca de C a través del módulo L<POSIX>.  Algunas de esas funciones están siempre afectadas por la configuración regional actual.  Por ejemplo, C<POSIX::strftime()> usa C<LC_TIME>; C<POSIX::strtod()> usa C<LC_NUMERIC>; C<POSIX::strcoll()> y C<POSIX::strxfrm()> usa C<LC_COLLATE>; y funciones de clasificación de caracteres como C<POSIX::isalnum()> usa C<LC_CTYPE>.  Todas esas funciones se comportarán de acuerdo a la configuración regional subyacente, incluso si esa configuración no está expuesta al espacio Perl.

=item *

Módulos XS para todas las categorías excepto C<LC_NUMERIC> obtienen la configuración regional subyacente, y por ello cualquier función de biblioteca C que llamen usará esa configuración regional.  Perl siempre inicializa C<LC_NUMERIC> a C<"C"> porque demasiados módulos son incapaces de soportar el punto decimal en
un numero flotante que no tiene un punto (en muchas configuraciones regionales es una coma).
Pero note que estos módulos son vulnerables porque C<LC_NUMERIC>, actualmente, se puede cambiar en cualquier momento con una llamada a la función C C<set_locale()> por el código XS o por algo que el código XS llame, o por el C<POSIX::setlocale()> por código Perl.  Esto también es cierto para los pequeños envoltorios proporcionados por Perl para módulos XS para usar algunas funciones de biblioteca C C<printf>: C<Gconvert>, L<my_sprintf|perlapi/my_sprintf>, L<my_snprintf|perlapi/my_snprintf>, y L<my_vsnprintf|perlapi/my_vsnprintf>.
...
Ciertas operaciones Perl que están activadas dentro del ámbito de una variante C<use locale> retienen ese efecto incluso fuera del ámbito.
Eso incluye:
...
El formato de salida de un L<write()|perlfunc/write> se determina por una declaración temprana de formato (L<perlfunc/format>), así que si la salida queda o no afectada por la configuración regional queda determinado por si C<format()> está dentro del ámbito de una variante C<use locale>, no el del C<write()>.

=item *

Se pueden compilar patrones de expresiones regulares utilizando L<qrE<sol>E<sol>|perlop/qrE<sol>STRINGE<sol>msixpodual> para utilizarlas más tarde en las coincidencias.  De nuevo, si la compilación se hace o no dentro del ámbito de C<use locale> determina el comportamiento de la coincidencia, no si las coincidencias se realizan dentro de tal ámbito o no.
...
Todas las operaciones que no son de Perl.
...
=item B<Bajo de un simple C<"use locale;">>
...
Nótese que C<use locale> y C<use locale ':not_characters'> se pueden anidar, y que lo que está en vigor dentro de un ámbito interno volverá a las reglas del ámbito externo al final del ámbito interno.
...
Si está compilando Perl, el archivo de prueba F<lib/locale.t> se puede usar para comprobar las configuraciones regionales en su sistema.
...
Las quejas de Perl sobre los problemas con la configuración regional se pueden silenciar mediante el ajuste de la variable de entorno PERL_BADLANG a un valor cero, por ejemplo, "0".
...
La función C<POSIX::localeconv()> le permite obtener datos de la información del formateo numérico dependiente de la configuración regional especificada por las actuales configuraciones regionales C<LC_NUMERIC> y C<LC_MONETARY>.  (Si sólo desea el nombre de la configuración regional actual para una determinada categoría, use C<POSIX::setlocale()> con un solo parámetro -vea L<La función setlocale>).
...
Todas las ordenaciones siguientes tienen sentido y usted puede seguir cualquiera de ellas si usa "use locale".
...
Perl solo admite configuraciones regionales de un solo byte para C<LC_COLLATE>.  Eso significa que una configuración regional UTF-8 probablemente sólo le dará una ordenación nativa de la máquina.
...
Finalmente, C<LC_CTYPE> afecta a las funciones de prueba de clases de caracteres POSIX (obsoletas) -C<POSIX::isalpha()>, C<POSIX::islower()>, y así sucesivamente-.  Por ejemplo, si se muda de la configuración regional "C" a una escandinava de 7-bit, puede encontrar -posiblemente, para su sorpresa- que "|" se mueve desde la clase C<POSIX::ispunct()> a la C<POSIX::isalpha()>.
...
Comenzando en v5.20, Perl soporta configuraciones regionales UTF-8 para C<LC_CTYPE>, pero, por contra, Perl sólo soporta configuraciones regionales basadas en bytes únicos, como la serie ISO 8859.  Esto significa que las configuraciones regionales con caracteres anchos, como por ejemplo las lenguas asiáticas, no están soportadas.  El soporte para la configuración regional UTF-8 es, actualmente, un superconjunto de las configuraciones regionales POSIX, porque tiene un completo comportamiento Unicode como si no estuviera en efecto ninguna configuración regional (excepto para el entintado; vea L</SECURITY>).  Las configuraciones regionales POSIX, incluso las UTF-8, carecen de ciertos conceptos de Unicode, como la idea de que al cambiar de tamaño de caja un carácter, éste podría expandirse a más de un carácter.
...
Después de una apropiada llamada C<POSIX::setlocale()>, y dentro del ámbito de una de las variantes de C<use locale>, Perl sigue la información de configuración regional C<LC_NUMERIC>, que controla la idea que tiene la aplicación de cómo los números deben ser formateados para facilitar su legibilidad humana.
...
La interpolación de cadenas con cambios de tamaño de caja, como en, por ejemplo, C<$destino = "C:\U$nombre.$ext">, puede producir resultados peligrosos si una tabla falsa LC_CTYPE de correspondencia de caja está en vigor.
...
Los nombres de la fecha y el día en fechas formateadas por C<strftime()> podrían manipularse para aprovecharse de un usuario malicioso capaz de subvertir la configuración regional C<LC_DATE>.  ("¡Mira!: Me dice que no estaba en el edificio, el domingo").
...
Resultado escalar verdadero/falso (o menor/igual/mayor) que nunca es entintado.

=item  *

B<Interpolación de tamaño de caja> (con C<\l>, C<\L>, C<\u>,  C<\U> o C<\F>)

La cadena resultante conteniendo material interpolado queda entintada si C<use locale> (pero no S<C<use locale ':not_characters'>>) está en vigor.

=item  *

B<Operador de coincidencia> (C<m//>):

El resultado escalar verdadero/falso nunca es entintado.

Todos los subpatrones, tanto si son entregados como resultado en un contexto en lista, o como C<$1>, etc., quedan entintados si C<use locale> (pero no S<C<use locale ':not_characters'>>) está en efecto, y el subpatrón de expresión regular contiene una construcción dependiente de la configuración regional.  Estas construcciones incluyen C<\w> (para coincidir con un carácter alfanumérico), C<\W> (carácter no-alfanumérico), C<\b> y C<\B> (márgenes y no-márgenes de palabra, que dependen de lo que coinciden con C<\w> y C<\W>), C<\s> (carácter espacio en blanco), C<\S> (no carácter espacio en blanco), C<\d> y C<\D> (dígitos y no-dígitos), y las clases de caracteres POSIX, tales como C<[:alpha:]> (vea L<perlrecharclass/POSIX Character Classes>).

Entintar también es posible si el patrón para la coincidencia es independiente del tamaño de caja (vía C</i>).  La excepción es si todos los códigos de carácter que buscamos de esta manera están por encima del 255 y no se pliegan bajo las reglas Unicode por debajo de 256.  El entintado no se realiza para ellos, porque Perl sólo usa reglas Unicode para tales códigos de carácter, y esas reglas son las mismas sin importar qué configuración regional esté en activo.

Las variables de coincidencia de patrones, C<$&>, C<$`> (pre-coincidencia), C<$'> (pos-coindencia), y C<$+> (última coincidencia) también quedan entintadas.

=item  *

B<Operador de sustitución> (C<s///>):

Tiene el mismo comportamiento que el operador de coincidencia.  También, el operando a la izquierda de C<=~> queda entintado cuando C<use locale> (pero no S<C<use locale ':not_characters'>>) está en efecto si queda modificado como resultado de una substitución basada en una coincidencia de expresión regular incluyendo cualquiera de las cosas mencionadas en el ítem anterior, o de mapeo de cambio de tamaño de caja, como C<\l>, C<\L>,C<\u>, C<\U>, o C<\F>.

=item *

B<Funciones de formateo de la salida> (C<printf()> y C<write()>):

Los resultados nunca quedan entintados, porque de lo contrario, incluso hasta la salida con print, por ejemplo, C<print(1/7)>, debería quedar entintado si C<use locale> estuviera en efecto.

=item *

B<Funciones de translación de tamaño de caja> (C<lc()>, C<lcfirst()>, C<uc()>, C<ucfirst()>):

Los resultados quedan entintados si C<use locale> (pero no S<C<use locale ':not_characters'>>) está en efecto.

=item *

B<Funciones dependientes de la configuración regional POSIX> (C<localeconv()>, C<strcoll()>, C<strftime()>, C<strxfrm()>):

Los resultados nunca quedan entintados.
...
Los resultados verdadero/falso nunca quedan entintados.

=back

Tres ejemplos ilustran el entintado dependiente de la configuración regional.
El primer programa, que ignora su configuración regional, no se ejecutará: un valor tomado directamente de la línea de comandos no puede ser utilizado para nombrar un archivo de salida cuando los controles de entintado están habilitados.

        #/usr/local/bin/perl -T
        # Ejecutar con comprobación de contaminación

        # Omitida la comprobación de la línea de comandos...
        $archivo_salida_entintado = shift;

        open(F, ">$archivo_salida_entintado")
            or warn "La apertura de $archivo_salida_entintado falló: $!\n";

Se puede obligar a que el programa haga un "lavado" del valor entintado llevándolo a través de una expresión regular: el segundo ejemplo -que todavía ignora la información de la configuración regional- se ejecuta, creando el archivo con el nombre indicado en su línea de comando, si lo puede hacer.

        #/usr/local/bin/perl -T

        $archivo_salida_entintado = shift;
        $archivo_salida_entintado =~ m%[\w/]+%;
        $archivo_salida_entintado = $&;
...
Este tercer programa falla porque C<$&> está entintado: es el resultado de una coincidencia que contiene C<\w>, mientras que C<use locale> estaba en efecto.
...
Esta variable de entorno, disponible a partir de Perl v5.20, y si se evalúa a VERDADERO, le indica a Perl que no use el resto de las variables de entorno en la inicialización.  De lo contrario, Perl usa lo que esté ajustado en la actual configuración regional.  Esto es particularmente útil en entornos incrustados. Vea L<perlembed/Usando Perl incrustado con configuraciones regionales POSIX>.
...
Sin embargo, son tratados como configuraciones regionales de un solo byte, y tendrá las restricciones indicadas a continuación.
...
Aún así, otro problema es que esta aproximación pueden llevar a que dos códigos de carácter signifiquen el mismo carácter.  Así, en la configuración regional griega, tanto U+03A7 como U+00D7 son GREEK CAPITAL LETTER CHI.
