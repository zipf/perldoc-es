...
Igual que F<lib/>, este directorio contiene módulos que solo han sido publicados como parte del núcleo.  A diferencia de F<lib/>, sin embargo, un módulo bajo F<ext/> tiene, generalmente, un estilo y disposición parecido al de CPAN, y tiene su propio F<Makefile.PL>.  No se espera que un módulo bajo F<ext/> funcione con versiones anteriores de Perl 5.  Por ello, dicho módulo puede aprovecharse de las ventajas sintácticas y otras mejoras de Perl 5.

=item * F<dist/>

Este directorio contiene módulos de evolución dual en los que la versión canónica es la del código fuente de la rama B<blead>. Tenga en cuenta que es posible que algunos de los módulos de este directorio aún no se hayan publicado por separado en CPAN.  Los módulos bajo F<dist/> tendrían que hacer un esfuerzo para funcionar en versiones anteriores de Perl 5.
...
Pruebas para las funciones predefinidas de perl que, como las que hay en F<t/op/>, no encajan en ninguno de otros directorios, pero que, además, ya no puede usar F<t/test.pl>, ya que el programa depende de la funcionalidad del propio archivo de prueba.
