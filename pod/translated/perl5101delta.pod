...
Tenga en cuenta que, como en Perl 5.10.0, C<when (1..10)> no va servirá para probar si un valor es un número entero entre 1 y 10, así que en su lugar debería usar C<when ([1..10])> (nótese la referencia al array).

Sin embargo, contrariamente a 5.10.0, la evaluación de los operadores flip-flop en contexto booleano asegura que ahora puede ser útil en un C<when()>, especialmente para implementar condiciones biestable, como en:
...
Una expresión compuesta, que incluya al operador definido-or, como en C<when (expr1 // expr2)>, será tratada como booleana si la primera expresión es booleana. (Esto amplia la regla actual que se aplica al operador or normal, como en C<when (expr1 || expr2)>.)
...
Referencias a código con un prototipo vacío ya no son tratados de forma especial.
Se les pasa un argumento como a otras referencias de código (incluso si deciden ignorarlo).

=item *

C<%hash ~~ sub {}> y C<@array ~~ sub {}> ahora prueban a que la subrutina devuelva un valor verdadero por cada clave del hash (o cada elemento del array), en lugar de pasar todo el hash o el array, como referencias, a la subrutina.
...
La copia de la base de datos de caracteres Unicode incluidos en Perl 5.10.1 ha sido actualizado desde la 5.0.0 a la 5.1.0. Vea L<http://www.unicode.org/versions/Unicode5.1.0/#Notable_Changes> para consultar los cambios principales.
...
La opción de línea de comandos C<-i.sufijo> ahora a crear el archivo con permisos restringidos, antes de cambiar su modo para que coincida con el archivo original. Esto elimina una potencial condición de carrera [RT #60904].
...
C<< $object->isa('Foo') >> podría reportar falso si el paquete C<Foo> no existe, incluso si el objeto de C<@ISA> contenía a C<Foo>.
...
Fugas memoria asociados con capturas con nombre en expresiones regulares han sido arregladas [RT #57024].
...
Llamando a C<sv_chop()> o, de lo contrario, la actualización de un SV podría resultar en un desalineado acceso de 64 bits en la arquitectura SPARC [RT #60574].
...
Una nueva herramienta, C<Porting/expand-macro.pl> ha sido añadida, que permite ver cómo una macro del preprocesador C se amplíe cuando se compile.
...
C<Switch> es defectuoso y debe ser evitado. De Perl 5.11.0 en adelante, se pretende que el uso de la versión principal de este módulo emita una advertencia, y que el módulo, con el tiempo, será eliminado del núcleo principal (probablemente en perl 5.14.0). Vea L<perlsyn/"Instrucciones Switch"> para ver sus reemplazos.
...
Algo del trabajo realizado en esta publicación ha sido financiado con una ayuda de la TPF.
...
Craig Berry fue incansable en conseguir que se ejecutara maint en VMS, sin importar cuántas veces lo rompíamos.
