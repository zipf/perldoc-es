...
Esta lista de preguntas y respuesta sobre Unicode en Perl debe leerse a continuación de L<perlunitut>.

=head2 En realidad, perlunitut no es un tutorial de Unicode, ¿verdad?

No. Y este documento tampoco contiene preguntas y respuestas sobre Unicode.

Perl incluye una interfaz que abstrae todas las codificaciones de caracteres compatibles; este documento es en realidad un tutorial de C<Encode> genérico con las preguntas más frecuentes sobre C<Encode>. Pero muchas personas piensan que Unicode es algo especial y mágico. Como no quiero que se sientan defraudadas, he decidido llamar clasificar este documento como un tutorial de Unicode.
...
Si puede, actualice a la versión más reciente. Pero como mínimo debe usar la versión C<5.8.1>.
En este tutorial y en las preguntas más frecuentes se supone que usa la versión más reciente.
...
No obstante, hay que evitar combinar cadenas de texto con cadenas binarias. Si necesita texto en una secuencia binaria, codifique las cadenas de texto con la codificación adecuada y después únalas con cadenas binarias. Vea también: "¿Qué pasa si no descodifico?".

=head2 ¿Cuándo debo descodificar o codificar?

Siempre que vaya a comunicar texto a alguna entidad externa al proceso de perl, como una base de datos, un archivo de texto, un socket u otro programa. Aunque dicha entidad externa también se haya programado en Perl.

=head2 ¿Qué pasa si no descodifico?

Siempre que la cadena binaria codificada se use junto con una cadena de texto, Perl
supondrá que la cadena binaria se codificó con ISO-8859-1 (también llamada latin-1). Si la codificación no es latin-1, el resultado de la conversión de los datos no será lo que se esperaba. Por ejemplo, si la codificación es UTF-8, los bytes individuales de los caracteres multibyte se verán como caracteres independientes y se volverán a convertir a UTF-8. Esta codificación doble es comparable a la codificación doble de HTML (C<&amp;gt;>) o a la codificación doble de URI (C<%253E>).

Esta descodificación implícita silenciosa se denomina "actualización". Es posible que esto parezca algo positivo, pero es mejor evitarlo.
...
Como el formato interno suele ser UTF-8, es difícil detectar estos errores (puesto que normalmente la codificación deseada es UTF-8). Pero no sea perezoso; no debe basarse en el hecho de que el formato interno de Perl es UTF-8. Codifique explícitamente para evitar errores extraños y para indicar a los programadores de mantenimiento que ///you
...
Puede suministrar esta capa al abrir el archivo con C<open>:
...
Por supuesto. Si el código fuente está codificado como UTF-8, puede indicarlo con el pragma 
C<use utf8>.

    use utf8;

Esto no afecta ni a la entrada ni a la salida de datos. Sólo influye en la manera de leer el código fuente. Puede usar Unicode en literales de cadena, en identificadores (aunque deben ser "caracteres de palabra" compatibles con C<\w>) e incluso en delimitadores personalizados.
...
Desde la versión Perl 5.14 (y, de forma parcial, la versión Perl 5.12), solo tiene que agregar C<use feature 'unicode_strings'> al principio del programa.
En su ámbito léxico, este problema no debería producirse.  También se habilita automáticamente en C<use feature ':5.12'> o mediante el modificador C<-E> en la línea de comandos para Perl 5.12 o superior.

Esto se hace así para evitar que dejen de funcionar programas antiguos, que se basan en el funcionamiento anterior a la creación de Unicode.  Esos programas antiguos solo usaban el juego de caracteres ASCII, por lo que es posible que no funcionen con otros caracteres.  Si una cadena está codificada en UTF-8, Perl supone que el programa está preparado para trabajar con Unicode, pero si la codificación es otra, Perl supone que solo se desea usar ASCII (salvo en plataformas EBCDIC), por lo que los caracteres que no sean ASCII no se reconocerán como lo que serían en Unicode.
...
Sin embargo, en versiones anteriores de Perl, o si pasa cadenas a subrutinas fuera del ámbito del pragma C<feature>, puede forzar el uso de semántica Unicode cambiando la codificación a UTF-8 mediante C<utf8::upgrade($cadena)>. Esto se puede aplicar de manera segura a cualquier cadena, puesto que comprueba si una cadena ya se ha promovido y, en caso afirmativo, no la modifica.

Encontrará una descripción más detallada en L<Unicode::Semantics> (en CPAN).
...
=head2 ¿Cómo puedo pasar de la codificación FOO a la codificación BAR?
...
Primero debe convertir la cadena de bytes con la codificación A en una cadena de texto y después la cadena de texto en una cadena de bytes con la codificación B:
    ...
    my $cadena_texto = decode('FOO', $cadena_foo);
    my $cadena_bar  = encode('BAR', $cadena_texto);

También puede omitir la parte de la cadena de texto y pasar directamente de una codificación binaria a otra:

    use Encode qw(from_to);
    from_to($cadena, 'FOO', 'BAR');  # cambia el contenido de $cadena

o dejar que la descodificación y la codificación automáticas se encarguen de hacer la conversión:

    open my $foofh, '<:encoding(FOO)', 'ejemplo.foo.txt';
    open my $barfh, '>:encoding(BAR)', 'ejemplo.bar.txt';
    print { $barfh } $_ while <$foofh>;

=head2 ¿Para qué sirven C<decode_utf8> y C<encode_utf8>?

Ofrecen una sintaxis alternativa para C<decode('utf8', ...)> y C<encode('utf8',
...)>.

=head2 ¿Qué es un "carácter ancho"?

Este término se usa para definir los caracteres que tienen un valor ordinal superior a 127, los caracteres que tienen un valor ordinal superior a 255 o cualquier carácter que ocupe más de un byte, según el contexto.
...
Para evitar esta advertencia y evitar la mezcla de distintas codificaciones de salida en una sola secuencia, especifique siempre una codificación explícitamente (por ejemplo, con una capa de PerlIO):
...
A menos que tenga que trabajar con los componentes internos o depurar cosas extrañas, olvídese de la marca UTF8. Esto significa que es muy probable que no deba usar nunca C<is_utf8>, C<_utf8_on> y C<_utf8_off>.
...
Resulta que uno de los formatos internos de Perl es UTF-8. Como Perl no se calla nada, esto es de dominio público. Esto crea muchas confusiones. Es mejor imaginar que el formato interno es alguna codificación desconocida y acostumbrarse a codificar y descodificar explícitamente siempre.

=head2 ¿Y el pragma C<use bytes>?

No lo use. No tiene sentido trabajar con bytes en una cadena de texto ni trabajar con caracteres en una cadena de bytes. Realice las conversiones apropiadas (mediante descodificación o codificación) y todo saldrá bien: obtendrá recuentos de caracteres para los datos descodificados y recuentos de bytes para los datos codificados.

C <use bytes> suele ser un intento fallido de hacer algo útil. Olvide que existe.

=head2 ¿Y el pragma C<use encoding>?

No lo use. Por desgracia, supone que la codificación utilizada en el entorno del programador es la misma que la utilizada en el entorno del usuario. Utilizará la misma codificación para el código fuente que para STDIN y STDOUT. Cuando se copia un programa a otro equipo, el código fuente no cambia, pero podría cambiar el entorno STDIO.

Si necesita utilizar caracteres que no pertenezcan a ASCII en el código fuente, codifíquelo como UTF-8 y use el pragma C<use utf8>.
...
Existen algunos métodos abreviados para scripts de una línea (I<one-liners>); vea la descripción del modificador C<-C> en L<perlrun>.

= Head2 ¿Cuál es la diferencia entre C <UTF-8> y C<utf8>?

C<UTF-8> es el estándar oficial. C<utf8> es la manera de Perl de ser liberal en lo que acepta. Si tiene que comunicarse con algo que no es tan libera, es posible que sea mejor utilizar C<UTF-8>. Sin embargo, si tiene que comunicarse con algo que es demasiado liberal, puede que tenga que utilizar C<utf8>. Encontrará información detallada en 
L<Encode>.

Internamente, C<UTF-8> se denomina C<utf-8-strict>. En el tutorial se usa siempre UTF-8, incluso cuando se use utf8 internamente, ya que esta distinción puede resultar difícil de apreciar y además suele ser irrelevante.

Por ejemplo, se puede usar utf8 para códigos de carácter que no existen en Unicode, como 9999999, pero si los codifica en UTF-8, obtendrá caracteres sustitutos de manera predeterminada (vea L<Encode/"Handling Malformed Data"> para obtener información sobre otras maneras de ///dealing with
this.)
...
Perl sabe cómo se ha almacenado la cadena internamente, y usará ese conocimiento al codificar. Es decir, no intente averiguar cuál es la codificación interna de una cadena determinada; sólo tiene que codificarla con la codificación que desee.
