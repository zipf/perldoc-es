...
Los modificadores C<while> y C<until> tienen la usual semántica de los "bucles C<while>" (la condición se evalúa primero), excepto cuando se aplica a un bloque C<do> (o a la obsoleta instrucción de subrutina C<do>), en cuyo caso el bloque se ejecuta una vez antes de que sea evaluada la condición.  De esa manera puede escribir bucles como este:
...
Cuando una instrucción C<given> es también una expresión válida (p.e. cuando es la última instrucción de un bloque), se evalúa a:
