...
Los nombres de variables Perl también pueden estar formados por una secuencia de dígitos o un solo singno de puntuación o carácter de control.  Todos estos nombres están reservados para usos especiales en Perl; por ejemplo, los nombres formados exclusivamente por dígitos se usan para almacenar datos capturados por retrorreferencias detectadas por una expresión regular.  Perl tiene una sintaxis especial para los nombres de un solo carácter de control: entiende que C<^X> (acento circunflejo-C<X>) representa al carácter de control-C<X>.  Por ejemplo, la notación C<$^W> (signo de dólar, acento circunflejo, C<W>) es la variable escalar cuyo nombre es el carácter único de control-C<W>.  Esto es mejor que escribir un control-C<W> literal en su programa.

Desde la versión Perl 5.6, los nombres de variables Perl pueden ser cadenas alfanuméricas que empiezan por caracteres de control (o mejor, por un signo de intercalación).
Estas variables deben escribirse con la sintaxis C<${^Foo}>; las llaves no son opcionales.  C<${^Foo}> denotas la variable escalar cuyo nombre es un control-C<F> seguido por dos C<o>.  Estas variables están reservadas para futuros usos especiales de Perl, a excepción de los que comienzan con C<^_> (control-guión bajo o acento circunflejo-guión bajo).  Ningún nombre de carácter de control que comience con C<^_> adquirirá un significado especial en cualquier futura versión de Perl; por lo tanto, estos nombres pueden ser usados con seguridad en los programas actuales.  C<$^_>, sin embargo, I<está> reservada.

Los identificadores de Perl que comienzan con dígitos, caracteres de control, o caracteres de puntuación están exentos de los efectos de la declaración C<package> y siempre se ven obligados a estar en el paquete C<main>; también están exentos de los errores de C<strict 'vars'>.  Otros pocos nombres más también están exentos de esas reglas:
...
En particular, las variables especiales C<${^_XYZ}> se consideran que están siempre en el paquete C<main>, independientemente de cualquier declaración C<package> en el ámbito actual.
...
al principio del programa.  Esto crea alias cortos, de los nombres largos, en el paquete actual.  Algunos incluso tienen nombres de tamaño medio tomados, en general, de B<awk>.  Para evitar un impacto en el rendimiento, si no va a necesitar C<$PREMATCH>, C<$MATCH>, o C<$POSTMATCH>, lo mejor es utilizar el módulo C<English>, descartándolas:
...
La entrada por defecto y el espacio de búsqueda de patrones.  Los pares siguientes son equivalentes:
    ...
    while (<>) {...}    # ¡equivalente solo con while!
    while (defined($_ = <>)) {...}
...
abs, alarm, chomp, chop, chr, chroot, cos, defined, eval, evalbytes, exp, glob, hex, int, lc, lcfirst, length, log, lstat, mkdir, oct, ord, pos, print, quotemeta, readlink, readpipe, ref, require, reverse (solo en contexto de escalar), rmdir, sin, split (en su segundo argumento), sqrt, stat, study, uc, ucfirst, unlink, unpack.
...
El lugar por defecto para poner un registro de entrada cuando se comprueba el resultado de una operación C<< <FH> >>, cuando aparece como el único criterio de un test C<while>.  Esto no ocurrirá, fuera de un test C<while>.

=back

Como C<$_> es una variable global, esto puede dar lugar, en algunos casos, a efectos secundarios no deseados.  A partir de perl 5.10, puede utilizar una versión léxica de C<$_> declarándola en un archivo o en un bloque con C<my>.  Por otra parte, declarando C<our $_>, restaura la C<$_> global en el ámbito actual.
...
Cuando un array o una rebanada de un array se interpola en una cadena entre comillas dobles o de un contexto similar, tales como C</.../>, sus elementos quedan separados por este valor.  El valor predeterminado es un espacio.  Por ejemplo, esto:
...
El número de proceso de Perl ejecutando este I<script>.  A pesar de que I<puede> darle un valor a esta variable, por lo general se desaconseja hacerlo, aunque puede resultar muy útil para algunas comprobaciones de pruebas.  Será restablecerá automáticamente a través de las llamadas C<fork()>.
...
Tenga en cuenta que existen limitaciones específicas de la plataforma sobre la longitud máxima de C<$0>.  En el caso más extremo que puede estar limitado al espacio ocupado por el C<$0> original.
...
Del mismo modo, un valor asignado a C<$)> también debe ser una lista de números separada por espacios.  El primer número establece el gid efectivo, y el resto (si existe) se pasan a C<setgroups()>.  Para obtener el efecto de una lista vacía para C<setgroups()>, sólo tiene que repetir el nuevo gid efectivo; es decir, obligar a que un gid efectivo de 5 y un lista C<setgroups()> efectivamente vacía, diga C< $) = "5 5">.
...
Cambios en C<$)> requiere una verificación de C<$!> para detectar posibles errores después de un intento de cambio.
...
Pero no ponga
    ...
    @foo{$a,$b,$c}	# un trozo -fíjese en el @-
...
Debido a este carácter especial de C<$a> y C<$b> no tienen que ser declaradas (usando C<use vars>, u C<our()>) incluso cuando se utiliza el pragma C<strict 'vars'>.  No los localice con C<my $a> o C<my $b> si quiere utilizarlos en el bloque o función de comparación de C<sort()>.
...
El máximo descriptor de archivos del sistema, normalmente 2.  Los descriptores de archivos del sistema se pasan a procesos ejecutados con C<exec()>, mientras que los descriptores de archivo más altos, no.  También, durante un C<open()> los descriptores de archivos del sistema se conservan incluso si el C<open()> falla (los descriptores de archivos ordinarios son cerrados antes de que se intente un C<open()>).  El estado de cerrar-en-ejecución de un descriptor de archivo será decidido de acuerdo al valor de 
C<$^F> cuando el correspondiente archivo, tubería o I<socket> sea abierto, no en el momento del C<exec()>.

=item @F
X<@F>

El array C<@F> contiene los campos de cada línea leída cuando está activado el modo auto-partir.  Consulte L<perlrun> para ver la opción B<-a>.  Este array es específico de un paquete, y debe ser declarado o indicar un nombre completo de un paquete si no está dentro del paquete main cuando se ejecuta bajo C<strict 'vars'>.

=item @INC
X<@INC>

El array C<@INC> contiene la lista de lugares en que las construcciones C<do EXPR>, C<require>, o C<use> buscan sus archivos de biblioteca.  En un principio se compone de los argumentos de cualquier opción B<-I> en la línea de comandos, seguido de la biblioteca Perl predeterminada, probablemente F</usr/local/lib/perl>, seguido por ".", para representar el directorio actual.  ("." no será añadido si está activado el modo I<taint> -contaminado-, tanto por C<-T> como por C<-t>).  Si necesita modificar este comportamiento en tiempo de ejecución, debe utilizar el pragma C<use lib> para conseguir que la biblioteca dependiente de la máquina sea cargada correctamente:
...
Sin embargo, si está adecuadamente compilado, Perl puede utilizar el contenido de C<^M> como un bloque de memoria de emergencia después de C<die()> (morir).  Supongamos que su Perl se compila con C<-DPERL_EMERGENCY_SBRK> y utiliza el malloc del Perl.
Entonces

    $^M = 'a' x (1 << 16);

asignaría un búfer de 64K para usarse en caso de emergencia.  Vea el archivo F<INSTALL> en la distribución Perl para obtener información sobre cómo añadir banderas de compilación C personalizadas al compilar perl.  Para desalentar el uso casual de este sistema avanzado, no existe un nombre largo en L<English|English> para esta variable.
...
El hash C<%SIG> contiene los gestores de señal para el procesamiento de señales.  Por ejemplo:
...
Si su sistema tiene la función C<sigaction()>, entonces los manejadores de señales instalados la usarán.  Esto significa que obtiene un manejo de señales fiable.

La política predeterminada de entrega de señales cambió en Perl 5.8.0, de "inmediato" (también conocido como "inseguro") a "diferido", también conocido como "señales seguras".  Vea L<perlipc> para más información.

Algunos enganches internos también se pueden establecer mediante el hash C<%SIG>.  La rutina indicada por C<$SIG{__WARN__}> es llamada cuando un mensaje de advertencia está a punto de ser impreso.  El mensaje de advertencia se pasa como primer argumento.  La presencia de un enganche en C<__WARN__> hace que se suprima la impresión normal de las advertencias hacia C<STDERR>.  Puede usar esto para salvar las advertencias en una variable, o convertir las advertencias en errores fatales, como este:
...
La rutina indicada por C<$SIG{__DIE__}> es llamada cuando una excepción grave está a punto de ser lanzada.  El mensaje de error es pasado como primer argumento.  Cuando una rutina enganchada en C<__DIE__> regresa, el procesamiento de la excepción continúa como si no existiera el enganche, a menos que la rutina enganchada termine a través de un C<goto &sub>, la salida de un bucle, o un C<die()>.  El manejador de C<__DIE__> está explícitamente inhabilitado durante la llamada, para que pueda morir desde un controlador C<__DIE__>.  Lo mismo para C<__WARN__>.

Debido a un fallo de implementación, el enganche C<$SIG{__DIE__}> es llamado incluso dentro de un C<eval()>.  No lo utilice para volver a reescribir una excepción pendiente en C<$@>, o como un sustituto extraño para sobreescribir C<CORE::GLOBAL::die()>.  Esta extraña acción será arreglada en una futura versión de manera que C<$SIG{__DIE__}> sólamente sea llamada si el programa está a punto de terminar, como era la intención original.  Cualquier otro uso es obsoleto.

Los manejadores C<__DIE__>/C<__WARN__> son muy especiales en un aspecto: pueden ser llamados para informar de (posibles) errores encontrados por el analizador léxico.  En tal caso, el analizador léxico puede estar en estado de incoherencia, por lo que cualquier intento de evaluar código Perl desde tales manejadores probablemente resultará en una violación de segmento.  Esto significa que las advertencias o errores que se deriven del análisis léxico de Perl se deberían usar con extrema precaución, por ejemplo:
...
Aquí, la primera línea cargará C<Carp> I<excepto> si es el analizador léxico quien llama al manejador.  La segunda línea imprimirá la traza de llamadas y morirá si C<Carp> estaba disponible.  La tercera línea se ejecutará sólo si C<Carp> no estuviera disponible.

El tener que pensar ni siquiera en la variable C<$^S> en sus manejadores de excepción es, simplemente, erróneo.  C<$SIG{__DIE__}>, tal como está actualmente implementado, es una dolorosa y difícil solución para localizar errores.  Evítelo y use en su lugar un C<END{}> o sobreescriba un Core::GLOBAL::die.
...
El momento en que se inició el programa en ejecución, en segundos desde la época UNIX (principios de 1970).  Los valores devueltos por los test de archivo B<-M>, B<-A> y B<-C> se basan en este valor.
...
La revisión, versión y la subversión del intérprete de Perl, representado como un objeto C<version>.

Esta variable apareció por primera vez en Perl 5.6.0; las versiones anteriores de perl verán un valor indefinido.  Antes de Perl 5.10.0 C<$^V> fue representado como un v-string.
...
Para convertir C<$^V> en su representación en cadena de caracteres, use la conversion 
C<"%vd"> de C<sprintf()>:
...
Vea también C<$]> para ver una representación más antigua de la versión de Perl.

Esta variable se incorporó en Perl 5.6.

Mnemónico: use ^V para el Control de Versión.
...
Esta variable se incorporó en Perl 5.10.
...
No es seguro usar el valor de C<$^X> como un nombre de ruta de un archivo, ya que algunos sistemas operativos que tienen un sufijo obligatorio en los archivos ejecutables no requieren el uso del sufijo al invocar un comando.  Para convertir el valor de C<$^X> a un nombre de ruta, utilice las siguientes instrucciones:
    ...
    # Construir un conjunto de nombres de archivo (no de nombres de comandos).
    use Config;
    my $este_perl = $^X;
    if ($^O ne 'VMS') {
...
La mayoría de las variables especiales relacionadas con las expresiones regulares tienen efectos secundarios.  Perl establece estas variables cuando se tiene una concordancia exitosa, por lo que debería comprobar el resultado de la coincidencia antes de usarlas.  Por ejemplo:
...
El resultado muestra que mientras se esté en el bloque C<EXTERNO>, los valores de C<$1> y C<$2> son de la concordancia con C<$externo>.  Dentro del bloque C<INTERNO>, los valores de C<$1> y C<$2> son de la coincidencia con C<$interior>, pero sólo hasta el final del bloque (es decir, en el ámbito dinámico).  Después de que el bloque C<INTERNO> se complete, los valores de C<$1> y C<$2> retornan a los valores para la coincidencia con C<$exterior> a pesar de que no hayamos hecho otra coincidencia:

    $1 es Wallace; $2 es Grommit
    $1 es Mutt; $2 es Jeff
    $1 es Wallace; $2 es Grommit

Debido a un desafortunado accidente en la implementación de Perl, C<use English> impone una considerable penalización de rendimiento en todos las coincidencias de expresiones regulares en un programa, ya que utiliza el C<$`>, C<$&>, y C<$'>, con independencia de que se aparezcan o no en el ámbito de C<use English>.  Por esa razón, no se recomienda escribir C<use English> en las bibliotecas a menos que lo importe sin las variables de emparejamiento:
...
Desde Perl 5.10, puede usar la opción de coincidencia C</p> y las variables C<${^PREMATCH}>, C<${^MATCH}> y C<${^POSTMATCH}> para no sufrir penalizaciones en el rendimiento.
...
Contiene el sub-patrón del correspondiente conjunto de paréntesis de captura de la última coincidencia de patrón con éxito, sin contar los patrones encontrados en los bloques anidados que hayan terminado ya.
...
El uso de esta variable en cualquier parte de un programa impone una penalización de rendimiento considerable en todas las coincidencias de expresiones regulares.  Para evitar esta penalización, puede extraer la misma subcadena mediante el uso de L</@->.  A partir de Perl 5.10, puede utilizar la opción de coincidencia C</p> y la variable C<${^MATCH}> para hacer lo mismo que antes para las operaciones de coincidencia.
...
Esta es similar a C<$&> (C<$MATCH>) excepto que no se incurrirá en la penalización de rendimiento asociada a esa variable, y se garantiza que sólo devolverá un valor definido cuando el patrón se compile o se ejecute con el modificador C</p>.
...
El uso de esta variable en cualquier parte de un programa impone una penalización de rendimiento considerable en todas las coincidencias de expresiones regulares.  Para evitar esta penalización, puede extraer la misma subcadena mediante el uso de L</@->.  A partir de Perl 5.10, puede utilizar la opción de coincidencia C</p> y la variable C<${^PREMATCH}> para hacer lo mismo que antes para las operaciones de coincidencia.
...
Esta es similar a C<$`> ($PREMATCH), excepto que no se incurrirá en la penalización de rendimiento asociada a esa variable, y se garantiza que sólo devolverá un valor definido cuando el patrón se compile o se ejecute con el modificador C</p>.
...
El uso de esta variable en cualquier parte de un programa impone una penalización de rendimiento considerable en todas las coincidencias de expresiones regulares.
Para evitar esta penalización, puede extraer la misma subcadena mediante el uso de L</@->.  A partir de Perl 5.10, puede utilizar la opción de coincidencia C</p> y la variable C<${^POSTMATCH}> para hacer lo mismo que antes para las operaciones de coincidencia.
...
Esta es similar a C<$'> (C<$POSTMATCH>) excepto que no se incurrirá en la penalización de rendimiento asociada a esa variable, y se garantiza que sólo devolverá un valor definido cuando el patrón se compile o se ejecute con el modificador C</p>.
...
Esto es útil si no sabe cuál de un conjunto de alternativas de patrones ha coincidido.  Por ejemplo:
...
El texto coincidente con el grupo de captura más recientemente cerrado (es decir, el grupo con el paréntesis de cierre más a la derecha) del último patrón de búsqueda con éxito.
...
Esta variable se incorporó en Perl 5.8.

Mnemónico: el paréntesis (posiblemente) anidado más recientemente cerrado.
...
B<Nota:> C<%-> y C<%+> están vinculadas en un hash interno común asociado con la última expresión regular exitosa.  Por lo tanto mezclar un acceso iterativo en ellas a través de C<each> puede tener resultados impredecibles.
...
Así, después de una concordancia contra C<$_>, C<$&> coincide con C<substr $_, $-[0],
$+[0] - $-[0]>.  Del mismo modo, $I<n> coincide con C<substr $_, $-[n], $+[n] - $-[n]> si C<$-[n]> está definida, y $+ coincide con C<substr $_, $-[$#-], $+[$#-] - $-[$#-]>.  Uno puede usar C<$#-> para encontrar el último subgrupo emparejado en la última coincidencia exitosa.  Compárelo con C<$#+>, el número de subgrupos en la expresión regular.  Compárelo con C<@+>.
...
El comportamiento de C<%-> es proporcionado por el módulo L<Tie::Hash::NamedCapture>.
...
El resultado de la evaluación de la última aserción de expresión regular C<(?{ código })> exitosa (vea L<perlre>).  Y puede escribirse.
...
Controla el modo en que se aplican ciertas optimizaciones de expresiones regulares y la cantidad de memoria que utilizan.  Este valor es, por defecto, 65536, que corresponde a una memoria caché temporal de 512kB.  Ponga esto a un valor más alto para intercambiar memoria por velocidad cuando busque en alternancias de gran tamaño.  Ajústelo a un valor inferior si desea que las optimizaciones sean más conservadores en cuanto a la memoria tanto como sea posible, y póngala a un valor negativo para evitar la optimización y conservar la mayor cantidad de memoria.
...
=head2 Variables relacionadas con gestores de archivo

Las variables que dependen del actualmente gestor de archivo seleccionado puede ser asignadas llamando a un método de objeto correspondiente al objeto C<IO::Handle>, aunque esto es menos eficaz que usando las variables integradas.  (Para resumir, en las líneas siguientes se usará la palabra GESTOR).
...
Debido a que la carga de la clase C<IO::Handle> es una operación costosa, debe aprender a utilizar las habituales variables integradas.
...
Es el gestor de archivo especial que itera sobre los nombres de archivos de la línea de comandos en C<@ARGV>.  Por lo general, escrito como el gestor de archivo nulo con el operador de ángulo C<< <> >>.  Tenga en cuenta que en la actualidad C<ARGV> sólo tiene su efecto mágico con el operador C<< <> >>; en otras situaciones es sólo un gestor de archivo simple correspondiente al último archivo abierto por C<< <> >>.  Dicho sea de paso, pasar C<\*ARGV> como un parámetro a una función que espera un gestor de archivo, no hace que pueda conseguir que su función pueda leer de forma automática el contenido de todos los archivos que están en C<@ARGV>.

=item ARGVOUT
X<ARGVOUT>

Es el gestor de archivo especial que apunta al actual archivo de salida abierto al hacer edición in-situ al procesarlo con B<-i>.  Es útil cuando tiene que hacer un montón de inserciones y no quiere seguir modificando C<$_>.  Consulte L<perlrun> para ver la opción B<-i>.

=item Gestor->output_field_separator( EXPR )
...
=item GESTOR->input_line_number( EXPR )
...
Número de línea actual para el gestor de archivo accedido por última vez.

Cada gestor de archivo en Perl cuenta el número de líneas que se han leído de él.  (Dependiendo del valor de C<$/>, la idea de Perl de lo que constituye una línea puede que no coincida con la suya).  Cuando una línea se lee desde un gestor de archivo (a través de C<readline()> o C<< <> >>), o cuando se llama a C<tell()> o C<seek()>, C<$.> se convierte en un alias para el contador de líneas para el gestor de archivo.

Se puede ajustar el contador mediante la asignación a C<$.>, pero esto realmente no moverá el puntero de búsqueda.  I<Localizar C<$.> no localizará el contador de líneas del gestor de archivo>.  En su lugar, se localizará la noción de perl del gestor de archivo al que C<$.> está referenciando.

C<$.> es reiniciado cuando el gestor de archivo es cerrado, pero B<no> cuando un gestor de archivo abierto es reabierto sin intervención de C<close()>.  Para más detalles, vea L<perlop/"Operadores EE<sol>S">.  Debido a que C<< <> >> nunca hace un cierre explícito, los números de línea aumentan a lo largo de los archivos C<ARGV> (pero vea los ejemplos en L<perlfunc/eof>).

También puede utilizar C<< GESTOR->input_line_number(EXPR) >> para acceder al contador de líneas de un gestor de archivo determinado sin tener que preocuparse de qué gestor fue accedido por última vez.

Mnemónico: muchos programas usan "." como el actual número de línea.

=item GESTOR->input_record_separator( EXPR )
...
El separador de registros a la entrada; de forma predeterminada, los caracteres que forman la nueva línea.  Esto influye en la idea que tiene Perl de lo que es una "línea".  Funciona como la variable RS del B<awk>, incluyendo el tratamiento de líneas en blanco como un terminador si se establece a la cadena nula (una línea en blanco no puede contener espacios en blanco o tabuladores).  Puede ponerle una cadena con varios caracteres para que coincida con un terminador de múltiples caracteres, o un C<undef> para leer todo hasta al final del archivo.  Si se establece a C<"\n\n"> significa algo ligeramente diferente que si se establece a C<"">, si el archivo contiene líneas en blanco consecutivas.  Si se establece a C<""> tratará a dos o más líneas consecutivas vacías como una sola línea en blanco.  Si se establece a C<"\n\n"> asumirá ciegamente que el siguiente carácter de entrada pertenece al párrafo siguiente, incluso si se trata de un carácter de nueva línea.
    ...
    local $/;           # activa el modo succionador
    local $_ = <FH>;    # obtendremos aquí todo el archivo
    s/\n[ \t]+/ /g;

Recuerde: el valor de C<$/> es una cadena, no una expresión regular.  B<awk> tenía que ser mejor por algo.  

Estableciendo C<$/> a una referencia a un número entero, o un escalar conteniendo un número entero, o un escalar que es convertible a un número entero, intentará leer registros en lugar de líneas, siendo el tamaño máximo del registro el número entero referenciado.  Así que esto:

    local $/ = \32768; # o \"32768", o \$var_conteniendo_32768
    open my $fh, "<", $miarchivo or die $!;
    local $_ = <$fh>;

leerá un registro de no más de 32768 bytes desde $fh.  Si no está leyendo un archivo orientado a registros (o su sistema operativo no tiene archivos orientados a registros), entonces lo más probable es que obtenga una porción completa de los datos con cada lectura.  Si un registro es mayor que el tamaño del registro que ha establecido, obtendrá ese registro en pedazos.  Tratar de establecer el tamaño del registro a cero o menor hará que se lea en (el resto de) todo el archivo.

Solo en VMS, el registro pasa por alto las capas PerlIO y cualquier búfer asociado, por lo que no debe mezclar las lecturas de registros y no registros en el mismo gestor de archivos.  El modo de acceso por registro se puede mezclar con el modo línea sólo cuando la capa de almacenamiento en búfer está en uso para ambos modos.

Si realiza una lectura de un registro en un ARCHIVO con una capa de codificación, tal como C<:encoding(latin1)>o C<:utf8>, puede conseguir como resultado una cadena inválida, puede dejar el ARCHIVO situado entre caracteres en el flujo y quizás no pueda leer el número de bytes del archivo que ha especificado.  Este comportamiento puede cambiar sin previo aviso, en una futura versión de Perl.
...
=item GESTOR->output_record_separator( EXPR )
...
Las variables especiales para formatos son un subconjunto de los gestores de archivo.  Vea L<perlform> para obtener más información acerca de los formatos de Perl.
...
=item GESTOR->format_formfeed(EXPR)
...
=item GESTOR->format_page_number(EXPR)
...
=item GESTOR->format_lines_left(EXPR)
...
=item GESTOR->format_line_break_characters EXPR
...
=item GESTOR->format_lines_per_page(EXPR)
...
=item GESTOR->format_top_name(EXPR)
...
El nombre del actual formato de la cabecera de página para el actual canal de salida seleccionado.  El valor predeterminado es el nombre del gestor de archivo con C<_TOP> agregado.  Por ejemplo, el nombre del formato predeterminado de la cabecera para el gestor de archivos C<STDOUT> es C<STDOUT_TOP>.

Mnemónico: apunta a la cabecera de la página.

=item GESTOR->format_name(EXPR)
...
El nombre del actual formato de informe para el actual canal de salida seleccionado.  El nombre predeterminado del formato es el mismo que el del gestor de archivo.  Por ejemplo, el nombre del formato predeterminado para el gestor de archivos C<STDOUT> es C<STDOUT>.

Mnemonic: hermano de C<$^>.
...
Estado actual del intérprete.
	...
	$^S              Estado
	---------        -------------------
	undef            Interpretando un módulo o un eval()
	verdadero (1)    Ejecutando un eval()
	falso (0)        Cualquier otra cosa

El primer estado puede aparecer en los gestores C<$SIG{__DIE__}> y C<$SIG{__WARN__}>.
...
8 >>>), y C<$? & 127> devuelve qué señal, si hay alguna, por el que el proceso murió, y C<$? & 128> informa de cuando se ha producido un volcado del núcleo.

Además, si la variable C<h_errno> está soportada en C, su valor es devuelto a través de C<$?> si falla alguna función C<gethost*()>.

Si ha instalado algún gestor de señales para C<SIGCHLD>, el valor de C<$?> será, generalmente, incorrecto, fuera de ese gestor.
...
Mnemónico: ¿"@" dónde se encontrado el error de sintaxis?
...
El valor actual de la bandera asociada con la opción B<-c>.
Su uso principal es con B<-MO=...> para permitir al código alterar su comportamiento cuando es compilado, tal como por ejemplo hace C<AUTOLOAD> en tiempo de compilación, en lugar de lo más normal, después de haber sido cargado.  Haciendo C<$^C = 1> es similar a llamar a C<B::minus_c>.
...
La I<referencia de objecto> al objeto C<Encode> que se ha utilizado para convertir el código fuente a Unicode.  Gracias a este variable su I<script> Perl no tiene por qué estar escrito en UTF-8.  El valor predeterminado es I<undef>.  La manipulación directa de esta variable está sumamente desaconsejada.
...
El C<PerlInterpreter*> ha sido construido a través de C<perl_construct>.  Este valor está aquí principalmente para complementar y para uso a través de la variable subyacente de C C<PL_phase>.  No es realmente posible ser ejecutado el código Perl a no ser que la construcción del intérprete esté terminada.

=item START

Este es el tiempo de compilación completo.  Eso incluye, básicamente, cada bloque C<BEGIN> ejecutado directamente o indirectamente durante
el tiempo de compilación del nivel superior del programa.

Esta fase no es llamada "BEGIN" para evitar confusión con los bloques C<BEGIN>, ya que son ejecutados durante el tiempo de compilación de cualquier unidad de compilación, no solo en el nivel superior del programa.  Un nuevo tiempo de compilación local, que haya entrado en tiempo de ejecución, por ejemplo por construcciones como C<eval "use SomeModule">, no son fases globales del intérprete, y por tanto no quedan reflejadas en C<${^GLOBAL_PHASE}>.
...
Note también que no hay ningún valor para los bloques UNITCHECK.  Es porque ellos son ejecutados en cada unidad de compilación de forma individual, y por tanto no
es una fase global del intérprete.

No todos los programa tienen que pasar por cada todas las fases posibles, pero la transición de una fase a otra sólo puede suceder en el orden descrito en la lista anterior.
...
Esta variable contiene pistas en tiempo de compilación para el intérprete de Perl.  Al final de la compilación de un BLOQUE el valor de esta variable es restaurado al valor que tenía cuando el intérprete comenzó a compilar el BLOQUE.

Cuándo perl empieza a analizar cualquier construcción de bloque que proporciona un ámbito léxico
(p. ej., un cuerpo eval, archivo requerido, cuerpo de subrutina, cuerpo de bucle, o bloque
condicional), el valor actual de C<$^H> es salvado, pero su valor queda sin cambios.
Cuándo se ha completado la compilación del bloque, se recupera el valor salvado.
Entre los puntos donde su valor es salvado y restaurado, el código que se ejecuta dentro de los bloques BEGIN es libre de cambiar el valor de C<$^H>.

Este comportamiento proporciona la semántica del ámbito léxico, y es utilizado en, por ejemplo, el pragma C<use strict>.

Los contenidos tendrían que ser un entero; bits diferentes de él son utilizados para diferentes banderas pragmáticas.  Aquí hay un ejemplo:
...
Considere qué pasa durante la ejecución del bloque BEGIN.  Al llegar a este punto el bloque BEGIN ya ha sido compilado, pero el cuerpo de C<foo()> todavía está siendo compilado.  El nuevo valor de C<$^H> será por tanto visible sólo mientras el cuerpo de C<foo()> se está compilando.
...
El hash C<%^H> proporciona el mismo ámbito semántico que C<$^H>.  Esto lo hace útil para la implementación de I<pragmas> en ámbito léxico.  Vea L<perlpragma>.
...
Un módulo tendría que utilizar llaves únicas que empiecen con el nombre del módulo (el
nombre de su paquete principal) y un carácter "/".  Por ejemplo, un módulo C<Foo::Bar> tendría que utilizar llaves como C<Foo::Bar/baz>.
...
Mantiene información sobre las líneas del fuente en las que una subrutina está definida.

=item 0x20

Iniciar con un paso simple.
...
Proporciona nombres de "archivo" informativos para los eval basados en el lugar donde fueron compilados.

=item 0x200

Proporciona nombres informativos para las subrutinas anónimas basados en el lugar donde fueron compiladas.

=item 0x400

Salva líneas del código fuente en C<@{"_<$nombrearchivo"}>.
...
Esta variable es inicializada durante el arranque de Perl y después es de solo lectura.
...
Esta variable indica si una configuración regional UTF-8 fue detectada por perl en el arranque.  Esta información es utilizada por perl cuándo está en modo ajustar-utf8-a-local (como cuando corre con la opción C<-CL>); vea L<perlrun> para más información sobre esto.
...
Una vez que una variable sea eliminada, su uso provoca un error que le dice que la variable no está soportada.
...
Después de un ciclo de obsolescencia, su magia fue eliminada en Perl 5.10 y usarla ahora dispara un aviso: C<$# is no longer supported> (C<$# ya no está soportada>).
...
Retirada en Perl 5.10.

=item $*
X<$*>

C<$*> era una variable que podría ser utilizada para habilitar concordancia multilínea.
Después de un ciclo de obsolescencia, su magia fue eliminada en Perl 5.10.
...
Esta variable almacena el índice del primer elemento en un array, y el primer carácter en una subcadena.  El valor predeterminado es 0, pero podría teóricamente ser puesto a 1 para hacer que Perl se comporte más como B<awk> (o Fortran) cuándo se extraen subcadenas y cuándo se evalúan las funciones index() y substr().

Con la publicación de Perl 5, las asignaciones a C<$[> eran tratadas como directivas de compilación, y no podían influir en el comportamiento de cualquier otro archivo.
(Es por eso que sólo se le pueden asignar constantes en tiempo de compilación).
Su uso está sumamente desaconsejado.

Con anterioridad a Perl 5.10, la asignación a C<$[> podría ser vista desde ámbitos léxicos
externos en el mismo archivo, a diferencia de otras directivas de tiempo de compilación (como
L<strict>).  Utilizando local() en ella uniría su valor estrictamente a un bloque léxico.  Ahora tiene siempre un ámbito léxico.

A partir de Perl 5.16, está implementada por el módulo C<arybase>.  Vea L<arybase> para más detalles sobre su comportamiento.
...
Declarada obsoleta en Perl 5.12.
...
Vea L</$^V> para una representación más moderna de la versión Perl que permite comparaciones de cadena más cuidadosas.

La versión + nivel de parcheo / 1000 del intérprete Perl.  Esta variable puede ser usada para determinar si el intérprete de Perl que ejecuta un I<script> está en el rango correcto de versiones:

    warn "¡No hay suma de control!\n" if $] < 3.019;

La representación de punto flotante a veces puede llevar a comparaciones numéricas inexactas.

Vea también la documentación de C<use VERSION> y C<require VERSION> para una manera conveniente de fallar si el intérprete de Perl en ejecución es demasiado viejo.

Mnemónico: ¿Es la versión de perl correcta?
