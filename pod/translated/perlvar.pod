...
Desde la versión Perl 5.6.0, los nombres de variables en Perl pueden ser cadenas alfanuméricas que empiecen por caracteres de control (o mejor aún, por un acento circunflejo).
...
Pero no ponga
    ...
    @foo{$a,$b,$c}	# una porción -fíjese en el @-
...
Esta variable apareció por primera vez en perl v5.6.0; las versiones anteriores de perl verán un valor indefinido.  Antes de perl v5.10.0, C<$^V> se representó como un v-string.
...
Para convertir C<$^V> en su representación en cadena de caracteres, use la conversion C<"%vd"> de C<sprintf()>:
...
En Perl 5.10.0, se introdujeron la bandera de operación de coincidencia C</p> y las variables C<${^PREMATCH}>, C<${^MATCH}>, y C<${^POSTMATCH}>, que le permiten sufrir las penalizaciones sólo en los patrones marcados con C</p>.
...
No puede llamar a C<output_field_separator()> en un gestor, sólo como método estático.  Vea L<IO::Handle|IO::Handle>.
...
También puede utilizar C<< GESTOR->input_line_number(EXPR) >> para acceder al contador de líneas de un identificador de archivo determinado sin tener que preocuparse de qué identificador fue accedido por última vez.
...
No puede llamar a C<input_field_separator()> en un gestor, sólo como método estático.  Vea L<IO::Handle|IO::Handle>.
...
No puede llamar a C<output_record_separator()> en un gestor, sólo como método estático.  Vea L<IO::Handle|IO::Handle>.
...
Es el mismo gestor que usan C<$.>, C<tell> y C<eof> si no se les pasa ningún argumento.  Es también el gestor que se usa cuando Perl añade ", <STDIN> line 1" a un mensaje de error o advertencia.
...
No puede llamar a C<format_formfeed()> en un gestor, sólo como método estático.  Vea L<IO::Handle|IO::Handle>.
...
No puede llamar a C<format_line_break_characters()> en un gestor, sólo como método estático.  Vea L<IO::Handle|IO::Handle>.
...
Note que cuando se pasa a cadena, el texto siempre se devuelve como si estuvieran activados S<L<C<"use locale">|perllocale>> y S<L<C<"use bytes">|bytes>>.  Esto, probablemente cambie en v5.22.
...
La I<referencia de objecto> al objeto C<Encode> que se ha utilizado para convertir el código fuente a Unicode.  Gracias a este variable su script Perl no tiene por qué estar escrito en UTF-8.  El valor predeterminado es I<undef>.  Se desaconseja la manipulación directa de esta variable.
...
Los contenidos tendrían que ser un entero; bits diferentes de él se utilizan para diferentes banderas pragmáticas.  Aquí hay un ejemplo:
...
La versión + nivel de parcheo / 1000 del intérprete Perl.  Esta variable se puede usar para determinar si el intérprete de Perl que ejecuta un script está en el rango correcto de versiones:

    warn "No PerlIO\n" if $] lt '5.008';

La representación de punto flotante a veces puede llevar a comparaciones numéricas inexactas, así que lo recomendado es hacer comparaciones de cadenas de caracteres.
