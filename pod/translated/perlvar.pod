...
El número de proceso de Perl ejecutando este script.  A pesar de que I<puede> darle un valor a esta variable, por lo general se desaconseja hacerlo, aunque puede resultar muy útil para algunas comprobaciones de pruebas.  Será restablecerá automáticamente a través de las llamadas C<fork()>.
...
La I<referencia de objecto> al objeto C<Encode> que se ha utilizado para convertir el código fuente a Unicode.  Gracias a este variable su script Perl no tiene por qué estar escrito en UTF-8.  El valor predeterminado es I<undef>.  La manipulación directa de esta variable está sumamente desaconsejada.
...
La versión + nivel de parcheo / 1000 del intérprete Perl.  Esta variable puede ser usada para determinar si el intérprete de Perl que ejecuta un script está en el rango correcto de versiones:
