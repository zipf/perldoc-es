...
Si está leyendo esto a partir de estas fechas, entonces es posible que el estado del arte haya cambiado. Recomendamos que empiece leyendo el documento perlootut de la última distribución estable de Perl, en lugar de esta versión.
...
Si prueba C<Moose> y encuentra que algunas de estas características impiden su uso, le recomendamos entonces el uso de L<Moo>. C<Moo> implementa un subconjunto de la funcionalidad de C<Moose> en un paquete más simple.
La interfaz final I<es la misma> para todas las características que implementa, de forma que es posible cambiar de C<Moo> a C<Moose>, de forma sencilla.

C<Moo> no implementa la mayor parte de la interfaz de introspección de C<Moose>, por lo que la carga de módulos resulta más rápida. Además, ninguna de sus dependencias requieren XS, así que puede instalarse en máquinas que no tengan compilador.

Una de las características más destacadas de C<Moo> es su interoperabilidad con C<Moose>. Cuando alguien intenta usar la API de instrospección de C<Moose> en una clase o rol de C<Moo>, se infla de forma transparente a una clase o role de C<Moose>. Esto hace más fácil incorporar código de C<Moo> en código basado en C<Moose>, y viceversa.

Por ejemplo, una clase C<Moose> puede crear una subclase a partir de una clase C<Moo> usando C<extends>, o consumir un rol C<Moo> usando C<with>.

Los autores de C<Moose> esperan que algún día, cuando C<Moose> haya mejorado lo suficiente, C<Moo> pueda quedar obsoleto, pero de momento proporciona una sólida alternativa a C<Moose>.
...
Por último, tenemos L<Class::Tiny>. Este módulo se ajusta realmente a su nombre. Tiene una interfaz de programación realmente mínima y carece de dependencias con cualquier Perl reciente. Sin embargo, es mucho más sencillo usar este módulo que escribir código orientado a objetos desde cero.
...
Con C<Class::Tiny> todos los métodos de acceso son de solo lectura. También genera automáticamente un constructor y todos los métodos de acceso que defina.

También puede usar L<Class::Tiny::Antlers> para tener una sintaxis parecida a C<Moose>.
...
Aun así es útil y funciona bien con C<Class::Accessor> y C<Class::Tiny>
...
C<Moo> es una versión reducida de C<Moose> que ofrece una alternativa razonable cuando Moose no es adecuado para su aplicación.
...
Use C<Role::Tiny> con C<Class::Tiny> o C<Class::Accessor> si desea usar herencia múltiple. Si se decanta por C<Moose>, podrá disponer de su implementación de roles.
...
Para sistemas pequeños, tanto L<Class::Tiny> como L<Class::Accessor> ofrecen sistemas minimalistas de objetos que se ocupan de las tareas repetitivas y simplifican el trabajo.
...
Le recomendamos que pruebe L<Moose>, L<Class::Accessor> y L<Class::Tiny> para decidir qué sistema de orientación a objetos le conviene más.
