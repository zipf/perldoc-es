...
Binario C<+> devuelve la suma de dos números.
X<+>

Binario C<-> devuelve la diferencia de dos números.
X<->

Binario C<.> concatena dos cadenas.
...
Binario C<<< << >>> devuelve el valor de su argumento de la izquierda, desplazado a la izquierda por el número de bits especificado por el argumento de la derecha.  Los argumentos deben ser números enteros.  (Ver también L<Aritmética entera>).

Binario C<<< >> >>> devuelve el valor de su argumento de la izquierda, desplazado a la derecha por el número de bits especificado por el argumento de la derecha.  Los argumentos deben ser números enteros.  (Ver también L<Aritmética entera>).
...
(Tenga en cuenta que los pragmas L<bigint>, L<bigrat>, y L<bignum> soportan "NaN").
...
La entrada del código "igual que" no siempre es una interpretación exacta.  Por ejemplo, el operador de coincidencia inteligente cortocircuita cuando sea posible, pero C<grep> no lo hace.  Además, C<grep> en contexto de escalar devuelve el número de coincidencias, pero C<~~> sólo devuelve verdadero o falso.
...
Eso es así porque la correspondiente posición en C<@a> contiene un array que (eventualmente) tiene un 4 en ella.
...
El operador puede ser asignado si tanto el segundo y tercer argumentos son valores-izquierda legales (lo que significa que les puede asignar valores):
...
El operador C<< => >> es un sinónimo de la coma, salvo que causa que su operando de la izquierda debe interpretarse como una cadena si comienza con una letra o un guión bajo, y está exclusivamente integrado por letras, dígitos y guiones bajos.
...
C<\x{100}> y C<\o{400}> son, ambos dos, 256 en decimal, por lo que el número se interpreta como un punto de código Unicode no importa qué codificación nativa sea.  El nombre del carácter en la posición 256º (empezando en 0) en Unicode es C<LATIN CAPITAL LETTER A WITH MACRON>.
...
Si C<use locale> está en vigor (pero no C<use locale ':not_characters'>), el mapeo de tamaños de caja usado por C<\l>, C<\L>, C<\u> y C<\U> es tomado de la configuración regional actual.  Ver L<perllocale>.
...
Un formato de entrecomillado orientado a líneas basado en la sintaxis shell "here-document" (documento incrustado).  Siguiendo a C<< << >> especifica una cadena que termine el material entrecomillado, y todas las líneas siguientes a la actual hacia abajo hasta la cadena de terminación, son el valor del elemento.
...
En otros contextos booleanos, C<< <IDENTIFICADOR_ARCHIVO> >> sin una explícita prueba C<defined> o comparación, provoca una advertencia si está en vigor el I<pragma> C<use warnings> o la opción B<-w> en la línea de comandos (la variable C<$^W>).
...
El identificador de archivo nulo <> es especial: puede ser utilizado para emular el comportamiento de B<sed> y B<awk>, y cualquier otro programa de filtrado Unix que tome una lista de nombres de archivo, haciendo lo mismo en cada línea de la entrada para todos ellos.  La entrada desde <> puede venir, ya sea desde la entrada estándar, o de cada archivo listado en la línea de comandos.  He aquí cómo funciona: la primera vez que <> es evaluado, el array @ARGV es comprobado, y si está vacío, C<$ARGV[0]> es configurado a "-", que cuando sea abierto le ofrecerá a usted la entrada estándar.  Luego, el array @ARGV es procesado como una lista de nombres de archivo.  El bucle
