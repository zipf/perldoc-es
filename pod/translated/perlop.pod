...
En Perl, el operador determina qué operación se realiza, independiente del tipo de los operandos. Por ejemplo, C<$a + $b> siempre es una adición numérica, y si C<$a> o C<$b> no contienen números, se hace un intento para convertirles primero a números.

Esto contrasta con muchos otros lenguajes dinámicos, donde la operación se determina por el tipo del primer argumento. También significa que Perl tiene dos versiones de algunos operadores: uno para numérico y otro para cadenas de caracteres. Por ejemplo, C<$a == $b> compara dos números por su igualdad, y C<$a eq $b> comparas dos cadenas.
...
Si el valor absoluto del operando derecho (C<abs($b)>) es mayor o igual a C<(UV_MAX + 1)>, "%" computa el resto en punto flotante C<$r> en la ecuación C<($r = $a - $i*$b)> donde C<$i> es un cierto entero que hace que C<$r> tenga el mismo signo que el operando de la derecha C<$b> (B<no> como el operando izquierdo C<$a> como la función C C<fmod()>) y el valor absoluto menor que el de C<$b>.
...
Contexto escalar o lista es propagado hacia adelante, en el segundo o tercer argumento, cualquiera que sea el seleccionado.
    ...
    @a = $ok ? @b : @c;  # obtener un array
    $a = $ok ? @b : @c;  # oops, ¡esto sólo cuenta elementos!
...
En otras palabras, es el carácter cuyo código de punto era 64 xor con su mayúscula.  C<\c?> es DELETE porque C<ord("?") ^ 64> es 127, y C<\c@> es NULL porque el ord de "@" es 64, por lo que haciendo el xor 64 produce un 0.
...
Tener menos de 3 dígitos puede dar lugar a un mensaje de advertencia engañoso, que dice que lo que sigue es ignorado.  Por ejemplo, C<"\128"> en el juego de caracteres ASCII es equivalente a los dos caracteres C<"\n8">, pero se mostrará la advertencia I<Illegal octal digit '8' ignored> (C<Se ignora el carácter octal ilegal '8'>).  Si lo que quiere es C<"\n8">, puede evitar la aparición de esta advertencia rellenando su número octal con C<cero>s: C<"\0128">.
...
Ejemplos:
    ...
    $_ = 'abc123xyz';
    $a = s/abc/def/r;			# $a es 'def123xyz' y
