...
Si cualquier operador de lista (print(), etc.) o cualquier otro operador unario (chdir(), etc.) es seguido por un paréntesis de apertura como el siguiente I<token>, el operador y los argumentos entre paréntesis se considera que son de mayor precedencia, como en una llamada a una función normal.
...
Si cualquier operador de lista (print(), etc.) o cualquier otro operador unario (chdir(), etc.) es seguido por un paréntesis de apertura como el siguiente I<token>, el operador y los argumentos entre paréntesis se considera que son de mayor precedencia, como en una llamada a una función normal.  Por ejemplo, debido a que los operadores unarios, con nombre, tienen mayor precedencia que C<||>:
...
El binario "&" devuelve la operación AND, bit a bit, de sus operandos.
...
Como operador escalar:
    ...
    if (101 .. 200) { print; } # imprime las segundas cien líneas,
                               # versión corta de if ($. == 101 .. $. == 200) { print; }
...
Sin embargo, debido a existen I<muchos> otros caracteres griegos en minúscula aparte de estos, para buscar por caracteres griegos en minúscula en una expresión regular, se utiliza el patrón C</(?:(?=\p{Greek})\p{Lower})+/>.
...
Los últimos cuatro modificadores enumerados anteriormente, agregados en Perl 5.14, controlan las semánticas del juego de caracteres, pero C</a> es el único que querrá especificar explícitamente; los otros tres son seleccionados automáticamente por medio de diversos I<pragmas>.
...
Las opciones son como se describe anteriormente en C<qr//>; además, los siguientes modificadores de coincidencia están disponibles:
 ...
 c	No reinicia la posición de búsqueda en una coincidencia fallida cuando /g está activa.
...
C<\Q>, C<\U>, C<\u>, C<\L>, C<\l>, C<\F> (posiblemente emparejadas con C<\E>) se convierten a las correspondientes construcciones Perl.  Así, C<"$foo\Qbaz$bar"> se convierte internamente a C<$foo . (quotemeta("baz" . $bar))>.
...
El terminador de esta construcción es encontrado usando las mismas reglas que para encontrar el terminador de la construcción delimitada por llaves, C<{}>, con la única excepción de que C<]> inmediatamente después de un C<[> se trata como si estuviera precedido por una barra diagonal inversa.  Del mismo modo, el terminador de C<(?{...})> se encuentra usando las mismas reglas que para encontrar el terminador de una construcción delimitada por C<{}>.
