...
Este documento proporciona información detallada sobre el uso de git para el desarrollo de Perl. Si solo quiere trabajar en una corrección rápida, consulte primero L<perlhack>.
...
Si no puede usar el protocolo git a causa de un firewall, puede clonarlo a través de http, aunque este método es mucho más lento:
...
Las ramas que empiezan por "origin" corresponden al "git remoto" del que ha generado un clon (se llama "origin"). Estas ramas harán un seguimiento exacto de cada rama del repositorio remoto. NUNCA debe trabajar en estas ramas de seguimiento remoto. Solo se trabaja en la rama local. Las ramas locales se pueden configurar para realizar una combinación automática (al traer cambios) desde una rama de seguimiento remoto designada. Este es el caso de la rama predeterminada, C<blead>, que se configurará para combinarse desde la rama de seguimiento remoto C<origin/blead>.

Para ver los depósitos recientes:
...
Ahora deposite el cambio localmente:
...
Ahora debe crear un archivo de revisión para todos los cambios locales:
...
(El modificador C<-a> indica a git que debe agregar ///to this commit todos los archivos modificados.
...
=head2 Uso de git para enviar mensajes de correo electrónico con revisiones
...
Con esta configuración, se le pedirá su contraseña de gmail cuando ejecute 'git send-email'. También puede configurar C<sendemail.smtppass> con su contraseña si no le importa almacenarla en el archivo .gitconfig.
...
El comando C<git clean> se puede usar con diversos argumentos como sustituto de used C<make clean>.
...
Make sure you have a good commit message. Encontrará información detallada en L<perlhack/Commit message>///.
