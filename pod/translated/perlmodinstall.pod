...
Puede pensar en un módulo como la unidad fundamental de código Perl reutilizable; encontrará más información en L<perlmod>.  Si crea un fragmento de código Perl que considere que puede ser útil para otros programadores, debe registrarse como desarrollador de Perl en http://www.cpan.org/modules/04pause.html y subir el código a CPAN.  El CPAN es la red amplia de archivos Perl (I<Comprehensive Perl Archive Network>) y se puede acceder en http://www.cpan.org/ , y búsquedas en http://search.cpan.org/ .

Esta documentación es para las personas que quieren descargar e instalar módulos de CPAN.

=head2 PREÁMBULO

En primer lugar, ¿está seguro de que el módulo no está ya en el sistema?  Pruebe C<perl -MFoo -e 1>.  (Reemplace "Foo" con el nombre del módulo; por ejemplo, C<perl -MCGI::Carp -e 1>.
...
Así que ahora tiene un archivo con extensión .tar.gz (o, menos frecuente, .zip).  Sabe que hay un sabroso módulo en su interior.  Ahora debe realizar cuatro pasos:
...
B<Si está en un sistema Unix o parecido a Unix>,
...
Descomprima el archivo con C<gzip -d sumodulo.tar.gz>
...
La mayoría de los sistemas Unix disponen de vinculación dinámica. Si el suyo no lo tiene, o si por otra razón tiene un perl enlazado estáticamente, B<y> el módulo requiere compilación, tendrá que construir un nuevo binario Perl que incluye el módulo.  Una vez más, probablemente tendrá que ser root.

=item *

B<Si usa ActivePerl (Win95/98/2K/NT/XP, Linux, Solaris)>

En primer lugar, escriba C<ppm> desde una consola y vea si en algún repositorio PPM de ActiveState está su módulo.  Si es así, puede instalarlo con C<ppm> y no tendrá que molestarse con ninguno de los siguientes pasos.  Podría intentar usar las instrucciones CPAN de la sección "Unix o Linux" anterior; dele una oportunidad.  De lo contrario, tendrá que llevar a cabo los pasos siguientes.
...
¿El módulo requiere de compilación (es decir, contiene archivos con la extensión .xs, .c, .h, .y, .cc, .cxx o .C)?  Si lo hace, la vida se le ha vuelto oficialmente difícil, porque tiene que compilar el módulo por sí mismo (que no es tarea fácil en Windows).  Necesitará un compilador, como Visual C++.  Como alternativa, puede descargar un paquete PPM precompilado desde ActiveState.
...
En primer lugar, asegúrese de que tiene la última distribución B<cpan-mac> ( http://www.cpan.org/authors/id/CNANDOR/ ), que tiene utilidades para hacer todos los pasos.  Lea atentamente las instrucciones de cpan-mac e instálelo.  Si decide no utilizar cpan-mac por alguna razón, hay algunas alternativas listadas a continuación.
...
en los archivos de código fuente.
...
Necesitará los paquetes indicados en F<README.dos>, en la distribución Perl.
...
o, para módulos comprimidos, escriba
...
Tenga en cuenta que la herramienta gzip/gunzip de GNU no es lo mismo que la heramienta zip/unzip de Info-ZIP.  La primera es una simple herramienta de compresión; la segunda permite la creación de archivos multi-fichero.
