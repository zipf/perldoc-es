...
perlrecharclass - Clases de carácter en expresiones regulares Perl

=head1 DESCRIPCIÓN

La documentación de alto nivel acerca de expresiones regulares de Perl se encuentra en L<perlre>.

En esta página de manual se describe la sintaxis y uso de las clases de carácter en las expresiones regulares en Perl.
...
Una secuencia de barra diagonal inversa es una secuencia de caracteres, en que el primero es una barra diagonal inversa.  Perl atribuye un significado especial a tales secuencias, y algunas de ellas son clases de carácter.  Es decir, cada una de ellas coinciden con un único carácter, que pertenece al conjunto de caracteres definido por la secuencia.
...
A partir de Perl v5.18, experimentalmente, también coincide con el tabulador vertical, C<\cK>.
...
C<\s> coincide con [\t\n\f\r ] y, experimentalmente, a partir de Perl v5.18, el tabulador vertical, C<\cK>.
(Vea la nota C<[1]> más abajo, sobre una explicación sobre esto).
...
C<\R> coincide con cualquiera cosa que se considere una nueva línea bajo las reglas Unicode. No es una clase carácter, ya que puede coincidir con una secuencia de varios bytes. Por lo tanto, no puede utilizarse dentro de una clase de caracteres entre corchetes; use en su lugar C<\v> (espacio en blanco vertical).  Usa el conjunto de caracteres nativo de la plataforma, y no considera ninguna configuración regional que podría estar en uso.
...
Antes de Perl v5.18, C<\s> no coincidía con el tabulador vertical.  El cambio en v5.18 se considera un experimento, que significa que podría volverse atrás en v5.22 si la experiencia indica que rompe demasiado código existente.  Si este cambio le afecta negativamente, mande un correo electrónico a C<perlbug@perl.org>; si le afecta positivamente, envíelo a C<perlthanks@perl.org>.  Mientras tanto, C<[^\S\cK]> coincide (oscuramente) con lo que tradicionalmente coincidía C<\s>.
...
* Hay una excepción a la clase de caracteres entre corchetes coincidiendo con solo un único carácter.  Cuando a la clase se le indica que coincida independientemente del tamaño de caja, bajo las reglas de coincidencia de C</i>, y un carácter que se menciona explícitamente dentro de la clase coincide con una secuencia multi-carácter independiente del tamaño de caja bajo las reglas Unicode, la clase (cuando no se L<niega|/Negación>) también coincidirá con la secuencia.  Por ejemplo, Unicode dice que la letra C<LATIN SMALL LETTER SHARP S> debe coincidir con la secuencia C<ss> bajo las reglas C</i>.  Por lo tanto,

 'ss' =~ /\A\N{LATIN SMALL LETTER SHARP S}\z/i             # Coincide
 'ss' =~ /\A[aeioust\N{LATIN SMALL LETTER SHARP S}]\z/i    # Coincide

Para que esto suceda, el carácter debe especificarse explícitamente, y no ser parte de un rango multi-carácter (incluso como uno de sus puntos de código finales).
(L</Rangos de carácter> serán explicados dentro de poco).  Por tanto,

 'ss' =~ /\A[\0-\x{ff}]\z/i        # No coincide
 'ss' =~ /\A[\0-\N{LATIN SMALL LETTER SHARP S}]\z/i    # No coincide
 'ss' =~ /\A[\xDF-\xDF]\z/i    # Coincide en plataformas ASCII, desde \XDF
                               # es LATIN SMALL LETTER SHARP S, y el
                               # rango es un único elemento
...
Las secuencias C<\a>, C<\c>, C<\e>, C<\f>, C<\n>, C<\N{I<NAME>}>, C<\N{U+I<hex char>}>, C<\r>, C<\t> y C<\x> son también especiales y tienen los mismos significados que tienen fuera de la clase de caracteres entre corchetes.  (Sin embargo, dentro de una clase de caracteres entre corchetes, si C<\N{I<NOMBRE>}> se expande a una secuencia de caracteres, solo se usa el primero de la secuencia, con una advertencia).
...
Si un guión, en una clase de caracteres no puede, sintácticamente, ser parte de un rango, por ejemplo porque sea el primer o el último carácter de la clase de caracteres, o si sigue inmediatamente a un rango, el guión no es especial, y así que se
considera como un carácter con el que se coincidirá literalmente.  Si quiere un guión en su conjunto de caracteres para que sea emparejado, y su posición en la clase es tal que podría ser considerado parte de un rango, debe escaparlo con una barra diagonal inversa.
...
En clases de carácter entre corchetes, invertidas, Perl ignora las reglas Unicode que, normalmente dice que ciertos caracteres deben coincidir con una secuencia de caracteres múltiples bajo una coincidencia independiente del tamaño de caja C</i>.  Seguir esas reglas podría llevar a situaciones altamente confusas:
...
Las clases de carácter con barras diagonales inversas no pueden formar uno de los puntos finales de un rango.  Así, no se puede decir:

 /[\p{Thai}-\d]/     # ¡Error!

=head3 Clases de carácter POSIX
X<character class> X<\p> X<\p{}>
X<alpha> X<alnum> X<ascii> X<blank> X<cntrl> X<digit> X<graph>
X<lower> X<print> X<punct> X<space> X<upper> X<word> X<xdigit>

Las clases de carácter POSIX tiene la forma C<[:clase:]>, donde I<clase> es el nombre, y el C<[:> y C<:]> delimitadores. Las clases de carácter POSIX solo aparecen I<dentro> de las clases de carácter entre corchetes, y son una forma conveniente y descriptiva de listar un grupo de caracteres.
...
Perl reconoce las siguientes clases de carácter POSIX:
...
La mayor parte de las clases de carácter POSIX tienen dos estilos de propiedades C<\p> Unicode homólogas.  (No son propiedades Unicode oficiales, sino extensiones Perl derivadas de propiedades Unicode oficiales).  La tabla más abajo muestra la relación entre las clases de carácter POSIX y sus homólogas.
...
En el rango ASCII, los caracteres cuyos puntos de código están entre 0 y 31, inclusive, más el 127 (C<DEL>), son caracteres de control.
...
C<\p{SpacePerl}> y C<\p{Space}> coinciden con lo mismo, a partir de Perl v5.18.  En versiones anteriores, diferían solo en la coincidencia no-regional, C<\p{SpacePerl}> no coincide con el tabulador vertical, C<\cK>.
...
=head4 Negación de clases de carácter POSIX
...
Perl reconoce las clases de carácter POSIX C<[=clase=]> y C<[.class.]>, pero (¿aún?) no las soporta.  Cualquier intento de usar esta construcción levantará una excepción.
...
=head3 Clases de carácter entre corchetes extendidas
X<character class>
X<set operations>
...
Todos los operadores binarios se asocian por la izquierda, y tienen la misma precedencia.
El operador unario se asocia por la derecha, y tiene la mayor precedencia.  Use los paréntesis para sobrepasar el orden de las asociaciones.  Algunos comentarios que hemos recibido indican un deseo de que la intersección tenga una precedencia mayor que la unión.  Esto es algo que los comentarios desde la trinchera puede hacernos cambiar en futuros lanzamientos; quizás quiera poner paréntesis de forma copiosa para evitar que los futuros cambios afecten a su código, hasta que esta característica ya no se considere experimental.
...
Todos los otros escapes aceptados por las clases de carácter normales entre corchetes también se aceptan aquí; pero los escapes no reconocidos que en las clases de carácter normales generan advertencias, aquí son errores fatales.
...
La diferencia final entre clases de carácter regulares entre corchetes y éstas, es que no es posible que éstas coincidan con un plegado multicarácter.  Por lo tanto,
...
Cualesquiera clases de carácter POSIX incluidas, incluyendo cosas como C<\w> y C<\D> respetan los modificadores C<E<sol>a> (y C<E<sol>aa>).
...
El procesado de C<E<sol>x> dentro de esta clase, es un formato extendido.
Además de los caracteres que se consideran espacio blanco en el procesamiento normal de C</x>, hay otros cinco, recomendados por el estándar Unicode:
...
Podemos cambiarlo de modo que cosas que siguen siendo legales en clases de carácter normales entre corchetes podrían volverse ilegales dentro de esta construcción experimental.  Una propuesta, por ejemplo, es el de prohibir usos adyacentes del mismo carácter, como en C<(?[ [aa] ])>.  La motivación para tal cambio es que su uso es como el de una errata, ya que la segunda "a" no añade nada.
