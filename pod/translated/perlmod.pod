...
Perl proporciona un mecanismo para los espacios de nombres alternativos, de forma que se evite que los paquetes se pisen unos a otros sus variables.  De hecho, no hay nada parecido a variables globales en Perl.  La sentencia del paquete declara la unidad de compilación a usar en el espacio de nombres dado.  El ámbito de la declaración de paquete alcanza desde la declaración misma hasta el final del bloque de cierre, C<<eval> o archivo, el primero que aparezca (el mismo ámbito que el de los operadores my() y local()).  Los identificadores dinámicos inhábiles estarán en el mismo espacio de nombres, excepto aquellos pocos identificadores que si no son hábiles, se usan por defecto los del paquete principal en lugar de los del actual, tal y como se indica a continuación.  Una sentencia de paquete afecta únicamente a las variables dinámicas, incluyendo aquellas usadas mediante local(), pero I<o> las variables léxicas creadas con my().  Normalmente será la primera declaración en un archivo incluido por medio de los operadores C<do>, C<require> o C<use>.  Puede cambiar a un paquete en más de un lugar; únicamente influye en qué tabla de símbolos se usa en el resto del bloque.  Puede referirse a variables y gestores de archivo en otros paquetes usando el prefijo en el identificador con el nombre del paquete y dos puntos: C<$Paquete::Variable>.  Si el nombre del paquete se omite, se asume que el paquete es C<main>.  Es decir, C<$::vela> equivale a C<$main::vela>.
