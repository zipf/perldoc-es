...
=head2 ¿Es este el documento que debe leer?

Existen otros documentos que podrían contener la información por la que está buscando:

=over 2

=item Este documento

Paquetes de Perl, espacios de nombres, y alguna información sobre clases.

=item L<perlnewmod>

Tutorial sobre cómo hacer un nuevo módulo

=item L<perlmodstyle>

Buenas prácticas para hacer un nuevo módulo.
...
El programa B<begincheck> expone esto de forma clara (o eso esperamos):
  ...
  print         "13.   Solo _parece_ lioso, pero no lo es en realidad.\n";
...
Perl agrega compatibilidad con un nuevo tipo de hilos de ejecución, denominado hilos de ejecución de intérprete (I<ithreads>).
Estos hilos de ejecución se pueden usar de forma explícita e implícita.

Los hilos de ejecución de intérprete clonan el árbol de datos de forma que no haya datos compartidos entre distintos hilos de ejecución. Estos hilos de ejecución pueden usarse con el módulo C<threads> o bien ejecutando fork() en win32 (en realidad, una simulación de fork()). Cuando se clona un hilo de ejecución, se clonan también todos los datos de Perl. Sin embargo, los datos que no pertenecen a Perl no se pueden clonar automáticamente.  Desde la versión 5.8.0 se tiene soporte a la subrutina especial C<CLONE>.  C<CLONE> le permite hacer todo lo que necesite, como controlar la clonación de datos que no sean de Perl, si es necesario.
