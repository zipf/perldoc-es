...
C<given> es análogo a la palabra clave C<switch>, de otros lenguajes. C<given> y C<when> se utilizan en Perl para implementar sentencias parecidas a C<switch>/C<case>.
...
C<when> es análogo a la palabra clave C<case>, de otros lenguajes. Usando con un bucle C<foreach> o el experimental bloque C<given>, C<when> puede ser usado en Perl para implementar sentencias parecidas a C<switch>/C<case>.  Disponible como una sentencia después de Perl 5.10 y como un modificador de sentencia después de 5.14.  
