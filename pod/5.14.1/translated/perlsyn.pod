...
La palabra clave C<foreach> es realmente un sinónimo de la palabra clave C<for>, así que puede usar C<foreach> por legibilidad o C<for> por brevedad.  (O si le es más familiar el shell Bourne que el I<csh>, escribir C<for> le será más natural). Si VAR se omite, C<$_> se inicializa con cada valor.
X<$_>
...
que activa la característica conmutador, que se basa estrechamente en la propuesta en Perl 6.  A partir de Perl 5.16, se puede cambiar el prefijo de las palabras clave con C<CORE::> para acceder a las nuevas características sin tener que usar una declaración C<use feature>.

Las palabras clave C<given> y C<when> son análogos a C<switch> y C<case> en otros lenguajes, por lo que el código anterior podrá ser escrito como
...
Además, Perl inspecciona los operandos de los operadores booleanos binarios para decidir si se debe usar coincidencia inteligente para cada uno de ellos mediante la aplicación de la prueba anterior a los operandos:

=over 4

=item *

Si EXPR es C<... && ...> o C<... and ...>, el test se aplica recursivamente a ambos operandos. Si I<ambos> operandos pasan el test, entonces la expresión es tratada como un booleano; de otra forma, se usará coincidencia inteligente.

=item *

Si EXPR es C<... || ...>, C<... // ...> o C<... or ...>, el test se aplica recursivamente al primer operando (que puede ser un operador de mayor precedencia, como el AND, por ejemplo). Si el primer operando utiliza coincidencia inteligente, entonces se aplicará a ambos operandos; si no es así, entonces, ni el segundo argumento lo usará.

=back

Estas reglas parecen complicadas, pero usualmente harán lo que usted desea. Por ejemplo:
...
usará coincidencia inteligente, porque sólo I<uno> de los operandos es un valor booleano; los otros utilizan coincidencia inteligente, y el resultado será verdadero.
...
pondrá a prueba sólo la expresión regular, lo que hace que ambos operandos sean tratados como booleanos.
Tenga cuidado con esto, porque una referencia a un array es siempre un valor verdadero, lo que, efectivamente, es redundante.

Los operadores booleanos tautológicos tienen que ser optimizados. No caiga en la tentación de escribir

    when ('foo' or 'bar') { ... }

Esto quedará optimizado en C<'foo'>, así que C<'bar'> nunca será considerado (incluso aunque las reglas digan que hay que usar coincidencia inteligente en C<'foo'>). Para una alternativa como ésta, una referencia a un array funcionará, porque esto lanzará a la coincidencia inteligente:

    when ([qw(foo bar)] { ... }

Esto es algo equivalente a la funcionalidad de la sentencia C<switch> del C (no debe confundirse con la funcionalidad de I<Perl>; véase más adelante), en donde el mismo bloque se utiliza para varias declaraciones C<case>.
...
Cuando una sentencia C<given> es también una expresión válida (p.e. cuando es la última sentencia de un bloque), se evalúa a:

=over 4

=item *

una lista vacía tan pronto como se encuentre una sentencia C<break>.

=item *

el valor de la última expresión evaluada de la exitosa cláusula C<when>/C<default>, si hay alguna.
...
Tenga en cuenta que, a diferencia de C<if> y C<unless>, sentencias fallidas de C<when> siempre se evalúan a una lista vacía.

    my $precio = do { given ($item) {
	when ([ 'pera', 'manzana' ]) { 1 }
	break when 'voto';      # Mo voto no puede ser comprado
        1e10  when /Mona Lisa/;
        'desconocido';
    } };
...
=head2 Antiguos comentarios (¡Pero no tanto!)
